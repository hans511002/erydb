...
void WriteOnceConfig::initializeDefaults()
{
	...
	fSharedMemoryTmpFile1 = make_pair("/tmp/erydbShm,", false);
	fTxnIDFile =            make_pair("/mnt/OAM/dbrm/SMTxnID", false);
	fSharedMemoryTmpFile2 = make_pair("/tmp/erydbSessionMonitorShm", false);
}
...
ByteStream WriteOnceConfig::load()
{
	...
	erydbassert(access(fConfigFileName.c_str(), F_OK) == 0);
