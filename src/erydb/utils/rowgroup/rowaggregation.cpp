...
void RowAggregation::updateStringMinMax(string val1, string val2, int64_t col, int func)
{
	...
	{
		int tmp = funcexp::utf8::erydb_strcoll(val1.c_str(), val2.c_str());
...
inline void RowAggregation::updateIntSum(int64_t val1, int64_t val2, int64_t col)
{
	...
	else
	{
		...
		if (fRow.getColTypes()[col] != execplan::erydbSystemCatalog::DOUBLE &&
			fRow.getColTypes()[col] != execplan::erydbSystemCatalog::UDOUBLE)
		{
			if (((val2 >= 0) && ((numeric_limits<int64_t>::max() - val2) >= val1)) ||
				((val2 <  0) && ((numeric_limits<int64_t>::min() - val2) <= val1)))
			{
			...
			{
				execplan::erydbSystemCatalog::ColDataType* cdtp = fRow.getColTypes();
				cdtp += col;
				*cdtp = execplan::erydbSystemCatalog::DOUBLE;
...
void RowAggregation::doBitOp(const Row& rowIn, int64_t colIn, int64_t colOut, int funcType)
{
	...
	switch (colDataType)
	{
		...
		case execplan::erydbSystemCatalog::UDECIMAL:
		{
			...
			{
				valIn = rowIn.getIntField(colIn);
				valIn /= ERYDB_pow[fRowGroupIn.getScale()[colIn]-1];
				if (valIn > 0)
					valIn += 5;
				else if (valIn < 0)
					valIn -= 5;
		...
		{
			double dbl = 0.0;
			if (colDataType == execplan::erydbSystemCatalog::DOUBLE ||
				colDataType == execplan::erydbSystemCatalog::UDOUBLE)
		...
		case execplan::erydbSystemCatalog::UDECIMAL:
		{
			...
			else
			{
				...
				if (((valOut >= 0) && ((numeric_limits<int64_t>::max() - valOut) >= valIn)) ||
					...
					fRow.setIntField(valIn + valOut, colOut);
#else /* PROMOTE_AGGR_OVRFLW_TO_DBL */
				if (fRow.getColTypes()[colOut] != execplan::erydbSystemCatalog::DOUBLE)
				{
					if (((valOut >= 0) && ((numeric_limits<int64_t>::max() - valOut) >= valIn)) ||
						((valOut <  0) && ((numeric_limits<int64_t>::min() - valOut) <= valIn)))
					{
...
void RowAggregation::doStatistics(const Row& rowIn, int64_t colIn, int64_t colOut, int64_t colAux)
{
	...
	switch (colDataType)
	{
		...
		case execplan::erydbSystemCatalog::INT:
		case execplan::erydbSystemCatalog::BIGINT:
		case execplan::erydbSystemCatalog::DECIMAL:   // handle scale later
		case execplan::erydbSystemCatalog::UDECIMAL:  // handle scale later
				...
				switch (colDataType)
				{
					...
					case execplan::erydbSystemCatalog::UDECIMAL:
...
#else /* PROMOTE_AGGR_OVRFLW_TO_DBL */
						{
							...
							avg = sum / cnt;
							avg += (avg < 0) ? (-0.5) : (0.5);
							fRow.getColTypes()[colOut] = execplan::erydbSystemCatalog::DOUBLE;
							fRow.setDoubleField(avg, colOut);
						}
						else
							fRow.setIntField((int64_t)avg, colOut);
