...
static void startServer()
{
	...
	do {
		try {
			retry = false;
			inMq = new MessageQueueServer(normServ, "./erydb.xml");
		}
		catch (exception& ex) {
...
{
	MessageQueueServer server("server3", "./erydb.xml");
	ByteStream msg;
...
static void startBrokenServer()
{
	...
	do {
		try {
			retry = false;
			inMq = new MessageQueueServer(brokeServ, "./erydb.xml");
		}
		catch (...) {
			//cout << endl << "MessageQueueServer ctor threw!" << endl;
			::usleep(5000000);
...
static void startWriteServer()
{
	...
	do {
		try {
			retry = false;
			inMq = new MessageQueueServer(writeServ, "./erydb.xml");
		}
		catch (...) {
			//cout << endl << "MessageQueueServer ctor threw!" << endl;
			::usleep(5000000);
...
void mq_1()
{
	...
	Config* cf = Config::makeConfig("./erydb.xml");
...
void mq_2()
{
	...
	MessageQueueClient outMq(brokeServ, "./erydb.xml");
	bs1 = outMq.read(&ts);
...
void mq_3()
{
	...
	//setenv("ERYDB_CONFIG_FILE", "./bogus.xml", 1);
...
void mq_4()
{
	boost::scoped_ptr<MessageQueueClient> outMq(new MessageQueueClient("server4", "./erydb.xml"));
...
void mq_8()
{
	...
	MessageQueueClient outMq(writeServ, "./erydb.xml");
	bs1 = outMq.read();
...
void mq_9()
{
	...
	boost::scoped_ptr<MessageQueueServer> mq1(new MessageQueueServer("server5", "./erydb.xml"));
	...
	boost::scoped_ptr<MessageQueueClient> mq2(new MessageQueueClient("server5", "./erydb.xml"));
	bs.reset();
	bs << "This is a test";
...
{
	InetStreamSocket* iss;
	boost::scoped_ptr<MessageQueueServer> mq1(new MessageQueueServer("server6", "./erydb.xml"));
	// Set a bogus fd
	int fd;
	fd = open("/dev/null", O_RDONLY);
	close(fd);
...
void mq_16()
{
	...
	string sTmp;
	MessageQueueClient client("server3", "./erydb.xml");
	ByteStream bs, bs2;
	char buf[1000];
...
void mq_18()
{
	...
	MessageQueueClient client1("server3", "./erydb.xml");
	MessageQueueClient client2("server3", "./erydb.xml");
