	...
	bSignedUnsignedJoin(false), uniqueLimit(100), finished(false)
{
	...
	for (uint32_t i = keyLength = 0; i < smallKeyColumns.size(); i++) {
		if (smallRG.getColTypes()[smallKeyColumns[i]] == erydbSystemCatalog::CHAR ||
          smallRG.getColTypes()[smallKeyColumns[i]] == erydbSystemCatalog::VARCHAR)
	...
	uint32_t keylen, FixedAllocator *fa)
{
	...
	for (i = 0; i < keyCols.size(); i++) {
		...
		if (type == erydbSystemCatalog::VARCHAR || type == erydbSystemCatalog::CHAR) {
