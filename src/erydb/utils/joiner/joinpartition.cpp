...
//	bucketCount = jp.bucketCount;
	...
	if (filenamePrefix.empty())
		filenamePrefix = "/tmp/erydb";

	filenamePrefix += "erydb-join-data-";
...
int64_t JoinPartition::convertToSplitMode()
{
	...
	for (i = 0; i < (int) bucketCount; i++)
		if (rowDist[i] == rowCount)
			throw ERYDBExcept("All rows hashed to the same bucket", ERR_DBJ_DATA_DISTRIBUTION);
...
void JoinPartition::readByteStream(int which, ByteStream *bs)
{
	...
	if (!fs) {
		...
		throw ERYDBExcept(os.str().c_str(), ERR_DBJ_FILE_IO_ERROR);
	}
