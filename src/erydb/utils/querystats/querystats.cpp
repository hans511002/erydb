...
const string SCHEMA = "erydb_querystats";

struct ERYDB_Drizzle
{
	ERYDB_Drizzle(): drzp(NULL), drzcp(NULL), drzrp(NULL) {}
	~ERYDB_Drizzle() 
	{
...
{
	ResourceManager rm;
	// check if query stats is enabled in erydb.xml
	...
	if (drzret != 0)
		handleMySqlError("fatal error connecting to EryDB in querystats lib", drzret);
	...
	if (drzret != 0)
		handleMySqlError("fatal error reading results from EryDB in querystats lib", drzret);
}
...
void QueryStats::handleMySqlError(const char* errStr, unsigned int errCode)
{
	...
	Message::Args args;
	args.add(oss.str());
	throw ERYDBExcept(ERR_CROSS_ENGINE_CONNECT, args);
}
