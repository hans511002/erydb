...
const int64_t erydb_precision[19] = {
0,
9,
99,
999,
...
// uncomment code below to support negative scale
	...
	if ( (ct.colDataType == erydbSystemCatalog::DECIMAL) ||
	     ...
	     (ct.scale > 0))
	{
		int64_t rangeUp = erydb_precision[ct.precision];
...
boost::any
	DataConvert::convertColumnData(const erydbSystemCatalog::ColType& colType,
	const std::string& dataOrig, bool& pushWarning, bool nulFlag, bool noRoundup, bool isUpdate )
{
	boost::any value;
	std::string data( dataOrig );
	pushWarning = false;
	erydbSystemCatalog::ColDataType type = colType.colDataType;
	//if ( !data.empty() )
	if (!nulFlag)
	{
		switch(type)
		{
			...
			case erydbSystemCatalog::UFLOAT:
			{
				...
				if ( number_value ( data ) )
				{
					...
					if (floatvalue < 0.0 && type == erydbSystemCatalog::UFLOAT &&
			...
			case erydbSystemCatalog::UDOUBLE:
			{
				...
				if ( number_value ( data ) )
				{
					...
					if (doublevalue < 0.0 && type == erydbSystemCatalog::UDOUBLE &&
	...
	else									//null
	{
		switch (type)
		{
			...
			case erydbSystemCatalog::UDECIMAL:
			{
				if (colType.colWidth == erydbSystemCatalog::ONE_BYTE)
				{
					char tinyintvalue = joblist::TINYINTNULL;
					value = tinyintvalue;
				}
				else if (colType.colWidth ==erydbSystemCatalog::TWO_BYTE)
				{
					short smallintvalue = joblist::SMALLINTNULL;
					value = smallintvalue;
				}
				else if (colType.colWidth ==erydbSystemCatalog::FOUR_BYTE)
				{
					int intvalue = joblist::INTNULL;
					value = intvalue;
				}
				else if (colType.colWidth ==erydbSystemCatalog::EIGHT_BYTE)
				{
...
int32_t DataConvert::convertColumnDate(
	const char* dataOrg,
	erydbDateTimeFormat dateFormat,
	int& status,
...
int64_t DataConvert::convertColumnDatetime(
	const char* dataOrg,
	erydbDateTimeFormat datetimeFormat,
	int& status,
	unsigned int dataOrgLen )
{
	status = 0;
	...
	char fld[10];
	int64_t value = 0;
	if ( datetimeFormat != ERYDBDATETIME_ENUM )
	{
		status = -1;
		return value;
	}
...
#if 0
bool DataConvert::isNullData(ColumnResult* cr, int rownum, erydbSystemCatalog::ColType colType)
{
...
erydbSystemCatalog::ColType DataConvert::convertUnionColType(vector<erydbSystemCatalog::ColType>& types)
{
	erydbassert(types.size());

	erydbSystemCatalog::ColType unionedType = types[0];
	for (uint64_t i = 1; i < types.size(); i++)
	{
		// limited support for VARBINARY, no implicit conversion.
		if (types[i].colDataType == erydbSystemCatalog::VARBINARY ||
			unionedType.colDataType == erydbSystemCatalog::VARBINARY)
		{
		...
		switch (types[i].colDataType)
		{
			...
			case erydbSystemCatalog::UDECIMAL:
			{
				switch (unionedType.colDataType)
				{
					...
					case erydbSystemCatalog::UDECIMAL:
						...
						if (types[i].colDataType == erydbSystemCatalog::DECIMAL || types[i].colDataType == erydbSystemCatalog::UDECIMAL)
						{
							unionedType.colDataType = erydbSystemCatalog::DECIMAL;
						}
						unionedType.scale = (types[i].scale > unionedType.scale) ? types[i].scale : unionedType.scale;
						break;
					case erydbSystemCatalog::DATE:
						unionedType.colDataType = erydbSystemCatalog::CHAR;
			...
			case erydbSystemCatalog::DATETIME:
			{
				switch (unionedType.colDataType)
				{
					...
					case erydbSystemCatalog::UDOUBLE:
						...
						break;
					case erydbSystemCatalog::DATE:
						unionedType.colDataType = erydbSystemCatalog::DATETIME;
						unionedType.colWidth = types[i].colWidth;
						break;
					case erydbSystemCatalog::CHAR:
			...
			case erydbSystemCatalog::UDOUBLE:
			{
				switch (unionedType.colDataType)
				{
					...
					case erydbSystemCatalog::UFLOAT:
					case erydbSystemCatalog::UDOUBLE:
						unionedType.colDataType = erydbSystemCatalog::DOUBLE;
						unionedType.scale = 0;
			...
			case erydbSystemCatalog::VARCHAR:
			{
				...
				unionedType.colDataType = erydbSystemCatalog::VARCHAR;
				break;
			}
