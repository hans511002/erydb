...
PosixFileSystem::PosixFileSystem() :
	ERYDBFileSystem( ERYDBFileSystem::POSIX )
{
}
...
int PosixFileSystem::mkdir(const char *pathname)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop( POSIX, "mkdir", pathname, this, ret);
...
int PosixFileSystem::remove(const char *pathname)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop( POSIX, "remove", pathname, this, ret);
...
int PosixFileSystem::rename(const char *oldpath, const char *newpath)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop2( POSIX, "rename", oldpath, newpath, this, ret);
...
off64_t PosixFileSystem::size(const char* path) const
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop( POSIX, "fs:size", path, this, ret);
...
off64_t PosixFileSystem::compressedSize(const char *path) const
{
    ...
    try
    {
        pFile = ERYDBDataFile::open(ERYDBDataFile::BUFFERED, path, "r", 0);
...
int PosixFileSystem::copyFile(const char* srcPath, const char* destPath) const
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logFSop2( POSIX, "copyFile", srcPath, destPath, this, ret);
