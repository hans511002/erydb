...
#ifndef ERYDBDATAFILE_H_
#define ERYDBDATAFILE_H_
...
/**
 * ERYDBDataFile is a combination abstract base class and factory.  The purpose
 * is to encapsulate different underlying storage types from the EryDB
...
public:
	 ...
	 * string (ala fopen).  Note that in general ERYDBDataFile only recognizes
	 ...
	 * is not complete support but rather only that necessary for Erydb so
	 * this will be reconsidered.
	 */
	static ERYDBDataFile* open(Types type, const char* fname, const char* mode, unsigned opts, unsigned colWidth = 4);
...
protected:
	 ...
	 */
	ERYDBDataFile( const char* fname );
...
inline
ERYDBDataFile::ERYDBDataFile( const char* fname ) :
	m_fname( fname ),
    m_fColWidth( 4 )
{
}
...
inline
ERYDBDataFile::~ERYDBDataFile()
{
}

inline
const std::string& ERYDBDataFile::name() const
{
	return m_fname;
}

}
#endif /* ERYDBDATAFILE_H_ */
