	...
	m_new(false)
{
	...
	string bufname = ERYDBPolicy::hdfsRdwrScratch() + input;
	boost::filesystem::path pathDir(bufname);

	// this will create the directory we want to work in
	if( ERYDBPolicy::mkdir(pathDir.parent_path().string().c_str()) != 0 )
	{
		ostringstream oss;
	...
	m_buffer = new BufferedFile( bufname.c_str(), "w+", ERYDBDataFile::USE_VBUF );
...
HdfsRdwrFileBuffer::HdfsRdwrFileBuffer(HdfsRdwrMemBuffer* pMemBuffer) throw (std::exception) :
	ERYDBDataFile(pMemBuffer->name().c_str()),
	m_buffer(NULL),
	m_dirty(false)
{
	...
	string bufname = ERYDBPolicy::hdfsRdwrScratch() + name();
	boost::filesystem::path pathDir(bufname);
...
int HdfsRdwrFileBuffer::close()
{
	...
	if( ERYDBPolicy::remove(buffile.c_str()) != 0 )
	{
		ostringstream oss;
