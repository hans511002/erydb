...
struct slice *makeSlice(sender_state_t sendst, int sliceNo) {
    ...
    erydbassert(slice->state == slice::SLICE_FREE);
    BZERO(*slice);
...
THREAD_RETURN netSenderMain(void	*args0)
{
    ...
    erydbassert(config->sliceSize <= MAX_SLICE_SIZE);
...
struct slice *initSlice(struct clientState *clst, 
			       ...
			       int sliceNo)
{
    erydbassert(slice->state == slice::SLICE_FREE || slice->state == slice::SLICE_RECEIVING);
...
struct slice *rcvFindSlice(struct clientState *clst, int sliceNo)
{
	...
	erydbassert(slice == NULL || slice->magic == SLICEMAGIC);
	while(slice->sliceNo != sliceNo) {
	    if(slice->state == slice::SLICE_FREE)
		return NULL;
	    erydbassert(slice->magic == SLICEMAGIC);
	    pos--;
	    if(pos < 0)
		pos += NR_SLICES;
			       ...
			       struct slice *slice)
{
    ...
    /* is this slice ready ? */
    erydbassert(clst->net_config->blockSize != 0);
	...
	if(clst->use_fec) {
	    ...
	    erydbassert(slice->state == SLICE_DONE || slice->state == SLICE_FEC);
...
//cerr << "good slice" << endl;
	...
	erydbassert(slice->missing_data_blocks[stripe] >= 0);
	if(slice->missing_data_blocks[stripe] <
	   slice->fec_blocks[stripe]) {
	    ...
	    erydbassert(slice->fec_descs[blockIdx].adr != 0);
