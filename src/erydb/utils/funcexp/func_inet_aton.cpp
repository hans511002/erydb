...
//------------------------------------------------------------------------------
execplan::erydbSystemCatalog::ColType Func_inet_aton::operationType(
	FunctionParm& fp,
...
//------------------------------------------------------------------------------
execplan::ERYDB_Decimal Func_inet_aton::getDecimalVal(rowgroup::Row& row,
	FunctionParm& fp,
	bool& isNull,
	execplan::erydbSystemCatalog::ColType& op_ct)
{
//	std::cout << "In Func_inet_aton::getDecimalVal" << std::endl;
	...
	if (!isNull)
	{
		int64_t iValue = convertAton( sValue, isNull );
		if (!isNull)
			return execplan::ERYDB_Decimal( iValue, 0, 0 );
	}
