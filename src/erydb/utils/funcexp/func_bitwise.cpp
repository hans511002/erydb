		...
		bool& isNull)
{
	switch (parm->data()->resultType().colDataType)
	{
		...
		{
			ERYDB_Decimal d = parm->data()->getDecimalVal(row, isNull);
			if (parm->data()->resultType().colDataType == execplan::erydbSystemCatalog::UDECIMAL &&
				d.value < 0)
			{
				d.value = 0;
			}
...
erydbSystemCatalog::ColType Func_leftshift::operationType( FunctionParm& fp, erydbSystemCatalog::ColType& resultType )
{
	return resultType;
}
...
erydbSystemCatalog::ColType Func_rightshift::operationType( FunctionParm& fp, erydbSystemCatalog::ColType& resultType )
{
	return resultType;
}
...
erydbSystemCatalog::ColType Func_bitor::operationType( FunctionParm& fp, erydbSystemCatalog::ColType& resultType )
{
	return resultType;
}
...
erydbSystemCatalog::ColType Func_bitxor::operationType( FunctionParm& fp, erydbSystemCatalog::ColType& resultType )
{
	return resultType;
}
