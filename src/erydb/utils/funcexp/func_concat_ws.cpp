...
erydbSystemCatalog::ColType Func_concat_ws::operationType(FunctionParm& fp, erydbSystemCatalog::ColType& resultType )
{
								...
								erydbSystemCatalog::ColType&)
{
...
#ifdef STRCOLL_ENH__
	wstring wstr;
	size_t strwclen = utf8::erydb_mbstowcs(0,delim.c_str(),0) + 1;
	wchar_t* wcbuf = (wchar_t*)alloca(strwclen * sizeof(wchar_t));
	strwclen = utf8::erydb_mbstowcs(wcbuf, delim.c_str(), strwclen);
	...
	for ( unsigned int id = 1 ; id < parm.size() ; id++) 
	{
		...
		if (!wstr.empty())
			wstr += wdelim;
		size_t strwclen1 = utf8::erydb_mbstowcs(0, tstr.c_str(), 0) + 1;
		wchar_t* wcbuf1 = (wchar_t*)alloca(strwclen1 * sizeof(wchar_t));
		strwclen1 = utf8::erydb_mbstowcs(wcbuf1, tstr.c_str(), strwclen1);
		wstring str1(wcbuf1, strwclen1);
		wstr += str1; 
	}

	size_t strmblen = utf8::erydb_wcstombs(0, wstr.c_str(), 0) + 1;
	char* outbuf = (char*)alloca(strmblen * sizeof(char));
	strmblen = utf8::erydb_wcstombs(outbuf, wstr.c_str(), strmblen);
	if (strmblen == 0)
		isNull = true;
	else
		isNull = false;
