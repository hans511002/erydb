...
// inet_ntoa() is getStrVal().  I tested in MySQL (outside erydb), and
...
// See ERYDB_add in udfsdk.h for explanation of this function.
//------------------------------------------------------------------------------
execplan::erydbSystemCatalog::ColType Func_inet_ntoa::operationType(
	FunctionParm& fp,
	execplan::erydbSystemCatalog::ColType& resultType)
{
...
//	std::cout << "In Func_inet_ntoa::getStrVal" << std::endl;
	...
	if ((fp[0]->data()->resultType().colDataType ==
		...
		execplan::erydbSystemCatalog::FLOAT)   ||
		(fp[0]->data()->resultType().colDataType ==
		execplan::erydbSystemCatalog::DOUBLE))
	{
...
// N/A so returning null.  See explanation at the top of this source file.
//------------------------------------------------------------------------------
execplan::ERYDB_Decimal Func_inet_ntoa::getDecimalVal(rowgroup::Row& row,
	FunctionParm& fp,
	bool& isNull,
	execplan::erydbSystemCatalog::ColType& op_ct)
{
...
//	ERYDB_Decimal dValue = fp[0]->data()->getDecimalVal(row, isNull);
	execplan::ERYDB_Decimal dValue ( joblist::NULL_INT64, 0, 0 );
	isNull = true;
