...
erydbSystemCatalog::ColType Func_maketime::operationType( FunctionParm& fp, erydbSystemCatalog::ColType& resultType )
{
	return resultType;
}
							...
							erydbSystemCatalog::ColType&)
{
	...
	switch (parm[1]->data()->resultType().colDataType)
	{
		...
		case erydbSystemCatalog::DECIMAL:
		{
			ERYDB_Decimal d = parm[1]->data()->getDecimalVal(row, isNull);
	...
	switch (parm[2]->data()->resultType().colDataType)
	{
		...
		case erydbSystemCatalog::DECIMAL:
		{
			ERYDB_Decimal d = parm[2]->data()->getDecimalVal(row, isNull);
