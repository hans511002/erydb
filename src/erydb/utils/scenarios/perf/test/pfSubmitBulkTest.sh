...
#
   ...
   echo cd /usr/local/erydb/data/bulk/log >> $cpimportScriptName
   ...
   echo "ls -alh /usr/local/erydb/data/bulk/data/import/*.tbl > fileStats.txt" >> $cpimportScriptName
#   echo "wc -l /usr/local/erydb/data/bulk/data/import/*.tbl >> fileStats.txt" >> $cpimportScriptName
#
   echo /usr/local/erydb/bin/colxml $testDB -r 2 -j 9999 >> $cpimportScriptName
   echo sleep 5 >> $cpimportScriptName
   echo sync >> $cpimportScriptName
#
   echo \# /usr/local/erydb/bin/cpimport -j 9999 -i >> $cpimportScriptName
   echo sleep 5 >> $cpimportScriptName
...
#
   ...
   echo "while [ ! -f /mnt/pm1/usr/local/erydb/data/bulk/log/finished.txt ]; do" >> $bulkScriptName
   echo    sleep 5 >> $bulkScriptName
   echo echo waiting...... >> $bulkScriptName
   echo done >> $bulkScriptName
#
   echo cp /mnt/pm1/usr/local/erydb/data/bulk/job/Job_9999.xml . >> $bulkScriptName
   echo cp /mnt/pm1/usr/local/erydb/data/bulk/log/Jobxml_9999.log . >> $bulkScriptName
   echo cp /mnt/pm1/usr/local/erydb/data/bulk/log/Job_9999.log . >> $bulkScriptName
   echo cp /mnt/pm1/usr/local/erydb/data/bulk/log/fileStats.txt . >> $bulkScriptName
#   
