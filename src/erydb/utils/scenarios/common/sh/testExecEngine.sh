...
#
   cp /usr/local/erydb/etc/erydb.xml $1/.
   /usr/local/erydb/bin/eryadmin geterydbsoftwareinfo > $1/geterydbsoftwareinfo.log
   /usr/local/erydb/bin/eryadmin getsystemstatus > $1/getSystemStatus.$2.log
   /usr/local/erydb/bin/eryadmin getsystemnetwork > $1/getSystemNetwork.$2.log
   /usr/local/erydb/bin/eryadmin getprocessstatus > $1/getProcessStatus.$2.log
}
#=========================================================================================
# log environment info
#=========================================================================================
...
#
   ...
   do
      ...
      if [ $fileExt = "sh" ]; then
         ...
         if [ $dbmsType = "M" ]; then
            pathSfn=$1\/$sess
            /usr/local/erydb/mysql/bin/mysql --defaults-file=/usr/local/erydb/mysql/my.cnf -u root $testDB <$sfn 2> $pathSfn\/$sfn.err.log |grep "^erydb" > $pathSfn\/$sfn.log &
         else
            su - oracle -c "sqlplus /nolog @/home/qa/srv/common/script/callogin.sql $testDB $testDB xe srvqaperf2 <$sfn" |grep "^erydb" > $1\/$sess\/$sfn.log &
         fi
      fi
      pids[$sess]=$!
   done
