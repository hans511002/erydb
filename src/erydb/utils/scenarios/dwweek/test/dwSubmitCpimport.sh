...
#
   ...
   echo \# >> $bulkScriptName
#
   echo rm -f /usr/local/erydb/data/bulk/log/Jobxml_9999.log >> $bulkScriptName
   echo rm -f /usr/local/erydb/data/bulk/log/Job_9999.log >> $bulkScriptName
   echo /usr/local/erydb/bin/colxml $testDB -t lineitem -l $sourceFileName -j 9999 >> $bulkScriptName
   echo /usr/local/erydb/bin/cpimport -j 9999 >> $bulkScriptName
   echo cp /usr/local/erydb/data/bulk/job/Job_9999.xml . >> $bulkScriptName
   echo cp /usr/local/erydb/data/bulk/log/Jobxml_9999.log . >> $bulkScriptName
   echo cp /usr/local/erydb/data/bulk/log/Job_9999.log . >> $bulkScriptName
