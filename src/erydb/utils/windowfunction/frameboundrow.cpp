...
int64_t FrameBoundExpressionRow<T>::getBound(int64_t b, int64_t e, int64_t c)
{
	...
	if (fRow.isNullValue(fExprIdx))
		throw ERYDBExcept(ERYDBErrorInfo::instance()->errorMsg(ERR_WF_BOUND_OUT_OF_RANGE, "NULL"),
	...
	if (fOffset < 0)
	{
		throw ERYDBExcept(ERYDBErrorInfo::instance()->errorMsg(ERR_WF_BOUND_OUT_OF_RANGE, fOffset),
				ERR_WF_BOUND_OUT_OF_RANGE);
	}
