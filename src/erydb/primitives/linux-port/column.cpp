...
erydb_regex_t placeholderRegex;
...
bool isLike(const char *val, const erydb_regex_t *regex)
{
	if (!regex)
...
inline bool colStrCompare_(uint64_t val1, uint64_t val2, uint8_t COP, uint8_t rf, const erydb_regex_t* regex)
{
	switch(COP) {
	case COMPARE_NIL:
		return false;
...
#if 0
inline bool colStrCompare_(uint64_t val1, uint64_t val2, uint8_t COP, const erydb_regex_t* regex)
{
	switch(COP) {
	case COMPARE_NIL:
		return false;
...
inline bool colCompare(int64_t val1, int64_t val2, uint8_t COP, uint8_t rf, int type, uint8_t width, const erydb_regex_t& regex, bool isNull=false)
{
// 	cout << "comparing " << hex << val1 << " to " << val2 << endl;
	...
	else if ( !isNull && (type == erydbSystemCatalog::FLOAT || type == erydbSystemCatalog::DOUBLE)) {
		double dVal1, dVal2;
		if (type == erydbSystemCatalog::FLOAT)
		{
			dVal1 = *((float *) &val1);
			dVal2 = *((float *) &val2);
		}
	...
	else if ( (type == erydbSystemCatalog::CHAR || type == erydbSystemCatalog::VARCHAR) && !isNull )
	{
		if (!regex.used && !rf)
...
inline bool colCompareUnsigned(uint64_t val1, uint64_t val2, uint8_t COP, uint8_t rf, int type, uint8_t width, const erydb_regex_t& regex, bool isNull=false)
{
						 ...
						 unsigned outSize, unsigned *written, int* block)
{
	...
	for (argIndex = 0; argIndex < in->NVALS; argIndex++) {
		...
		if (isUnsigned((erydbSystemCatalog::ColDataType)in->DataType))
		{
			switch (in->DataSize)
			{
			case 1:
						   ...
						   boost::shared_ptr<ParsedColumnFilter> parsedColumnFilter)
{
	...
	scoped_array<erydb_regex_t> std_regex;
	erydb_regex_t* regex = NULL;
	uint8_t likeOps = 0;

	// no pre-parsed column filter is set, parse the filter in the message
	if (parsedColumnFilter.get() == NULL)
	{
		std_regex.reset(new erydb_regex_t[in->NOPS]);
		regex = &(std_regex[0]);
		...
		else
		{
			...
			for (argIndex = 0; argIndex < in->NOPS; argIndex++)
			{
				...
				case 4:
#if 0
					if (in->DataType == erydbSystemCatalog::FLOAT)
					{
						double dTmp;
	...
	while (!done)
	{
		...
		if (out->ValidMinMax && !isNull && !isEmpty)
		{
			...
			if ((in->DataType == erydbSystemCatalog::CHAR || in->DataType == erydbSystemCatalog::VARCHAR ) && 1 < W)
			{
			...
			else if (isUnsigned((erydbSystemCatalog::ColDataType)in->DataType))
			{
 ...
 uint32_t BOP)
{
	...
	ret->prestored_regex.reset(new erydb_regex_t[filterCount]);
	...
	*/
	...
	for (argIndex = 0; argIndex < filterCount; argIndex++)
	{
		...
		if (isUnsigned((erydbSystemCatalog::ColDataType)colType))
		{
			switch (colWidth) {
			case 1:
		...
		else
		{
			...
			case 4:
#if 0
				if (colType == erydbSystemCatalog::FLOAT) {
					double dTmp;
