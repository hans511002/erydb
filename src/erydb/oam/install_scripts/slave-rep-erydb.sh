...
# $Id$
#
# generic EryDB Slave Replication script.
#
...
cat >/tmp/erydb_slave-rep.sql <<EOD
stop slave;
EOD

cat /tmp/erydb_slave-rep.sql >>/tmp/slave-rep-status.log
$installdir/mysql/bin/mysql \
	--defaults-file=$installdir/mysql/my.cnf \
	--user=root $pwprompt \
	erydbsys </tmp/erydb_slave-rep.sql >>/tmp/slave-rep-status.log
