...
#
# Validate erydb RPM is installed
#
send_user "\n"
send_user "Check erydb RPM installed                     "
expect -re "# "
send "ssh $USERNAME@$SERVER 'rpm -iq erydb'\n"
expect {
...
expect {
	...
	-re "package erydb"     { send_user "ERROR: erydb RPM not installed on External Module\n" ; exit -1 }
}
#
...
send "scp /tmp/etc/$MODULE/inittab.erydb  $USERNAME@$SERVER:/tmp/.\n"
expect -re "word: "
# send the password
send "$PASSWORD\n"
expect {
...
send "ssh $USERNAME@$SERVER 'cat /tmp/inittab.erydb >> /etc/inittab'\n"
expect -re "word: "
# send the password
send "$PASSWORD\n"
expect {
