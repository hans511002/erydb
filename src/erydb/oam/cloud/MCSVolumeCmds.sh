...
test -f $prefix/erydb/post/functions && . $prefix/erydb/post/functions

AWSCLI="aws ec2 "

$prefix/erydb/bin/MCSgetCredentials.sh >/dev/null 2>&1 

#get Region
Region=`$prefix/erydb/bin/MCSInstanceCmds.sh getRegion`
...
createvolume() {
	# get zone
	zone=`$prefix/erydb/bin/MCSInstanceCmds.sh getZone`
...
detachvolume() {
	...
	if [ $STATUS == "detaching" ]; then
		...
		$prefix/erydb/bin/cplogger -w 100 "detachvolume failed: $STATUS"
		echo "failed"
		exit 1
	fi
	...
	test -f /usr/local/erydb/post/functions && . /usr/local/erydb/post/functions
	$prefix/erydb/bin/cplogger -w 100 "detachvolume failed status: $STATUS"
	echo $STATUS
	exit 1
}
...
attachvolume() {
	...
	if [ $STATUS == "attaching" -o $STATUS == "already-attached" ]; then
		...
		test -f /usr/local/erydb/post/functions && . /usr/local/erydb/post/functions
		$prefix/erydb/bin/cplogger -w 100 "attachvolume failed: $STATUS"
		echo "failed"
		exit 1
	fi
