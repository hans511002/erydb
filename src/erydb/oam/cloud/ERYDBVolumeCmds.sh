...
ec2=`$prefix/erydb/bin/getConfig Installation EC2_HOME`
...
java=`$prefix/erydb/bin/getConfig Installation JAVA_HOME`
path=`$prefix/erydb/bin/getConfig Installation EC2_PATH`
...
# get Keys and region
AmazonAccessKeyFile=`$prefix/erydb/bin/getConfig Installation AmazonAccessKey`
if [ $AmazonAccessKeyFile == "unassigned" ]; then
	echo "FAILED: missing Config Setting AmazonAccessKey : $AmazonAccessKeyfile"
	exit 1
fi

AmazonSecretKeyFile=`$prefix/erydb/bin/getConfig Installation AmazonSecretKey`
...
Region=`$prefix/erydb/bin/getConfig Installation AmazonRegion`
...
createvolume() {
	# get zone
	zone=`$prefix/erydb/bin/ERYDBInstanceCmds.sh getZone`

	if [ $moduleType == "um" ]; then
		# get type
		volumeType=`$prefix/erydb/bin/getConfig Installation UMVolumeType`
		if [ $volumeType == "io1" ]; then
			# get IOPS
			volumeIOPS=`$prefix/erydb/bin/getConfig Installation UMVolumeIOPS`
		fi
	else	# pm
		# get type
		volumeType=`$prefix/erydb/bin/getConfig Installation PMVolumeType`
		if [ $volumeType == "io1" ]; then
			# get IOPS
			volumeIOPS=`$prefix/erydb/bin/getConfig Installation PMVolumeIOPS`
		fi
	fi
