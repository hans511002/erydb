 ...
 * 1) A backup was restored including the erydb database files, the file containing the last txnId, and the 
...
class ReplayTxnLog {
	...
	private:
		...
		execplan::erydbSystemCatalog::SCN fStartVersionID;
		execplan::erydbSystemCatalog::SCN fDDLCount; 
		execplan::erydbSystemCatalog::SCN fCommitCount;
		execplan::erydbSystemCatalog::SCN fRollbackCount;
		...
		struct LogEntry {
			execplan::erydbSystemCatalog::SCN fVersionID;
		...
		bool getNexterydbLogEntry(ifstream& dataFile, LogEntry& logEntry);
