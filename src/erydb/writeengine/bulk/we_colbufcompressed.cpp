        ...
        fFlushedStartHwmChunk(false)
{
    ...
    fCompressor = new compress::ERYDBCompressInterface( fUserPaddingBytes );
}
...
//------------------------------------------------------------------------------
int ColumnBufferCompressed::setDbFile(ERYDBDataFile* f, HWM startHwm, const char* hdrs)
{
    fFile        = f;
    ...
    long long& startFileOffset )
{
    ...
    if (!fToBeCompressedBuffer)
    {
        fToBeCompressedBuffer =
            new unsigned char[ERYDBCompressInterface::UNCOMPRESSED_INBUF_LEN];
    }
    BlockOp::setEmptyBuf( fToBeCompressedBuffer,
                          ERYDBCompressInterface::UNCOMPRESSED_INBUF_LEN,
    ...
    fToBeCompressedCapacity = ERYDBCompressInterface::UNCOMPRESSED_INBUF_LEN;

    // Set file offset past end of last chunk
    startFileOffset = ERYDBCompressInterface::HDR_BUF_LEN*2;
...
int ColumnBufferCompressed::writeToFile(int startOffset, int writeSize)
{
    ...
    if((fToBeCompressedCapacity<ERYDBCompressInterface::UNCOMPRESSED_INBUF_LEN) &&
    ...
    else // Not enough room to add all the data to the to-be-compressed buffer
    {
        ...
        while (writeSizeX > 0)
        {
            erydbassert( (fNumBytes <= fToBeCompressedCapacity) ); // DMC-temp debug
...
{
    const int OUTPUT_BUFFER_SIZE = ERYDBCompressInterface::maxCompressedSize(fToBeCompressedCapacity) +
        fUserPaddingBytes;
...
int ColumnBufferCompressed::saveCompressionHeaders( )
{
    ...
    char hdrBuf[ERYDBCompressInterface::HDR_BUF_LEN*2];
...
int ColumnBufferCompressed::initToBeCompressedBuffer(long long& startFileOffset)
{
    ...
    if (fPreLoadHWMChunk)
    {
        ...
        unsigned int outLen = ERYDBCompressInterface::UNCOMPRESSED_INBUF_LEN;
