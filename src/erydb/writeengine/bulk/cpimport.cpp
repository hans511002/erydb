                 ...
                 std::string& tableOIDStr)
{
    ...
    execplan::erydbSystemCatalog::TableName tbl(
...
int main(int argc, char **argv)
{
    ...
    if (argc > 0)
        pgmName = argv[0];
    logging::ERYDBErrorInfo::instance();
    ...
    {
		string ermsg,warn;
		if (!License::checkLicense(ermsg, warn)){
			logInitiateMsg(ermsg.c_str());
			if (!warn.empty())
				logInitiateMsg(warn.c_str());
			return  (EXIT_FAILURE);
		}
		if (!warn.empty())
			logInitiateMsg(warn.c_str());
    ...
    // Initialize cache used to store configuration parms from erydb.xml
    ...
    {
        startupError( std::string(
            "System is not ready.  Verify that EryDB is up and ready "
