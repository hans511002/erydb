...
#get bulkroot
if [ -n "$ERYDB_CONFIG_FILE" ]; then
  echo "ERYDB_CONFIG_FILE=" $ERYDB_CONFIG_FILE
elif [ -z "$ERYDB_CONFIG_FILE"]; then
  ERYDB_CONFIG_FILE="/usr/local/erydb/etc/erydb.xml"
  echo "ERYDB_CONFIG_FILE=" $ERYDB_CONFIG_FILE
else
  ERYDB_CONFIG_FILE="/usr/local/erydb/etc/erydb.xml"
  echo "ERYDB_CONFIG_FILE=" $ERYDB_CONFIG_FILE
...
awk '/BulkRoot/ { sub(/<BulkRoot>/,"",$0); sub(/<\/BulkRoot>/,"",$0); sub(/" "/,"",$0);print $0 > "tmp.txt"}' $ERYDB_CONFIG_FILE 
