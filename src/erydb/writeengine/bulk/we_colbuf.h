...
class ColumnBuffer {
  ...
  public:
    ...
    /** @brief Set the ERYDBDataFile* destination for the applicable col segment file.
     * 
     * @param The destination ERYDBDataFile stream to which buffer data will be written
     * @param Starting HWM for cFile
     * @param Headers with ptr information (only applies to compressed files)
     */
    virtual int setDbFile(ERYDBDataFile * const cFile, HWM startHwm, const char* hdrs);
  ...
  protected:
    ...
    ERYDBDataFile* fFile;            // The column file output stream
