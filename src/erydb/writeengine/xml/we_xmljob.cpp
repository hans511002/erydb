...
const long long erydb_precision[19] = {
0,
9,
99,
999,
...
void XMLJob::printJobInfo( Log& logger ) const
{
    ...
    for( unsigned int i = 0; i < job.jobTableList.size(); i++ ) {
        ...
        for( unsigned int j = 0; j < jobTable.fFldRefs.size(); j++ ) {
            ...
            if( jobCol.typeName == ColDataTypeStr[erydbSystemCatalog::DECIMAL] ) {
                oss3 << "\t\tColumn Precision  : " << jobCol.precision << endl;
                oss3 << "\t\tColumn Scale      : " << jobCol.scale << endl;
            }
            if( jobCol.typeName == ColDataTypeStr[erydbSystemCatalog::UDECIMAL] ) {
                oss3 << "\t\tColumn Precision  : " << jobCol.precision << endl;
                oss3 << "\t\tColumn Scale      : " << jobCol.scale << endl;
            }
...
void XMLJob::setJobDataColumn( xmlNode* pNode, bool bDefaultCol ) 
{
    ...
    if( ( curColumn.typeName == ColDataTypeStr[erydbSystemCatalog::INT] ||
          curColumn.typeName == ColDataTypeStr[erydbSystemCatalog::BIGINT] ||
          curColumn.typeName == ColDataTypeStr[erydbSystemCatalog::SMALLINT]||
          curColumn.typeName == ColDataTypeStr[erydbSystemCatalog::TINYINT])&&
          curColumn.scale > 0 ) 
        curColumn.typeName = ColDataTypeStr[erydbSystemCatalog::DECIMAL];
    // end of workaround
...
void XMLJob::fillInXMLDataAsLoaded(
    execplan::erydbSystemCatalog::RIDList& colRidList)
{
    boost::shared_ptr<execplan::erydbSystemCatalog> cat =
        execplan::erydbSystemCatalog::makeerydbSystemCatalog(
        BULK_SYSCAT_SESSION_ID);
    cat->identity(execplan::erydbSystemCatalog::EC);
    ...
    execplan::erydbSystemCatalog::TableName table(
        fJob.schema, tblName );
    if (fJob.jobTableList[iTbl].mapOid == 0)
    {
        execplan::erydbSystemCatalog::OID tblOid =
        ...
        iCol<fJob.jobTableList[iTbl].colList.size(); iCol++)
    {
        ...
        if (col.mapOid == 0)
        {
            execplan::erydbSystemCatalog::TableColName column;
            column.schema = fJob.schema;
            column.table  = tblName;
            column.column = col.colName;
            execplan::erydbSystemCatalog::OID colOid =
            ...
            if ((colType.scale > 0) ||
                (colType.colDataType ==
                 execplan::erydbSystemCatalog::DECIMAL) ||
                (colType.colDataType ==
                 execplan::erydbSystemCatalog::UDECIMAL))
            {
            ...
            if( ((col.typeName ==
                 ...
                 (col.scale > 0) )
            {
                col.typeName = ColDataTypeStr[erydbSystemCatalog::DECIMAL];
            }
    ...
    JobColumn& col )
{
    ...
    if (colType.constraintType ==
        execplan::erydbSystemCatalog::NOTNULL_CONSTRAINT)
    {
        col.fNotNull            = true;
        ...
        execplan::erydbSystemCatalog::DEFAULT_CONSTRAINT)
    {
        col.fWithDefault        = true;
    }
        ...
        switch (colType.colDataType)
        {
            ...
            case execplan::erydbSystemCatalog::DATE:
            {
                ...
                int32_t dt =
                    ...
                    col_defaultValue.c_str(),
                    dataconvert::ERYDBDATE_ENUM, convertStatus,
                    col_defaultValue.length() );
                if (convertStatus != 0)
                    bDefaultConvertError = true;
                col.fDefaultInt = dt;
            ...
            case execplan::erydbSystemCatalog::DATETIME:
            {
                ...
                int64_t dt =
                    ...
                    col_defaultValue.c_str(),
                    dataconvert::ERYDBDATETIME_ENUM, convertStatus,
                    col_defaultValue.length() );
                if (convertStatus != 0)
                    bDefaultConvertError = true;
                col.fDefaultInt = dt;
...
void XMLJob::validateAllColumnsHaveTags(
    const execplan::erydbSystemCatalog::RIDList& colRidList) const
{
    ...
    try
    {
        ...
        std::set<execplan::erydbSystemCatalog::OID> colOIDList;
        typedef std::set<execplan::erydbSystemCatalog::OID>::iterator SetIter;
            ...
            iCol<fJob.jobTableList[iTbl].colList.size(); iCol++)
        {
            ...
            if (!retVal.second)
            {
                ...
                execplan::erydbSystemCatalog::TableColName dbColName =
        ...
        execplan::erydbSystemCatalog::RIDList::const_iterator rid_iterator =
            colRidList.begin();
