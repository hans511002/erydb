...
//------------------------------------------------------------------------------
void XMLGenProc::makeTableData(const erydbSystemCatalog::TableName& table)
{
    ...
    if (fSysCatRpt) // Write full schema information for syscat rpt
    {
        try
        {
            boost::shared_ptr<erydbSystemCatalog> cat =
                erydbSystemCatalog::makeerydbSystemCatalog(
                BULK_SYSCAT_SESSION_ID);
            cat->identity(erydbSystemCatalog::EC);
            xmlTextWriterWriteFormatAttribute(fWriter,
...
//------------------------------------------------------------------------------
bool XMLGenProc::makeColumnData(const erydbSystemCatalog::TableName& table)
{
        ...
        col != cend; ++col)
    {
        ...
        if (fSysCatRpt) // Write full schema information for syscat rpt
        {
            ...
            if (col->colType.compressionType !=
                erydbSystemCatalog::NO_COMPRESSION)
                xmlTextWriterWriteFormatAttribute(fWriter,
            ...
            if ( (0 < col->colType.scale ) ||
                (col->colType.colDataType == erydbSystemCatalog::DECIMAL) ||
                (col->colType.colDataType == erydbSystemCatalog::UDECIMAL) )
            {
                xmlTextWriterWriteFormatAttribute(fWriter,
    ...
    SysCatColumnList&  colList)
{
    ...
    try
    {
        ...
        const erydbSystemCatalog::RIDList ridList =
