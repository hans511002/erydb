    ...
    std::string& errMsg) const
{
    int rc = fDbFile.readFile(
        pFile, (unsigned char*)controlHdr, ERYDBCompressInterface::HDR_BUF_LEN);
    if (rc != NO_ERROR)
    {
        WErrorCodes ec;
        std::ostringstream oss;
    ...
    ptrHdrSize       = hdrSize - ERYDBCompressInterface::HDR_BUF_LEN;
    char* pointerHdr = new char[ptrHdrSize];
    ...
    std::string& errMsg)
{
    ...
    if ( !ERYDBPolicy::exists( bulkRollbackSubPath.c_str() ) )
    {
        std::ostringstream oss;
    ...
    ERYDBDataFile* backupFile = ERYDBDataFile::open(
    							ERYDBPolicy::getType( bulkRollbackSubPath.c_str(), ERYDBPolicy::WRITEENG ),
...
// Added this function as part of hdfs port, because ERYDBDataFile::read()
