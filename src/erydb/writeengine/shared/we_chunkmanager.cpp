...
ChunkManager::ChunkManager() : fMaxActiveChunkNum(100), fLenCompressed(0), fIsBulkLoad(false),
                               fDropFdCache(false), fIsInsert(false), fIsHdfs(ERYDBPolicy::useHdfs()),
                               fFileOp(0), fSysLogger(NULL), fTransId(-1),
                               fLocalModuleId(Config::getLocalModuleID()),
                               fFs(fIsHdfs ?
                                   ERYDBFileSystem::getFs(ERYDBDataFile::HDFS) :
                                   ERYDBFileSystem::getFs(ERYDBDataFile::BUFFERED))
{
                           ...
                           string& aDMLLogFileName, int64_t size, int64_t offset) const
{
    ...
    //Open file
    boost::scoped_ptr<ERYDBDataFile> aDMLLogFile;
    try
    {
        aDMLLogFile.reset(ERYDBDataFile::open(
                               ERYDBPolicy::getType(aDMLLogFileName.c_str(), ERYDBPolicy::WRITEENG),
...
int ChunkManager::removeBackups(TxnID txnId)
{
    ...
    if (ERYDBPolicy::exists(aDMLLogFileName.c_str()))
    {
        ...
        if (aDMLLogFile) //need recover
        {
            ...
            aDMLLogFile.reset();  // closes the file in ERYDBDataFile destructor.
...
//------------------------------------------------------------------------------
// Get/Return ERYDBDataFile* for specified OID, root, partition, and segment.
// Function is to be used to open column files.
// If the ERYDBDataFile* is not found, then a segment file will be opened using the
// mode (mode) and I/O buffer size (size) that is given.  Name of the resulting
// file is returned in filename.
//
// For Bulk HDFS usage:
//  If useTmpSuffix flag is set, then ERYDBDataFile will use *.tmp for output.
//------------------------------------------------------------------------------
// @bug 5572 - HDFS usage: add *.tmp file backup flag
ERYDBDataFile* ChunkManager::getFilePtr(const Column& column,
                               uint16_t root,
                               uint32_t partition,
                               uint16_t segment,
                               string& filename,
...
//------------------------------------------------------------------------------
// @bug 5572 - HDFS usage: add *.tmp file backup flag
ERYDBDataFile* ChunkManager::getFilePtr(const FID& fid,
                               uint16_t root,
                               uint32_t partition,
                               uint16_t segment,
                               string& filename,
                               ...
                               bool useTmpSuffix) const
{
    CompFileData* fileData = getFileData(fid, root, partition, segment, filename, mode, size,
        erydbSystemCatalog::VARCHAR, 8, useTmpSuffix, true);  // hard code (varchar, 8) are dummy values for dictionary file
    return (fileData ? fileData->fFilePtr : NULL);
}

//------------------------------------------------------------------------------
// Get/Return CompFileData* for specified column OID, root, partition, and
// segment.  If the ERYDBDataFile* is not found, then a segment file will be opened
...
CompFileData* ChunkManager::getFileData(const FID& fid,
                                        ...
                                        const char* mode,
                                        int size,
                                        const erydbSystemCatalog::ColDataType colDataType,
                                        int colWidth,
                                        bool useTmpSuffix,
                                        bool dctnry) const
{
...
// TODO-There is no current way to make this setvbuf call as ERYDBDataFile only
...
// ERYDBDataFile manage it internally or expose a new setBuffer() option.
...
//------------------------------------------------------------------------------
// Return new ERYDBDataFile* for specified dictionary OID, root, partition, segment, and
...
//------------------------------------------------------------------------------
ERYDBDataFile* ChunkManager::createDctnryFile(const FID& fid,
                                     int64_t width,
                                     uint16_t root,
                                     uint32_t partition,
                                     uint16_t segment,
...
{
    FileID fileID(fid, root, partition, segment);
    CompFileData* fileData = new CompFileData(fileID, fid, erydbSystemCatalog::VARCHAR, width);
...
// into readBuf.
//------------------------------------------------------------------------------
int ChunkManager::readBlock(ERYDBDataFile* pFile, unsigned char* readBuf, uint64_t fbo)
{
    map<ERYDBDataFile*, CompFileData*>::iterator fpIt = fFilePtrMap.find(pFile);
...
// chunk.
//------------------------------------------------------------------------------
int ChunkManager::saveBlock(ERYDBDataFile* pFile, const unsigned char* writeBuf, uint64_t fbo)
{
...
int ChunkManager::flushChunks(int rc, const std::map<FID, FID> & columOids)
{
    ...
    if ((rc == NO_ERROR) && fIsInsert)
    {
        ...
        {
            map<ERYDBDataFile*, CompFileData*>::iterator i = fFilePtrMap.begin();
...
//------------------------------------------------------------------------------
int ChunkManager::fetchChunkFromFile(ERYDBDataFile* pFile, int64_t id, ChunkData*& chunkData)
{
    // return value
    int rc = NO_ERROR;
    ...
    if (!fIsBulkLoad && !fIsHdfs)
    {
        ...
        if (((rc = setFileOffset(fileData->fFilePtr, fileData->fFileName, offset, __LINE__)) ==
            ...
            ((rc = readFile(fileData->fFilePtr, fileData->fFileName, buf, size, __LINE__)) ==
             NO_ERROR))
        {
            ERYDBDataFile*  chkFilePtr = ERYDBDataFile::open(
                                            ERYDBPolicy::getType(chkFileName.c_str(),
                                                ERYDBPolicy::WRITEENG),
                                            chkFileName.c_str(),
                                            "w+b",
                                            0 );
            ...
            if (rc != NO_ERROR)
            {
                ERYDBPolicy::remove(chkFileName.c_str());
                return rc;
            }
...
{
    int rc = NO_ERROR;
    unsigned opts = ERYDBDataFile::USE_VBUF;
    if (fIsHdfs)
    {
        if (useTmpSuffix)
        {
    ...
    fileData->fFilePtr = ERYDBDataFile::open(
                            ERYDBPolicy::getType( fileData->fFileName.c_str(), ERYDBPolicy::WRITEENG ),
...
//------------------------------------------------------------------------------
int ChunkManager::setFileOffset(ERYDBDataFile* pFile, const string& fileName, off64_t offset, int ln) const
{
    int rc = NO_ERROR;
...
// is used for logging any error message.
//------------------------------------------------------------------------------
int ChunkManager::readFile(ERYDBDataFile* pFile, const string& fileName, void* buf, size_t size, int ln) const
{
...
// is used for logging any error message.
//------------------------------------------------------------------------------
int ChunkManager::writeFile(ERYDBDataFile* pFile, const string& fileName, void* buf, size_t size, int ln) const
{
...
int ChunkManager::writeHeader(CompFileData* fileData, int ln)
{
    ...
    if (!fIsHdfs && !fIsBulkLoad)
    {
        ...
        string aDMLLogFileName;
        ERYDBDataFile*  hdrFilePtr = ERYDBDataFile::open(
                                        ERYDBPolicy::getType(hdrFileName.c_str(),
                                                           ERYDBPolicy::WRITEENG),
                                        hdrFileName.c_str(),
                                        "w+b",
                                        0,
        ...
        {
            ERYDBPolicy::remove(hdrFileName.c_str());
        }
    }
    else
    {
...
//------------------------------------------------------------------------------
int ChunkManager::expandAbbrevColumnExtent(ERYDBDataFile* pFile, uint64_t emptyVal, int width)
{
    map<ERYDBDataFile*, CompFileData*>::iterator i = fFilePtrMap.find(pFile);
...
//------------------------------------------------------------------------------
int ChunkManager::updateColumnExtent(ERYDBDataFile* pFile, int addBlockCount)
{
    map<ERYDBDataFile*, CompFileData*>::iterator i = fFilePtrMap.find(pFile);
    if (i == fFilePtrMap.end())
    {
...
// blocks are allocated to the corresponding segment file.
//------------------------------------------------------------------------------
int ChunkManager::updateDctnryExtent(ERYDBDataFile* pFile, int addBlockCount)
{
    map<ERYDBDataFile*, CompFileData*>::iterator i = fFilePtrMap.find(pFile);
    if (i == fFilePtrMap.end())
    {
...
//------------------------------------------------------------------------------
int ChunkManager::readBlocks(ERYDBDataFile* pFile, unsigned char* readBuf, uint64_t fbo, size_t n)
{
...
// CompFileData object.
//------------------------------------------------------------------------------
int ChunkManager::restoreBlock(ERYDBDataFile* pFile, const unsigned char* writeBuf, uint64_t fbo)
{
...
//------------------------------------------------------------------------------
int ChunkManager::getBlockCount(ERYDBDataFile* pFile)
{
    map<ERYDBDataFile*, CompFileData*>::iterator fpIt = fFilePtrMap.find(pFile);
    erydbassert(fpIt != fFilePtrMap.end());
...
int ChunkManager::reallocateChunks(CompFileData* fileData)
{
    ...
    ERYDBDataFile*  origFilePtr = fileData->fFilePtr;
    origFilePtr->flush();
    ...
    ERYDBDataFile*  rlcFilePtr = ERYDBDataFile::open(
                                    ERYDBPolicy::getType( rlcFileName.c_str(), ERYDBPolicy::WRITEENG ),
                                    rlcFileName.c_str(),
                                    "w+b",
                                    0,
                                    fileData->fColWidth );
    ...
    if (rc != NO_ERROR)
    {
        ...
        delete rlcFilePtr;
        rlcFilePtr = NULL;
        ERYDBPolicy::remove(rlcFileName.c_str());
        return rc;
    }
    ...
    if (rc != NO_ERROR)
    {
        ...
        if (ERYDBPolicy::rename(rlcFileName.c_str(), dbgFileName.c_str()) == 0)
        ...
        ERYDBDataFile*  rlcPtrFilePtr = ERYDBDataFile::open(
                                        ERYDBPolicy::getType(rlcPtrFileName.c_str(),
                                            ERYDBPolicy::WRITEENG),
    ...
    if (rc == NO_ERROR)
    {
        ...
        if ( ERYDBPolicy::size( rlcFileName.c_str() ) != (int64_t) ptrs[k] )
        {
            ostringstream oss;
...
int ChunkManager::swapTmpFile(const string& src, const string& dest)
{
    ...
    ssize_t srcFileSize = ERYDBPolicy::size(src.c_str());
    if (srcFileSize <= 0)
    {
        ostringstream oss;
...
int ChunkManager::getDMLLogFileName(string& aDMLLogFileName, const TxnID& txnId) const
{
    ...
    {
        ostringstream oss;
        oss << "trans " << txnId << ":Need a valid DBRMRoot entry in erydb configuation file";
        logMessage(oss.str(), logging::LOG_TYPE_ERROR);
        return ERR_DML_LOG_NAME;
    }
