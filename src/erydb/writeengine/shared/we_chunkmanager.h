...
#else
//#define ERYDB_COMP_DEBUG
#ifdef ERYDB_COMP_DEBUG
...
const int UNCOMPRESSED_CHUNK_SIZE = compress::ERYDBCompressInterface::UNCOMPRESSED_INBUF_LEN;
const int COMPRESSED_FILE_HEADER_UNIT = compress::ERYDBCompressInterface::HDR_BUF_LEN;

// assume UNCOMPRESSED_CHUNK_SIZE > 0xBFFF (49151), 8 * 1024 bytes padding
const int COMPRESSED_CHUNK_SIZE = compress::ERYDBCompressInterface::maxCompressedSize(UNCOMPRESSED_CHUNK_SIZE) + 64+3 + 8*1024;
...
{
public:
    CompFileData(const FileID& id, const FID& fid, const execplan::erydbSystemCatalog::ColDataType colDataType, int colWidth) :
...
protected:
    FileID          fFileID;
    FID             fFid;
    execplan::erydbSystemCatalog::ColDataType fColDataType;
    int             fColWidth;
    bool            fDctnryCol;
    ERYDBDataFile*    fFilePtr;
...
public:
    ...
    //        for column file
    ERYDBDataFile* getFilePtr(const Column& column,
                    uint16_t root,
                    uint32_t partition,
                    uint16_t segment,
                    std::string& filename,
    ...
    // @brief Retrieve a file pointer in the chunk manager.
    //        for dictionary file
    ERYDBDataFile* getFilePtr(const FID& fid,
                    uint16_t root,
                    uint32_t partition,
                    uint16_t segment,
                    std::string& filename,
    ...
    ERYDBDataFile* createDctnryFile(const FID& fid,
                    int64_t  width,
                    uint16_t root,
                    uint32_t partition,
                    uint16_t segment,
    ...
    int  readBlock(ERYDBDataFile* pFile, unsigned char* readBuf, uint64_t fbo);

    // @brief Save a block to a chunk in pFile.
    //        The block is not written to disk immediately, will be delayed until flush.
    int  saveBlock(ERYDBDataFile* pFile, const unsigned char* writeBuf, uint64_t fbo);
    ...
    int expandAbbrevColumnExtent(ERYDBDataFile* pFile, uint64_t emptyVal, int width);

    // @brief Update column extent
    int updateColumnExtent(ERYDBDataFile* pFile, int addBlockCount);

    // @brief Update dictionary extent
    int updateDctnryExtent(ERYDBDataFile* pFile, int addBlockCount);

    // @brief Read in n continuous blocks to read buffer.
    //        for backing up blocks to version buffer
    int readBlocks(ERYDBDataFile* pFile, unsigned char* readBuf, uint64_t fbo, size_t n);

    // @brief Restore the data block at offset fbo from version buffer
    //        for rollback
    int restoreBlock(ERYDBDataFile* pFile, const unsigned char* writeBuf, uint64_t fbo);

    // @brief Retrieve the total block count of a DB file.
    int getBlockCount(ERYDBDataFile* pFile);
    ...
    // @brief bug5504, Use non transactional DML for EryDB with HDFS
...
protected:
    ...
    int fetchChunkFromFile(ERYDBDataFile* pFile, int64_t id, ChunkData*& chunkData);
    ...
    int setFileOffset(ERYDBDataFile* pFile, const std::string& fileName, off64_t offset, int ln) const;

    // @brief read from a compressed DB file.
    int readFile(ERYDBDataFile* pFile, const std::string& fileName, void* buf, size_t size, int ln) const;

    // @brief write to a compressed DB file.
    int writeFile(ERYDBDataFile* pFile, const std::string& fileName, void* buf, size_t size, int ln) const;
    ...
    mutable std::map<ERYDBDataFile*, CompFileData*> fFilePtrMap;
    ...
    bool                                        fIsHdfs;
    FileOp*                                     fFileOp;
    compress::ERYDBCompressInterface              fCompressor;
