...
off64_t getCompressedDataSize(string& fileName)
{
		...
		size_t nBytes;
		// Some ERYDBPolicy functions can throw exceptions, caller will catch it
		ERYDBPolicy::configERYDBPolicy();
		bool bHdfsFile = ERYDBPolicy::useHdfs();
		if (bHdfsFile)
			pFile = ERYDBDataFile::open(ERYDBDataFile::HDFS,
				fileName.c_str(), "r", 0);
		else
			pFile = ERYDBDataFile::open(ERYDBDataFile::BUFFERED,
				fileName.c_str(), "r", 0);
		if (!pFile)
		{
		...
		//--------------------------------------------------------------------------
		char hdr1[ERYDBCompressInterface::HDR_BUF_LEN];
		nBytes = readFillBuffer( pFile,hdr1,ERYDBCompressInterface::HDR_BUF_LEN);
		if ( nBytes != ERYDBCompressInterface::HDR_BUF_LEN )
		{
			std::ostringstream oss;
		...
		int64_t ptrSecSize = decompressor.getHdrSize(hdr1) - ERYDBCompressInterface::HDR_BUF_LEN;
...
struct ColumnThread
{
    ...
    void operator()()
    {
		...
		// This function relies on ERYDBPolicy being initialized by
		// ERYDBPolicy::init().  This is done when WriteEngineServer main() calls
		// ERYDBPolicy::configERYDBPolicy();
		ERYDBDataFile::Types fileType;
		bool bUsingHdfs = ERYDBPolicy::useHdfs();
		if (bUsingHdfs)
			fileType = ERYDBDataFile::HDFS;
		else
    ...
    std::string& errMsg, int key)
{
    ...
    try
    {
        ...
        bs >> fileName;
        fileSize = ERYDBPolicy::size(fileName.c_str());
        compressedFileSize = ERYDBPolicy::compressedSize(fileName.c_str());
    }
	catch(std::exception& ex)
	{
		errMsg = ex.what();
	...
	std::string& errMsg, int key)
{
	...
	try {
		...
		erydbSystemCatalog::RIDList columnList = systemCatalogPtr->columnRIDs(tableName);
		erydbSystemCatalog::ColType colType;
		erydbSystemCatalog::DictOIDList dictOidList = systemCatalogPtr->dictOIDs(tableName);
