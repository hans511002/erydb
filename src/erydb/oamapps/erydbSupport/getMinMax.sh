...
count=`$MYSQLCMD erydbsys --skip-column-names -e "$sql;"`
if [ $count -le 0 ]; then
	echo ""
	echo "$db.$table.$column does not exist in Erydb."
	echo ""
	exit 1
fi
...
dataType=`$MYSQLCMD erydbsys --skip-column-names -e "$sql"`
...
objectid=`$MYSQLCMD erydbsys --skip-column-names -e "$sql"`
...
#
/usr/local/erydb/bin/editem -o $objectid $parm | grep max | awk -v dataType=$dataType '
	BEGIN {
		allValid=1;
		foundValidExtent=0;
	}
