...
#       Mar 11 14:16:36 srvqaperf8 oamcpp[16365]: 36.263270 |0|0|0| W 08 CAL0000: min-max-monitor: log files are /tmp/erydb_mm_mon.sdiff.*.15190
...
#    Example:
#       **** Extent map min/max changed on the scan.  See results in /tmp/erydb_mm_mon.sdiff.3039.15190


# Define the cols array.  Here's a sql statement that will list the date and datetime cols in the expected format.
#   erydbmysql erydbsys -e "select concat(objectid, ':', \`schema\`, '.', tablename, '.', columnname) from syscolumn where datatype in (8, 11) and tablename not like 'temp%';" > www.txt
#
# NOTE:  The objectid will be looked up again when it's going through the columns in case the one in the array becomes stale.
#

if [ -z "$MYSQLCMD" ]; then
        MYSQLCMD="/usr/local/erydb/mysql/bin/mysql --defaults-file=/usr/local/erydb/mysql/my.cnf -u root"
fi
...
#
if [ $# -eq 1 ] && [ "$1" == "all" ]; then
	$MYSQLCMD --execute="select concat(objectid, ':', \`schema\`, '.', tablename, '.', columnname) from syscolumn where datatype not in (4, 10, 13) and not (datatype = 2 and columnlength > 8) and not (datatype = 12 and columnlength > 7);" erydbsys --skip-column-names > /tmp/erydb_mm_mon.cols
	cols=( $( cat /tmp/erydb_mm_mon.cols ) )
	rm -f /tmp/erydb_mm_mon.cols
...
#
	...
	db=$1
        $MYSQLCMD --execute="select concat(objectid, ':', \`schema\`, '.', tablename, '.', columnname) from syscolumn where datatype not in (4, 10, 13) and not (datatype = 2 and columnlength > 8) and not (datatype = 12 and columnlength > 7) and \`schema\` = '$db';" erydbsys --skip-column-names > /tmp/erydb_mm_mon.cols
	cols=( $( cat /tmp/erydb_mm_mon.cols ) )
	rm -f /tmp/erydb_mm_mon.cols
...
#
	...
	db=$1
	tbl=$2
        $MYSQLCMD --execute="select concat(objectid, ':', \`schema\`, '.', tablename, '.', columnname) from syscolumn where datatype not in (4, 10, 13) and not (datatype = 2 and columnlength > 8) and not (datatype = 12 and columnlength > 7) and \`schema\` = '$db' and tablename = '$tbl';" erydbsys --skip-column-names > /tmp/erydb_mm_mon.cols
        cols=( $( cat /tmp/erydb_mm_mon.cols ) )
        rm -f /tmp/erydb_mm_mon.cols
...
#
	...
	tbl=$2
	col=$3
        $MYSQLCMD --execute="select concat(objectid, ':', \`schema\`, '.', tablename, '.', columnname) from syscolumn where \`schema\` = '$db' and tablename = '$tbl' and columnname='$col';" erydbsys --skip-column-names > /tmp/erydb_mm_mon.cols
        cols=( $( cat /tmp/erydb_mm_mon.cols ) )
        rm -f /tmp/erydb_mm_mon.cols
fi
...
while [ $i -lt ${#cols[@]} ]; do
	...
	#
	if [ $# -eq 0 ]; then
		$MYSQLCMD --execute="select concat(objectid, ':', \`schema\`, '.', tablename, '.', columnname) from syscolumn where \`schema\` = '$schema' and tablename='$table' and columnname='$column';" erydbsys --skip-column-names > /tmp/erydb_mm_mon.cols
		results=`wc -l /tmp/erydb_mm_mon.cols | awk '{print $1}'`
		if [ $results -eq 0 ]; then
			oid=0
		else
			oid=`cat /tmp/erydb_mm_mon.cols`
		fi
	fi	

        $PGMPATH/editem -o$oid | awk '{print $1, $6, $8, $12}' >/tmp/erydb_mm_mon.$oid.1.$$
        $PGMPATH/editem -c$oid
        $MYSQLCMD --execute="select count($column) from $table" $schema -vvv
        $PGMPATH/editem -o$oid | awk '{print $1, $6, $8, $12}' >/tmp/erydb_mm_mon.$oid.2.$$
        sdiff /tmp/erydb_mm_mon.$oid.1.$$ /tmp/erydb_mm_mon.$oid.2.$$ --suppress-common-lines | grep -n -v invalid > /tmp/erydb_mm_mon.sdiff.$oid.$$
        count=`wc -l /tmp/erydb_mm_mon.sdiff.$oid.$$ | awk '{print $1}'`
        if [ $count -ne 0 ]; then
                badoidlist="$badoidlist $oid"
                ((j++))
                echo "**** Extent map min/max changed on the scan.  See results in /tmp/erydb_mm_mon.sdiff.$oid.$$"
	else
		rm -f /tmp/erydb_mm_mon.sdiff.$oid.$$ 
        fi
        rm -f /tmp/erydb_mm_mon.$oid.*.$$
        ((i++))
done
echo ""
if [ $j -eq 0 ]; then
	...
	exit 0
        ...
        $PGMPATH/cplogger -w 0 "min-max-monitor: log files are /tmp/erydb_mm_mon.sdiff.*.$$"
        echo "min-max-monitor: some values were reset for oids" "$badoidlist"
        echo "min-max-monitor: log files are /tmp/erydb_mm_mon.sdiff.*.$$"
        echo ""
	exit 1
fi
