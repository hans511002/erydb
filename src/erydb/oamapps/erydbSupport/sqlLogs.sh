...
#
# Script that does analysis on SQL statements from an Erydb debug log.
#
DB=erydb_erydb_sqllogs
TABLE=statements
...
if [ -z "$MYSQLCMD" ]; then
        MYSQLCMD="$ERYDB_INSTALL_DIR/mysql/bin/mysql --defaults-file=$ERYDB_INSTALL_DIR/mysql/my.cnf -u root"
        export MYSQLCMD
fi
...
{
        echo "
        This script can be used to analyze select statements from an Erydb debug log.
...
create ()
{
	...
	load data infile '/tmp/erydbtmp.tbl' into table start fields terminated by '|';
	"
	$MYSQLCMD -e "$sql"
	...
	load data infile '/tmp/erydbtmp.tbl' into table stop fields terminated by '|';
	"
	$MYSQLCMD $DB -e "$sql;"
