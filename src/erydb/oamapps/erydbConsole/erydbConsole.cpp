...
/******************************************************************************************
 * $Id: erydbConsole.cpp 3110 2013-06-20 18:09:12Z dhill $
 *
 ******************************************************************************************/
...
//	setupSignalHandlers();
    ...
    {
        cout << endl << "erydb EryDB Command Console" << endl;
        cout << "   enter 'help' for list of commands" << endl;
        cout << "   enter 'exit' to exit the erydb EryDB Command Console" << endl;
...
int processCommand(string* arguments)
{
    ...
    switch( CmdID )
    {
        ...
        case 3: // quit
        {
            ...
            logFile.close();
            cout << "Exiting the erydb Command Console" << endl;
...
//                    cout << "ProcessHeartbeatPeriod = " << systemconfig.ProcessHeartbeatPeriod << endl;
                    ...
                    cout << "OERYDBitmapFile = " << systemconfig.OERYDBitmapFile << endl;
    ...
    //					}
		...
		try
		{
			...
			{
				string cmd = "pdsh -a '/" + startup::StartUp::installDir() + "/bin/erydb stop' > /tmp/cc-stop.pdsh 2>&1";
				system(cmd.c_str());
			...
			{
				string cmd = startup::StartUp::installDir() + "/bin/erydb stop > /tmp/status.log";
				system(cmd.c_str());
			}
		}
            	catch (exception& e)
        ...
        case 18: // startSystem - parameters: Ack flag
        {
			...
			if (!oam.checkLogStatus("/tmp/status.log", "EryDB is running") ) 
			{
				...
				if ( systemModuleNumber > 1 ) 
				{
					...
					if ( DBRootStorageType == "hdfs")
					{
						string cmd = "pdsh -a '" + startup::StartUp::installDir() + "/bin/erydb restart' > /tmp/cc-restart.pdsh 2>&1";
						system(cmd.c_str());
        ...
        case 19: // restartSystem - parameters: graceful flag, Ack flag
        {
			...
			if (!oam.checkLogStatus("/tmp/status.log", "EryDB is running") ) 
			{
				...
				else
				{
					...
					cout.flush();
					string cmd = startup::StartUp::installDir() + "/bin/erydb restart > /tmp/start.log 2>&1";
					int rtnCode = system(cmd.c_str());
					if (WEXITSTATUS(rtnCode) != 0) {
...
//				break;
    ...
    //					}
       ...
       case 42: 
        ...
        case 47: // geterydbSoftwareInfo
        {
			cout << endl;
			if ( rootUser)
			{
				system("rpm -qi erydb-platform > /tmp/erydb.txt 2>&1");
				if (oam.checkLogStatus("/tmp/erydb.txt", "Name"))
					system("cat /tmp/erydb.txt");
				else {
					system("dpkg -s erydb > /tmp/erydb.txt 2>&1");
					if (oam.checkLogStatus("/tmp/erydb.txt", "Status: install"))
						system("cat /tmp/erydb.txt");
					else {
						SystemSoftware systemsoftware;
						oam.getSystemSoftware(systemsoftware);
