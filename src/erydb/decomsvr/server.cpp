...
// A version of erydbassert_s & log() that doesn't require linking the logging lib.
// Things printed to stderr go to /tmp/decomsvr.err
#ifndef __STRING
#define __STRING(x) #x
#endif
#define erydbassert_s(x, s) do { \
	if (!(x)) { \
...
bool initCtlFifo()
{
	...
	erydbassert_s(ListenSock >= 0, string("socket create error: ") + strerror(errno));
...
again:
	...
	if (rc < 0)
	{
		...
		erydbassert_s(0, string("socket bind error: ") + strerror(errno));
		//throw runtime_error(string("socket bind error: ") + strerror(errno));
	}
	rc = listen(ListenSock, 16);
	erydbassert_s(rc >= 0, string("socket listen error") + strerror(errno));
...
string readString(SockType fd)
{
	...
	erydbassert_s(len <= 64, "while reading a string len (>64)");
	//if (len > 64)
...
void ThreadFunc::operator()()
{
	...
	erydbassert_s(h != INVALID_HANDLE_VALUE, "while opening cpipe");
	...
	erydbassert_s(drrc != 0 && nread == 8 && ccount < 8 * 1024 * 1024, "while reading from cpipe");
	...
	erydbassert_s(drrc != 0 && nread == ccount, "while reading from cpipe");
	...
	try
	{
		...
		erydbassert_s(fd >= 0, "when opening data fifo for input");
	...
	erydbassert_s(h != INVALID_HANDLE_VALUE, "while opening upipe");
	//if (!(h != INVALID_HANDLE_VALUE))
	...
	erydbassert_s(dwrc != 0 && nwritten == 8, "while writing to upipe");
	//if (!(dwrc != 0 && nwritten == 8))
	//	throw runtime_error("while writing to upipe");

	dwrc = WriteFile(h, out.get(), (DWORD)outlen, &nwritten, 0);
	erydbassert_s(dwrc != 0 && nwritten == outlen, "while writing to upipe");
	...
	try
	{
		...
		erydbassert_s(fd >= 0, "when opening data fifo for output");
		//if (fd < 0)
		...
		erydbassert_s(rrc == 8, "when writing len to data fifo");

		rrc = writen(fd, out.get(), outlen);
		erydbassert_s(rrc == (ssize_t)outlen, "when writing data to data fifo");
...
#ifdef _MSC_VER
		erydbassert_s(dataSock != INVALID_SOCKET, string("socket accept error: ") + strerror(errno));
		//if (dataSock == INVALID_SOCKET)
		//	throw runtime_error(string("socket accept error: ") + strerror(errno));
#else
		//if (dataSock < 0)
		erydbassert_s(dataSock >= 0, string("socket accept error: ") + strerror(errno));
#endif
