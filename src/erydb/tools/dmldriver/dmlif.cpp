...
int DMLIF::sendOne(const string& stmt)
{
	...
	if (isDML)
	{
		...
		erydbSelectExecutionPlan::ColumnMap cm;
		cm.insert(make_pair(col1, srcp));
		pDMLPackage->get_ExecutionPlan()->columnMap(cm);
		erydbSelectExecutionPlan::ReturnedColumnList rcl;
		rcl.push_back(srcp);
...
int DMLIF::rf2Send()
{
	...
	pDMLPackage = erydbDMLFactory::makeerydbDMLPackage(dmlStmt1);
	if (pDMLPackage == 0)
	{
		cerr << "Failed to parse statement: " << dmlstr << endl;
		return -1;
