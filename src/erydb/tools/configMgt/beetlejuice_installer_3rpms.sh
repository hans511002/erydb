...
send "rm -f $SYSTEMRPM\n"
#expect -re "#"
send "smbclient $SHARED -Werydb -Uoamuser%erydb1 -c 'cd Iterations/$RELEASE/;prompt OFF;mget $SYSTEMRPM'\n"
expect {
...
send "ssh $USERNAME@$SERVER 'rm -f /etc/*.erydb;cp /etc/inittab /etc/inittab.erydb;cp /etc/syslog.conf /etc/syslog.conf.erydb'\n"
expect -re "word: "
# password for ssh
send "$PASSWORD\n"
# check return
...
send_user "\n"
expect -re "#"
send_user "Erase Old $ERYDBRPMNAME Package                     "
send "ssh $USERNAME@$SERVER ' rpm -e --nodeps $ERYDBRPMNAME'\n"
expect -re "word: "
# password for ssh
send "$PASSWORD\n"
# check return
...
expect -re "#"
set timeout 120
send_user "Install New $ERYDBRPMNAME Package                   "
send "ssh $USERNAME@$SERVER ' rpm -ivh /root/$ERYDBRPM'\n"
expect -re "word: "
# password for ssh
send "$PASSWORD\n"
# check return
...
send "ssh $USERNAME@$SERVER 'mv -f /etc/inittab.erydb /etc/inittab;mv -f /etc/syslog.conf.erydb /etc/syslog.conf'\n"
expect -re "word: "
# password for ssh
send "$PASSWORD\n"
# check return
