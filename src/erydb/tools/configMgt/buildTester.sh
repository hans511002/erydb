...
#
# Build, install, and test erydb Builds
#
set EMAIL " "
set USERNAME root
set PASSWORD erydb1
...
while true {
	...
	if { $arg($i) == "-h" } {
		...
		send_user "			release    	  - Required when build-type is entered: erydb release number or 'Latest'\n"
		...
		send_user "			password 	  - system password, default to 'qerydb!'\n"
	...
	if { $arg($i) == "-b" } {
		...
		if { $SYSTEM == "qaftest2" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,dcathey@erydb.com,bpaul@erydb.com
		} elseif { $SYSTEM == "qaftest2" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,dcathey@erydb.com,bpaul@erydb.com
		} elseif { $SYSTEM == "caldev02" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,pleblanc@erydb.com,chao@erydb.com,xlou@erydb.com,dcathey@erydb.com
		} elseif { $SYSTEM == "srvswdev11" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,pleblanc@erydb.com,chao@erydb.com,xlou@erydb.com,dcathey@erydb.com
		} elseif { $SYSTEM == "devint2" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,pleblanc@erydb.com
		} elseif { $SYSTEM == "devint3" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,pleblanc@erydb.com
		} elseif { $SYSTEM == "ss2" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,pleblanc@erydb.com,chao@erydb.com,dcathey@erydb.com
		} elseif { $SYSTEM == "ss2ImportSSB100" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,dcathey@erydb.com,pleblanc@erydb.com
		} elseif { $SYSTEM == "demo01" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,pleblanc@erydb.com,chao@erydb.com
		} elseif { $SYSTEM == "qperfd01" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,bpaul@erydb.com,dcathey@erydb.com,pleblanc@erydb.com
		} elseif { $SYSTEM == "qperfd01" } {
			set EMAIL dhill@erydb.com,wweeks@erydb.com,bpaul@erydb.com,dcathey@erydb.com
		} elseif { $SYSTEM == "qcald02a" } {
				set EMAIL dhill@erydb.com,wweeks@erydb.com,mroberts@erydb.com
		} else {
				set EMAIL dhill@erydb.com,wweeks@erydb.com
		}
	} elseif { $arg($i) == "-d" } {
		set DEBUG " "
	} elseif { $arg($i) == "-sv" } {
...
if { $BUILD != " " } {
	...
	log_user 1
	expect {
		-re "erydb RPM Build successfully completed" { } abort
...
#log_user 1
#send_user "\n\n*******************   erydb RPM Install Information  *******************************\n\n"
#exec ./remote_command_test.sh $SERVER $PASSWORD "rpm -iq erydb | grep Version -A 2" " " "No such file" 60 1
#exec ./remote_command.sh $SERVER $PASSWORD 'rpm -iq erydb' tools not 30 1
#expect {
#		-re " " { abort }
#		-re "FAILED" {
#			puts "\nerydb RPM not installed\n"; 
#			exec echo "Test Failed, erydb RPM not installed" >> $RUN_LOG_FILE
#			puts [exec mail -s "$SYSTEM/$SVN Automated Build Tester Results" $EMAIL < $RUN_LOG_FILE] ; exit -1
#		}
#}
set timeout 30
	...
	while true {
		...
		if { $TESTTYPE == "oracl" } {
			...
			if { $TESTTYPE == "mysql" } {
				...
				} elseif { $TESTCOMMAND == "queryerydbOnly" } {
					set timeout 1200
					log_user 1
					exec ./remote_command_test.sh $SERVER $PASSWORD "./nightly/startQueryTestererydbOnly $SVN" "startQueryTester completed" "No such file" 1200
					expect {
						-re " "  { exec ./remote_scp_get.sh $SERVER $PASSWORD "/root/$SVN/mysql/queries/queryTester_working_tpch1_erydbonly.report"
							exec cat queryTester_working_tpch1_erydbonly.report >> $RUN_LOG_FILE
							exec rm -f queryTester_working_tpch1_erydbonly.report} abort;
						-re "FAILED"  { exec echo "\nFailed to run queryTester\n" >> $RUN_LOG_FILE} abort;
						-re "TIMEOUT"  { exec echo "\nTimeout on run queryTester\n" >> $RUN_LOG_FILE} abort;
					}
