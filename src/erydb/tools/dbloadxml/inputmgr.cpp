...
bool InputMgr::input(int argc, char **argv)
{
    ...
    {
        fTables.push_back(execplan::erydbSystemCatalog::TableName(
...
{
    boost::shared_ptr<execplan::erydbSystemCatalog> systemCatPtr =
        execplan::erydbSystemCatalog::makeerydbSystemCatalog(
        BULK_SYSCAT_SESSION_ID);
    systemCatPtr->identity(execplan::erydbSystemCatalog::EC);
    const vector< pair<erydbSystemCatalog::OID, erydbSystemCatalog::TableName> > tables 
            = systemCatPtr->getTables( fSchema );
    for (vector<pair<erydbSystemCatalog::OID, erydbSystemCatalog::TableName> >::const_iterator it = tables.begin();
         it != tables.end(); ++it)
    {
