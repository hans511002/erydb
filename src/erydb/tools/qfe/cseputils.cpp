...
SimpleFilter* createSimpleFilter
				...
				const erydbSystemCatalog::TableColName& tcn,
				const string& opstr,
				ConstantColumn* cc
				)
{
	...
	op->data(opstr);
	erydbSystemCatalog::ColType ccct;
	ccct = op->resultType();
	...
	erydbSystemCatalog::OID oid = csc->lookupOID(tcn);
	erydbSystemCatalog::ColType ct = csc->colType(oid);
...
void updateParseTree(boost::shared_ptr<execplan::erydbSystemCatalog>& csc,
	execplan::erydbSelectExecutionPlan*& csep,
	...
	const std::string& relop, pair<int, string> cval)
{
	...
	if (sc->schemaName() == "erydb_unknown" && !DefaultSchema.empty())
