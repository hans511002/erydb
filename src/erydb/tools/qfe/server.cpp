...
#include "exceptclasses.h" //brings in erydbassert_s macro
...
{
	QueryMessage m;
	execplan::erydbSelectExecutionPlan* csep=0;
	...
	try {
		...
		if (m.isValid)
		{
			...
			{
				ostringstream oss;
				oss << "select erydbsys.systable.tablename from erydbsys.systable where "
					"erydbsys.systable.schema='" << m.defaultSchema << "';";
...
int main(int argc, char** argv)
{
	...
	for (;;)
	{
		...
		erydbassert_s(querySock != INVALID_SOCKET, string("socket accept error: ") + strerror(errno));
#if 0
		uint32_t sndbufsize;
		int sndbufsizelen=4;
		int rc;
		...
		int querySock = -1;
		querySock = accept(listenSock, 0, 0);
		erydbassert_s(querySock >= 0, string("socket accept error: ") + strerror(errno));
#endif
