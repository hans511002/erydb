...
int main(int argc, char** argv)
{
	...
	if (WSAStartup(minVersion, &wsadata) != 0)
		cerr << "networking startup error: " << ERYDBSysErrorStr(WSAGetLastError()) << endl;
#endif
	fd = ::socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);
	if (fd < 0)
	{
#ifdef _MSC_VER
		cerr << "socket create error: " << ERYDBSysErrorStr(WSAGetLastError()) << endl;
#else
	...
	{
#ifdef _MSC_VER
		cerr << "socket connect error: " << ERYDBSysErrorStr(WSAGetLastError()) << endl;
		WSACleanup();
#else
