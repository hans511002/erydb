...
struct readThr
{
	...
	void operator() ()
	{
...
#endif
		erydbassert(((ptrdiff_t)falignedbuff - (ptrdiff_t)frealbuff) < (ptrdiff_t)fpageSize);
		erydbassert(((ptrdiff_t)falignedbuff % fpageSize) == 0);
		...
		while (i>0) {
			...
			erydbassert(i==0||i==freadSize);
			erydbassert(i%fpageSize==0);
			erydbassert(facc%fpageSize==0);
