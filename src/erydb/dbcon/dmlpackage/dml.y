...
%token CURSOR ERYDB_DECIMAL DECLARE DEFAULT DELETE DESC ERYDB_DOUBLE
%token ESCAPE EXISTS FETCH ERYDB_FLOAT FOR FOREIGN FOUND FROM GOTO
%token GRANT ERYDB_GROUP HAVING IN INSERT INTEGER INTO
...
opt_group_by_clause:
		/* empty */ { $$ = NULL; }
	|	ERYDB_GROUP BY column_ref_commalist
		{
		    $$ = new GroupByClause();
...
data_type:
	...
	|	ERYDB_DECIMAL
	|	ERYDB_DECIMAL '(' INTNUM ')'
	|	ERYDB_DECIMAL '(' INTNUM ',' INTNUM ')'
	|	INTEGER
	|	SMALLINT
	|	ERYDB_FLOAT
	|	ERYDB_FLOAT '(' INTNUM ')'
	|	REAL
	|	ERYDB_DOUBLE PRECISION
	;
