...
void TupleConstantStep::initialize(const JobInfo& jobInfo, const RowGroup* rgIn)
{
	...
	for (uint64_t i = 0, j = 0; i < jobInfo.deliveredCols.size(); i++)
	{
		...
		if (cc != NULL)
		{
			erydbSystemCatalog::ColType ct = cc->resultType();
		...
		else
		{
			...
			{
				throw ERYDBExcept(ERR_NO_FROM);
			}
			erydbassert(j < oidsIn.size());
...
void TupleConstantStep::constructContanstRow(const JobInfo& jobInfo)
{
	...
	const vector<erydbSystemCatalog::ColDataType>& types = fRowGroupOut.getColTypes();
	for (vector<uint64_t>::iterator i = fIndexConst.begin(); i != fIndexConst.end(); i++)
	{
		...
		{
			if (types[*i] == erydbSystemCatalog::CHAR ||
				types[*i] == erydbSystemCatalog::VARCHAR)
			{
...
{
	fRowGroupOut.getRow(0, &fRowOut);
	erydbassert(fRowConst.getColumnCount() == fRowOut.getColumnCount());
