...
SJSTEP& SubQueryTransformer::makeSubQueryStep(execplan::erydbSelectExecutionPlan* csep,
                                              bool subInFromClause)
{
	...
	if (csep->unionVec().size() == 0)
		...
		throw ERYDBExcept(ERR_UNION_IN_SUBQUERY);
	...
	erydbSystemCatalog::OID tblOid = fVtable.tableOid();
	...
	for (uint64_t i = 0; i < outputCols; i++)
	{
		...
		erydbSystemCatalog::ColType ct = fVtable.columnType(i);
		erydbSystemCatalog::ColDataType colDataTypeInRg = row.getColTypes()[i];
			...
			dynamic_cast<WindowFunctionColumn*>(fSubReturnedCols[i].get()) != NULL)
		{
			...
			if (colDataTypeInRg != erydbSystemCatalog::VARCHAR &&
				colDataTypeInRg != erydbSystemCatalog::CHAR &&
				colDataTypeInRg != erydbSystemCatalog::VARBINARY)
			{
		...
		// MySQL date/datetime type is different from ERYDB type
		else if (colDataTypeInRg == erydbSystemCatalog::DATE ||
					colDataTypeInRg == erydbSystemCatalog::DATETIME)
		{
			ct.colWidth = row.getColumnWidth(i);
			ct.colDataType = row.getColTypes()[i];
			ct.scale = row.getScale(i);
...
void SubQueryTransformer::checkCorrelateInfo(TupleHashJoinStep* thjs, const JobInfo& jobInfo)
{
	...
	if (pos == -1 || (size_t) pos >= fVtable.columns().size())
	{
		...
		Message::Args args;
		args.add(name);
		string errMsg(ERYDBErrorInfo::instance()->errorMsg(ERR_CORRELATE_COL_MISSING, args));
		cerr << errMsg << ": " << pos << endl;
		throw ERYDBExcept(errMsg, ERR_CORRELATE_COL_MISSING);
	}
}
...
void SubQueryTransformer::updateCorrelateInfo()
{
	...
	// Insert at [1], not to mess with OUTER join and hint(ERYDB_ORDERED -- bug2317).
		...
		if (thjs)
		{
			if (thjs->getJoinType() & CORRELATED)
			{
				...
				if (thjs->correlatedSide() == 1)
				{
					...
					erydbSystemCatalog::ColType ct2 = fVtable.columnType(pos);
				...
				else
				{
					...
					erydbSystemCatalog::ColType ct1 = fVtable.columnType(pos);
...
*/
		...
		else if (es)
		{
			...
			vector<erydbSystemCatalog::OID>& tableOids = es->tableOids();
			...
			for (uint64_t j = 0; j < scList.size(); j++)
			{
				...
				if (sc != NULL)
				{
					if (subTables.find(tableKeys[j]) != subTables.end())
					{
						...
						if (k == subMap.end())
							throw ERYDBExcept(logging::ERR_NON_SUPPORT_SUB_QUERY_TYPE);
						...
						const erydbSystemCatalog::ColType& ct = fVtable.columnType(k->second);
					...
					{
						const erydbSystemCatalog::ColType&
...
void SimpleScalarTransformer::getScalarResult()
{
	...
	while (more)
	{
		...
		if (fEmptyResultSet && fRowGroup.getRowCount() == 1)
		{
			...
			if (fExistFilter)
			{
				fErrorInfo->errMsg = ERYDBErrorInfo::instance()->errorMsg(ERR_MORE_THAN_1_ROW);
