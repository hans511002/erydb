...
public:
	...
	explicit LBIDList(const execplan::erydbSystemCatalog::OID oid,
					const int debug);

	explicit LBIDList(const int debug);

	void init(const execplan::erydbSystemCatalog::OID oid,
			const int debug);
	...
	void UpdateMinMax(int64_t min, int64_t max, int64_t lbid,
	  execplan::erydbSystemCatalog::ColDataType type, bool validData = true);
	...
	bool CasualPartitionDataType(const execplan::erydbSystemCatalog::ColDataType type, const uint8_t size) const;
