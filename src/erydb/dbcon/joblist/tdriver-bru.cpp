...
private:
	...
	//--------------------------------------------------------------------------
	void initControl(execplan::erydbSystemCatalog::OID& oid);
	...
	struct ThreadArg
	{
		...
		execplan::erydbSystemCatalog::OID oid;  // column OID
...
void BucketReUseDriver::parseConfig()
{
	...
	// before run this test, make sure "tpch.lineitem.l_orderkey" is in the erydb.xml
	BucketReuseManager* control = BucketReuseManager::instance();
	ResourceManager rm;
	BucketReuseManager::instance()->startup(rm);

	// list all the predicates if any listed in the erydb.xml file
	...
	execplan::erydbSystemCatalog::TableColName columnName;
...
void BucketReUseDriver::initControl(execplan::erydbSystemCatalog::OID& oid)
{
	...
	if (found == false)
	{
		cout << "tpch.lineitem.l_orderkey is not in the erydb.xml!)" << endl;
...
void *BucketReUseDriver::scanThread(void* arg)
{
	...
	string dummy;
	bool scan = false;
	boost::shared_ptr<execplan::erydbSystemCatalog> c =
						execplan::erydbSystemCatalog::makeerydbSystemCatalog(0x80000000);
	execplan::erydbSystemCatalog::TableColName tcn = c->colName(a->oid);
