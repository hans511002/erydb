...
inline string colTypeIdString(erydbSystemCatalog::ColDataType type)
{
	switch (type)
	{
		case erydbSystemCatalog::BIT:       return string("BIT");
		case erydbSystemCatalog::TINYINT:   return string("TINYINT");
		case erydbSystemCatalog::CHAR:      return string("CHAR");
		case erydbSystemCatalog::SMALLINT:  return string("SMALLINT");
		case erydbSystemCatalog::DECIMAL:   return string("DECIMAL");
		case erydbSystemCatalog::MEDINT:    return string("MEDINT");
		case erydbSystemCatalog::INT:       return string("INT");
		case erydbSystemCatalog::FLOAT:     return string("FLOAT");
		case erydbSystemCatalog::DATE:      return string("DATE");
		case erydbSystemCatalog::BIGINT:    return string("BIGINT");
		case erydbSystemCatalog::DOUBLE:    return string("DOUBLE");
		case erydbSystemCatalog::DATETIME:  return string("DATETIME");
		case erydbSystemCatalog::VARCHAR:   return string("VARCHAR");
		case erydbSystemCatalog::CLOB:      return string("CLOB");
		case erydbSystemCatalog::BLOB:      return string("BLOB");
		case erydbSystemCatalog::UTINYINT:  return string("UTINYINT");
		case erydbSystemCatalog::USMALLINT: return string("USMALLINT");
		case erydbSystemCatalog::UDECIMAL:  return string("UDECIMAL");
		case erydbSystemCatalog::UMEDINT:   return string("UMEDINT");
		case erydbSystemCatalog::UINT:      return string("UINT");
		case erydbSystemCatalog::UFLOAT:    return string("UFLOAT");
		case erydbSystemCatalog::UBIGINT:   return string("UBIGINT");
		case erydbSystemCatalog::UDOUBLE:   return string("UDOUBLE");
...
void TupleAggregateStep::configDeliveredRowGroup(const JobInfo& jobInfo)
{
	...
	vector<erydbSystemCatalog::ColDataType>::const_iterator types0 =
	...
	fRowGroupDelivered = RowGroup(retColCount,
							...
							vector<erydbSystemCatalog::ColDataType>(types0, types0+retColCount),
	...
	JobInfo& jobInfo, vector<RowGroup>& rowgroups, vector<SP_ROWAGG_t>& aggregators)
{
	...
	const vector<erydbSystemCatalog::ColDataType>& typeProj = projRG.getColTypes();
	...
	vector<erydbSystemCatalog::ColDataType> typeAgg;
	...
	for (uint64_t i = 0; i < returnedColVec.size(); i++)
	{
		...
		if (aggOp == ROWAGG_FUNCT_UNDEFINE)
		{
			...
			else
			{
				...
				string emsg = ERYDBErrorInfo::instance()->errorMsg(ERR_NOT_GROUPBY_EXPRESSION, args);
				cerr << "prep1PhaseAggregate: " << emsg << " oid="
					 ...
					 << (int) aggOp << endl;
				throw ERYDBExcept(emsg, ERR_NOT_GROUPBY_EXPRESSION);
			}
		}
		...
		switch (aggOp)
		{
			...
			case ROWAGG_SUM:
			{
				if (typeProj[colProj] == erydbSystemCatalog::CHAR ||
					typeProj[colProj] == erydbSystemCatalog::VARCHAR ||
					typeProj[colProj] == erydbSystemCatalog::DATE ||
					typeProj[colProj] == erydbSystemCatalog::DATETIME)
				{
					Message::Args args;
					args.add("sum/average");
					args.add(colTypeIdString(typeProj[colProj]));
					string emsg =
						ERYDBErrorInfo::instance()->errorMsg(ERR_AGGREGATE_TYPE_NOT_SUPPORT, args);
					cerr << "prep1PhaseAggregate: " << emsg << endl;
					throw ERYDBExcept(emsg, ERR_AGGREGATE_TYPE_NOT_SUPPORT);
				}

				oidsAgg.push_back(oidsProj[colProj]);
				keysAgg.push_back(key);
				if (typeProj[colProj] == erydbSystemCatalog::DOUBLE ||
					typeProj[colProj] == erydbSystemCatalog::UDOUBLE ||
					typeProj[colProj] == erydbSystemCatalog::FLOAT ||
					typeProj[colProj] == erydbSystemCatalog::UFLOAT)
				{
					typeAgg.push_back(typeProj[colProj]);
					scaleAgg.push_back(scaleProj[colProj]);
					precisionAgg.push_back(precisionProj[colProj]);
				...
				{
					typeAgg.push_back(erydbSystemCatalog::UBIGINT);
				...
				{
					typeAgg.push_back(erydbSystemCatalog::BIGINT);
					uint32_t scale = scaleProj[colProj];
					// for int average, FE expects a decimal
					if (aggOp == ROWAGG_AVG)
						scale = jobInfo.scaleOfAvg[key]; // scale += 4;
			...
			case ROWAGG_STATS:
			{
				...
				typeAgg.push_back(erydbSystemCatalog::DOUBLE);
	...
	for (uint64_t i = 0; i < functionVec.size(); i++)
	{
		...
		scaleAgg.push_back(0);
		precisionAgg.push_back(0);
		typeAgg.push_back(erydbSystemCatalog::LONGDOUBLE);
							...
							vector<SP_ROWAGG_t>& aggregators)
{
	...
	vector<erydbSystemCatalog::ColDataType> typeAgg, typeAggDist;
			...
			switch (aggOp)
			{
				...
				case ROWAGG_AVG:
				{
						...
						typeProj[colProj] == erydbSystemCatalog::DATETIME)
					{
						...
						args.add("sum/average");
						args.add(colTypeIdString(typeProj[colProj]));
						string emsg = ERYDBErrorInfo::instance()->
...
//		}
	...
	//     put count(column name) column to the end, if it is for avg only
	{
		...
		for (uint64_t i = 0; i < returnedColVec.size(); i++)
		{
			...
			switch (aggOp)
			{
				...
				{
					if (typeAgg[colAgg] == erydbSystemCatalog::CHAR ||
						typeAgg[colAgg] == erydbSystemCatalog::VARCHAR ||
						typeAgg[colAgg] == erydbSystemCatalog::DATE ||
						typeAgg[colAgg] == erydbSystemCatalog::DATETIME)
					{
						Message::Args args;
						args.add("sum/average");
					...
					if (typeAgg[colAgg] != erydbSystemCatalog::DOUBLE &&
						typeAgg[colAgg] != erydbSystemCatalog::FLOAT)
					{
						typeAggDist.push_back(erydbSystemCatalog::BIGINT);
		...
		for (uint64_t i = 0; i < functionVec2.size(); i++)
		{
			...
			typeAggDist.push_back(erydbSystemCatalog::LONGDOUBLE);
	...
	if (jobInfo.distinctColVec.size() > 1)
	{
		...
		vector<erydbSystemCatalog::ColDataType> typeAggGb, typeAggSub;
	...
	JobInfo& jobInfo, vector<RowGroup>& rowgroups, vector<SP_ROWAGG_t>& aggregators)
{
	...
	vector<erydbSystemCatalog::ColDataType> typeAggPm, typeAggUm;
		...
		for (uint64_t i = 0; i < aggColVec.size(); i++)
		{
			...
			switch (aggOp)
			{
				...
				case ROWAGG_AVG:
				{
					...
					{
						typeAggPm.push_back(erydbSystemCatalog::UBIGINT);
						uint32_t scale = scaleProj[colProj];
						// for int average, FE expects a decimal
						if (aggOp == ROWAGG_AVG)
							scale = jobInfo.scaleOfAvg[aggKey]; // scale += 4;
					...
					{
						typeAggPm.push_back(erydbSystemCatalog::BIGINT);
						uint32_t scale = scaleProj[colProj];
						// for int average, FE expects a decimal
						if (aggOp == ROWAGG_AVG)
							scale = jobInfo.scaleOfAvg[aggKey]; // scale += 4;
				...
				case ROWAGG_STATS:
				{
					...
					typeAggPm.push_back(erydbSystemCatalog::DOUBLE);
					...
					scaleAggPm.push_back(0);
					precisionAggPm.push_back(0);
					typeAggPm.push_back(erydbSystemCatalog::LONGDOUBLE);
	...
	//     put count(column name) column to the end, if it is for avg only
	{
				...
				if (aggOp == ROWAGG_SUM || aggOp == ROWAGG_COUNT_COL_NAME)
				{
					...
					if (it != aggFuncMap.end())
					{
						...
						else
						{
							...
							typeAggUm.push_back(erydbSystemCatalog::BIGINT);
		...
		for (uint64_t i = 0; i < functionVecUm.size(); i++)
		{
			...
			typeAggUm.push_back(erydbSystemCatalog::LONGDOUBLE);
							...
							vector<SP_ROWAGG_t>& aggregators)
{
	...
	vector<erydbSystemCatalog::ColDataType> typeAggPm, typeAggUm, typeAggDist;
	...
	//     put count(column name) column to the end, if it is for avg only
	{
		...
		for (uint64_t i = 0; i < returnedColVec.size(); i++)
		{
			...
			switch (aggOp)
			{
				...
				case ROWAGG_DISTINCT_SUM:
				{
					if (typeAggUm[colUm] == erydbSystemCatalog::CHAR ||
						typeAggUm[colUm] == erydbSystemCatalog::VARCHAR ||
						typeAggUm[colUm] == erydbSystemCatalog::DATE ||
						typeAggUm[colUm] == erydbSystemCatalog::DATETIME)
					{
						Message::Args args;
						args.add("sum/average");
					...
					keysAggDist.push_back(retKey);
					if (typeAggUm[colUm] != erydbSystemCatalog::DOUBLE &&
						typeAggUm[colUm] != erydbSystemCatalog::FLOAT)
					{
						typeAggDist.push_back(erydbSystemCatalog::BIGINT);
...
void TupleAggregateStep::prepExpressionOnAggregate(SP_ROWAGG_UM_t& aggUM, JobInfo& jobInfo)
{
	...
	{
		erydbSystemCatalog::OID oid = (*i)->oid();
		uint32_t key = getTupleKey(jobInfo, *i);
		erydbSystemCatalog::OID dictOid = joblist::isDictCol((*i)->colType());
		if (dictOid > 0)
		{
			oid = dictOid;
				...
				for (uint32_t c = 0; c < fNumOfRowGroups && !cancelled(); c++)
				{
					...
					if (more)
					{
						...
						if (!fRm.getMemory(fRowGroupIns[threadID].getSizeWithStrings(), fSessionMemLimit))
						{
							...
							if (status() == 0)
							{
								errorMessage(ERYDBErrorInfo::instance()->errorMsg(
