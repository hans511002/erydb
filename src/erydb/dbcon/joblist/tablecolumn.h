...
/** @brief create a JobList object from a erydbExecutionPlan object
 *
...
public:
	...
	*/
	...
	/** @brief constructor
	*/
	EXPORT TableColumn(const execplan::erydbSystemCatalog::OID columnOID, const supportedType columnType);

	EXPORT TableColumn();

	/** @brief getter for the column's OID.
	*/
	inline execplan::erydbSystemCatalog::OID getColumnOID() const { return fColumnOID; }
	...
	*/
	EXPORT void addToSysDataList(execplan::erydbSystemCatalog::NJLSysDataList& sysDataList, const std::vector<uint64_t>& rids);

#if 0
	EXPORT void addToSysDataRids(execplan::erydbSystemCatalog::NJLSysDataList& sysDataList, const std::vector<uint64_t>& rids);
#endif
...
private:
	execplan::erydbSystemCatalog::OID fColumnOID;
