...
void deliveryStep_1()  //number column
{
		...
		cat = erydbSystemCatalog::makeerydbSystemCatalog(1);
...
void deliverTest(int numCols)   
{
		...
		erydbSystemCatalog::TableName tableName = make_table(schema, colstable);
		erydbSystemCatalog::ROPair p = cat->tableRID(tableName);
