...
public:
	...
	void tableOid(const execplan::erydbSystemCatalog::OID& oid) { fTableOid = oid; }
	const execplan::erydbSystemCatalog::OID& tableOid() const   { return fTableOid; }
	...
	const execplan::erydbSystemCatalog::OID& columnOid(uint32_t i) const;

	const std::vector<execplan::erydbSystemCatalog::ColType>& columnTypes() const
		{ return fColumnTypes; }
	void columnType(execplan::erydbSystemCatalog::ColType& type, uint32_t i);
	const execplan::erydbSystemCatalog::ColType& columnType(uint32_t i) const;
...
protected:
	...
	std::vector<execplan::erydbSystemCatalog::ColType> fColumnTypes;
