...
void getColumnValue(ConstantColumn** cc, uint64_t i, const Row& row)
{
	...
	switch (row.getColTypes()[i])
	{
		...
		case erydbSystemCatalog::UDECIMAL:
			...
			oss << (data / ERYDB_pow[row.getScale(i)]);
			if (row.getScale(i) > 0)
			{
				if (data > 0)
					oss << "." << (data % ERYDB_pow[row.getScale(i)]);
			...
			*cc = new ConstantColumn(oss.str(),
									ERYDB_Decimal(data, row.getScale(i), row.getPrecision(i)));
			break;
...
void sfInHaving(ParseTree* pt, void*)
{
	...
	if (sf != NULL)
		throw ERYDBExcept(ERR_NON_SUPPORT_HAVING);
}
...
ParseTree* trim(ParseTree*& pt)
{
	...
	{
		erydbassert(dynamic_cast<LogicOperator*>(pt->data())->data() == "and");
		ParseTree* br = pt;
...
void alterCsepInExistsFilter(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
...
void doCorrelatedExists(const ExistsFilter* ef, JobInfo& jobInfo)
{
	...
	erydbSelectExecutionPlan* csep = ef->sub().get();
	...
	if (!noFrom)
	{
		...
		if (!transformer.correlatedSteps().empty())
		{
			...
			if (tn.empty() || tn.compare(0,5,"$sub_"))
				tn = "sub-query";
			args.add(tn);
			throw ERYDBExcept(ERR_MISS_JOIN_IN_SUB, args);
		}
...
const SRCP doSelectSubquery(erydbExecutionPlan* ep, SRCP& sc, JobInfo& jobInfo)
{
	erydbSelectExecutionPlan* csep = dynamic_cast<erydbSelectExecutionPlan*>(ep);
	SRCP rc;
	SErrorInfo errorInfo(jobInfo.errorInfo);
...
bool simpleScalarFilterToParseTree(SimpleScalarFilter* sf, ParseTree*& pt, JobInfo& jobInfo)
{
	...
	erydbSelectExecutionPlan* csep = sf->sub().get();
	SOP sop = sf->op();
...
void preprocessHavingClause(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
	ParseTree* havings = (csep->having());
	erydbassert(havings != NULL); // check having exists before calling this function.
...
int doFromSubquery(erydbExecutionPlan* ep, const string& alias, const string& view, JobInfo& jobInfo)
{
	erydbSelectExecutionPlan* csep = dynamic_cast<erydbSelectExecutionPlan*>(ep);
	SErrorInfo errorInfo(jobInfo.errorInfo);
...
void addOrderByAndLimit(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
	...
	erydbSelectExecutionPlan::OrderByColumnList& orderByCols = csep->orderByCols();
...
//    type of pseudo column is set by connector
				if (sc->isEryDB() && !(dynamic_cast<PseudoColumn*>(sc)))
...
//X
		...
		else
		{
			...
			erydbSystemCatalog::ColType ct = rc->resultType();
...
void preprocessSelectSubquery(erydbSelectExecutionPlan* csep, JobInfo& jobInfo)
{
	RetColsVector& retCols = (csep->returnedCols());
	erydbSelectExecutionPlan::SelectList::const_iterator sub = csep->selectSubList().begin();
	for (RetColsVector::iterator i = retCols.begin(); i != retCols.end(); i++)
	{
...
SJSTEP doUnionSub(erydbExecutionPlan* ep, JobInfo& jobInfo)
{
	erydbSelectExecutionPlan* csep = dynamic_cast<erydbSelectExecutionPlan*>(ep);
	SErrorInfo errorInfo(jobInfo.errorInfo);
	SubQueryTransformer transformer(&jobInfo, errorInfo);
