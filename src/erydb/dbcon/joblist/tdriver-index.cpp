...
erydbSystemCatalog::TableColName testcol = { "tpch", "orders", "o_orderkey"};
erydbSystemCatalog::TableColName largecol = { "tpch", "lineitem", "l_orderkey"};
...
private:
     	int getIndexOID(const erydbSystemCatalog::TableColName&  col, boost::shared_ptr<erydbSystemCatalog> cat)
	{	
		const erydbSystemCatalog::IndexNameList iNames = cat->colValueSysindexCol(col);
		if (0 == iNames.size())
		{
			cout << "No index for " << col << endl;
			return -1;
		}
		erydbSystemCatalog::IndexOID ixoid = cat->lookupIndexNbr(*iNames.begin());
		return ixoid.objnum;
	}
...
public:
    	...
    	void indexTest_ss1() 
	{
		...
		boost::shared_ptr<erydbSystemCatalog> cat = erydbSystemCatalog::makeerydbSystemCatalog();
