...
TupleUnion::TupleUnion(erydbSystemCatalog::OID tableOID, const JobInfo& jobInfo) :
	JobStep(jobInfo),
	fTableOID(tableOID),
	output(NULL),
	outputIt(-1),
...
erydbSystemCatalog::OID TupleUnion::tableOid() const
{
	return fTableOID;
}
			...
			*/
			...
			if (distinct) {
				...
				if (!rm.getMemory(memDiff, sessionMemLimit)) {
					...
					{
						errorMessage(logging::ERYDBErrorInfo::instance()->errorMsg(
...
dec2:					uint64_t val = in.getIntField(i);
            ...
            case erydbSystemCatalog::DOUBLE:
			case erydbSystemCatalog::UDOUBLE: {
				double val = (in.getColTypes()[i] == erydbSystemCatalog::FLOAT ?
				...
				switch (out->getColTypes()[i]) {
					...
					case erydbSystemCatalog::UDECIMAL: {
						if (out->getScale(i) == scale)
							...
							out->setIntField(ERYDB_pow[out->getScale(i)-scale]*val, i);
					...
					case erydbSystemCatalog::FLOAT:
					case erydbSystemCatalog::UFLOAT: {
						float fval = ((float) val) / ERYDB_pow[scale];
						out->setFloatField(fval, i);
						break;
					}
					case erydbSystemCatalog::DOUBLE: {
						double dval = ((double) val) / ERYDB_pow[scale];
					...
					/*	ostringstream oss;
						if (scale == 0)
							...
							oss << (val / ERYDB_pow[scale]) << "."
								<< setw(scale) << setfill('0') << (val % ERYDB_pow[scale]); */
