...
#define ERYDB_SM_DEBUG 0
#define ERYDB_SM_PROFILE 0
...
const int ERYDB_INTERNAL_ERROR=-1007;
...
struct cpsm_tplsch_t
{
	...
	std::vector<execplan::erydbSystemCatalog::ColType> ctp;
	std::string errMsg;
...
/** @brief erydb connection handle structure */
class cpsm_conhdl_t
{
public:
	cpsm_conhdl_t(time_t v, const uint32_t sid, bool erydb_local_query) :
	value(v), sessionID(sid), queryState (NO_QUERY),
	exeMgr( new execplan::ClientRotator(sid, "ExeMgr", erydb_local_query)),
...
extern status_t sm_init(uint32_t, cpsm_conhdl_t**, uint32_t erydb_local_query = false);
