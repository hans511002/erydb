...
void cleanup(cpsm_conhdl_t* hndl)
{
		...
		erydbSystemCatalog::removeerydbSystemCatalog(hndl->sessionID);
		...
		else
		{
				...
				else
				{
					...
					else // @todo error handling
					{
						...
						ntplsch->errMsg = ERYDBErrorInfo::instance()->errorMsg(ERR_LOST_CONN_EXEMGR);
						return logging::ERR_LOST_CONN_EXEMGR;
					}
				}
...
{
#ifdef _MSC_VER
	const std::string DEFAULT_SAVE_PATH = "C:\\erydb\\tmp";
#else
		...
		cpsm_conhdl_t*	conn_hdl)
{
		...
		try {
				...
				if (ntplh->saveFlag != SAVED)
				{
						...
						erydbSystemCatalog::OID tableOID = static_cast<erydbSystemCatalog::OID>(tableid);
		...
		int* killed )
{
		...
		if (ntplsch->traceFlags & erydbSelectExecutionPlan::TRACE_NO_ROWS2)
		  ...
		  uint32_t erydb_local_query)
{
	...
	/**
	 * when local PM is detected, or erydb_local_query is set:
	...
	if (*conn_hdl)
	{
		...
		if ((*conn_hdl)->exeMgr->localQuery() || ! erydb_local_query)
			return STATUS_OK;
	...
	cpsm_conhdl_t* hndl = new cpsm_conhdl_t(time(0), sid, erydb_local_query);
	*conn_hdl = hndl;
	hndl->sessionID = sid;
