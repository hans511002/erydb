	...
	fData(sql)
{
	...
	if (fType == ConstantColumn::NULLDATA)
	{
        ...
        {
            fResultType.colDataType = erydbSystemCatalog::UBIGINT;
        }
        else
        {
	...
	fData(sql)
{
	...
	fResult.decimalVal.scale = 0;
	fResult.decimalVal.precision = 18;
	fResultType.colDataType = erydbSystemCatalog::DOUBLE;
	fResultType.colWidth = 8;
}
...
ConstantColumn::ConstantColumn(const string& sql, const ERYDB_Decimal& val) :
  ReturnedColumn(),
  fConstval(sql),
  fType(NUM),
	fData(sql)
{
	...
	fResultType.colDataType = erydbSystemCatalog::DECIMAL;
	fResultType.colWidth = 8;
