...
public:
	...
	inline const erydbSystemCatalog::OID& oid() const
	{
		return fOid;
	}

	inline void oid (const erydbSystemCatalog::OID& oid)
	{
		fOid = oid;
	}
	...
	inline const bool isEryDB() const { return fIsEryDB; }
	inline void isEryDB(const bool isEryDB) { fIsEryDB = isEryDB; }
  ...
  /** @brief check if this column is the same as the argument */
	...
	const erydbSystemCatalog::ColType& colType() const { return fResultType; }
...
/**
	 ...
	 */
	virtual bool singleTable(erydbSystemCatalog::TableAliasName& tan);
...
protected:
	...
	erydbSystemCatalog::OID fOid;    /// column TCN number
...
public:
	...
	virtual ERYDB_Decimal getDecimalVal(rowgroup::Row& row, bool& isNull)
	{
		evaluate(row, isNull);
...
ParseTree* replaceRefCol(ParseTree*& n, erydbSelectExecutionPlan::ReturnedColumnList&);
