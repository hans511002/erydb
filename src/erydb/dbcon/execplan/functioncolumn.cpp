...
void FunctionColumn::replaceRealCol(erydbSelectExecutionPlan::ReturnedColumnList& derivedColList)
{
...
bool FunctionColumn::singleTable(erydbSystemCatalog::TableAliasName& tan)
{
	tan.clear();
	setSimpleColumnList();
	for (uint i = 0; i < fSimpleColumnList.size(); i++)
	{
		erydbSystemCatalog::TableAliasName stan(fSimpleColumnList[i]->schemaName(),
