...
%token ACTION ADD ALTER AUTO_INCREMENT BIGINT BIT ERYDB_BLOB CASCADE ERYDB_CHAR CHARACTER CHECK CLOB COLUMN
COLUMNS COMMENT CONSTRAINT CONSTRAINTS CREATE CURRENT_USER DATETIME DEC
DECIMAL DEFAULT DEFERRABLE DEFERRED ERYDB_DELETE DROP ENGINE
FOREIGN FULL IMMEDIATE INDEX INITIALLY ERYDB_INT INTEGER KEY MATCH MAX_ROWS
...
VARYING WITH ZONE DOUBLE ERYDB_FLOAT REAL CHARSET ERYDB_IF EXISTS CHANGE TRUNCATE
...
/* Notice that we allow table_options here (e.g. engine=erydb) but
   we ignore them. */
create_index_statement:
...
delete_rule:
	ON ERYDB_DELETE referential_action {$$ = $3;}
	;
...
table_option:
	...
	AUTO_INCREMENT opt_equal ICONST
    {
 	...
 	|
 	DEFAULT ERYDB_CHAR SET opt_equal IDENT {$$ = new pair<string,string>("default charset", $5);}
	;
...
/*	   	$3->fLength = DDLDatatypeLength[DDL_DECIMAL]; */
	...
	| ERYDB_INT opt_display_width UNSIGNED
	{
		$$ = new ColumnType(DDL_UNSIGNED_INT);
		$$->fLength = DDLDatatypeLength[DDL_INT];
	}
...
approximate_numeric_type:
	...
	| ERYDB_FLOAT opt_display_precision_scale_null
	{
		$$ = new ColumnType(DDL_FLOAT);
		$$->fLength = DDLDatatypeLength[DDL_FLOAT];
	}
	| ERYDB_FLOAT opt_display_precision_scale_null UNSIGNED
	{
