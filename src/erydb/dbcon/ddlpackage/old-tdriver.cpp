...
#include "erydbddlpackage.h"
#include "erydbddlfactory.h"
...
public:
	...
	void test_write_read_create_table_object()
	{
		...
		std::string ddl_statement = "CREATE TABLE erydb.PART(p_partkey int not null, p_a varchar(55), p_b decimal(8,2) unique, p_c int default 1, p_d varchar(25) default 'unknown' check (varchar = 'a' ), foreign key (p_partkey) references Customers(p_partkey)) engine=erydb;";
		...
		erydbDDLPackage* pDDLPackage = erydbDDLFactory::makeerydbDDLPackage(*pDDLStatement);
	...
	void write_create_table_object( ByteStream& bs, erydbDDLPackage* pDDLPackage )
	{
	...
	void test_write_read_create_index_object()
	{
		...
		std::string ddl_statement = "CREATE INDEX erydb.index1 ON erydb.PART(p_partkey,p_a) engine=erydb;";
	...
	void write_create_index_object( ByteStream& bs, erydbDDLPackage* pDDLPackage )
	{
	...
	void test_write_read_alter_table_object()
	{
		...
		std::string ddl_statement = "ALTER TABLE erydb.PART_a ADD id int engine=erydb;";
	...
	void write_alter_table_object( ByteStream& bs, erydbDDLPackage* pDDLPackage )
	{
	...
	void test_write_read_drop_table_object()
	{
		...
		std::string ddl_statement = "DROP TABLE erydb.PART_a CASCADE CONSTRAINTS engine=erydb;";
	...
	void write_drop_table_object( ByteStream& bs, erydbDDLPackage* pDDLPackage )
	{
	...
	void test_write_read_drop_index_object()
	{
		...
		std::string ddl_statement = "DROP INDEX erydb.INDEX_1 on erydb.PART engine=erydb;";
	...
	void write_drop_index_object( ByteStream& bs, erydbDDLPackage* pDDLPackage )
	{
