...
namespace
{
const fs::path defaulterydbConfigFile("erydb.xml");
}
...
Config* Config::makeConfig(const char* cf)
{
	...
	{
#ifdef _MSC_VER
		string cfStr = ERYDBreadRegistry("ConfigFile");
		if (!cfStr.empty())
			cf = cfStr.c_str();
#else
		cf = getenv("ERYDB_CONFIG_FILE");
#endif
		if (cf == 0 || *cf == 0)
		{
			fs::path configFilePath = fs::path(installDir) / fs::path("etc") / defaulterydbConfigFile;
...
void Config::parseDoc(void)
{
	...
	if (xmlStrcmp(cur->name, (const xmlChar *)"erydb"))
	{
		xmlFreeDoc(fDoc);
		fDoc = 0;
		throw runtime_error("Config::parseDoc: error parsing config file " + fConfigFile);
...
void Config::writeConfig(const string& configFile) const
{
	...
	const fs::path defaulterydbConfigFileTemp("erydb.xml.temp");
	const fs::path saveerydbConfigFileTemp("erydb.xml.erydbSave");
	const fs::path tmperydbConfigFileTemp("erydb.xml.temp1");

	fs::path etcdir = fs::path(fInstallDir) / fs::path("etc");

	fs::path dcf = etcdir / fs::path(defaulterydbConfigFile);
	fs::path dcft = etcdir / fs::path(defaulterydbConfigFileTemp);
	fs::path scft = etcdir / fs::path(saveerydbConfigFileTemp);
	fs::path tcft = etcdir / fs::path(tmperydbConfigFileTemp);
	//perform a temp write first if erydb.xml file to prevent possible corruption
	if ( configFile == dcf ) {
		...
		try {
			...
			//good read, save copy, copy temp file tp tmp then to erydb.xml
