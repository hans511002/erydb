 ...
 * erydb_add(2,3). That way, the function does not run in a distributed fashion 
...
{
/**
 * ERYDB_ADD connector stub
 */
#ifdef _MSC_VER
__declspec(dllexport)
#endif
my_bool erydb_add_init(UDF_INIT* initid, UDF_ARGS* args, char* message)
{
	if (args->arg_count != 2)
	{
		strcpy(message,"erydb_add() requires two argument");
		return 1;
	}
...
__declspec(dllexport)
#endif
void erydb_add_deinit(UDF_INIT* initid)
{
}
...
__declspec(dllexport)
#endif
double erydb_add(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error)
{
	double op1, op2;
...
/**
 * ERYDB_ISNULL connector stub
 */
 
#ifdef _MSC_VER
__declspec(dllexport)
#endif
my_bool erydb_isnull_init(UDF_INIT* initid, UDF_ARGS* args, char* message)
{
	if (args->arg_count != 1)
	{
		strcpy(message,"erydb_isnull() requires one argument");
		return 1;
	}
...
__declspec(dllexport)
#endif
void erydb_isnull_deinit(UDF_INIT* initid)
{
}	
...
__declspec(dllexport)
#endif
long long erydb_isnull(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error)
{
	return 0;
}
