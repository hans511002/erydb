	...
	ERYDBDataFile( fname )
{
	...
	// special flags we set by convention due to EryDB usage
	flags |= O_LARGEFILE|O_NOATIME;

	if( opts & ERYDBDataFile::USE_ODIRECT )
	{
		flags |= O_DIRECT;
	}
...
ssize_t UnbufferedFile::pread(void *ptr, off64_t offset, size_t count)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logRW("pread", m_fname, this, offset, count, ret);
...
ssize_t UnbufferedFile::read(void *ptr, size_t count)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logRW("read", m_fname, this, offset, count, ret);
...
ssize_t UnbufferedFile::write(const void *ptr, size_t count)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logRW("write", m_fname, this, offset, count, ret);
...
off64_t UnbufferedFile::size()
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logSize(m_fname, this, ret);
...
int UnbufferedFile::close()
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logNoArg(m_fname, this, "close", ret);
	errno = savedErrno;
	return ret;
}
