...
bool ERYDBPolicy::s_usehdfs = false;
bool ERYDBPolicy::s_bUseRdwrMemBuffer = false;
int64_t ERYDBPolicy::s_hdfsRdwrBufferMaxSize = 0; 
std::string ERYDBPolicy::s_hdfsRdwrScratch;
bool ERYDBPolicy::s_configed = false;
boost::mutex ERYDBPolicy::s_mutex;

void ERYDBPolicy::init( bool bEnableLogging, bool bUseRdwrMemBuffer, const string& hdfsRdwrScratch, int64_t hdfsRdwrBufferMaxSize )
{
	ERYDBFactory::installDefaultPlugins();

	ERYDBLogger::enable( bEnableLogging );
	...
	if( hdfsRdwrScratch.length() > 0 )
	{
		...
		if (boost::filesystem::exists(tmpfilepath))
		{
			if (!boost::filesystem::is_directory(tmpfilepath) && useHdfs())
			{
				...
				ostringstream oss;
				oss << "ERYDBPolicy::init: scratch diretory setting " << hdfsRdwrScratch.c_str()
		...
		else
		{
			if (!boost::filesystem::create_directory(tmpfilepath))
			{
				...
				ostringstream oss;
				oss << "ERYDBPolicy::init: failed to create hdfs scratch directory "
...
bool ERYDBPolicy::installPlugin(const std::string& plugin)
{
	bool ret = ERYDBFactory::installPlugin(plugin);
	...
	try
	{
		...
		ERYDBFactory::name(ERYDBDataFile::HDFS);
		s_usehdfs = true;
	}
	catch (std::exception& )
	{
...
bool ERYDBPolicy::isLocalFile( const std::string& path )
{
    boost::filesystem::path filepath( path );
#ifdef _MSC_VER
    size_t strmblen = funcexp::utf8::erydb_wcstombs(0, filepath.extension().c_str(), 0) + 1;
	char* outbuf = (char*)alloca(strmblen * sizeof(char));
	strmblen = funcexp::utf8::erydb_wcstombs(outbuf, filepath.extension().c_str(), strmblen);
	...
	bool isInDbroot = path.find("/erydb/data") != string::npos;
	bool isScratch = path.find(s_hdfsRdwrScratch) == 0;

	return !isInDbroot || isXml || isVb || isScratch;
}

ERYDBDataFile::Types ERYDBPolicy::getType( const std::string& path, Contexts ctxt )
{
	...
	{
		if( isLocal || !useHdfs() )
			return ERYDBDataFile::UNBUFFERED;
		else
			return ERYDBDataFile::HDFS;
	}
	else
	{
		if( isLocal || !useHdfs() )
			return ERYDBDataFile::BUFFERED;
		else
			return ERYDBDataFile::HDFS;
	}
}

ERYDBFileSystem& ERYDBPolicy::getFs( const std::string& path )
{
	...
	return ERYDBFactory::getFs( getType( path, PRIMPROC) );
}

// ported from we_config.cpp
void ERYDBPolicy::configERYDBPolicy()
{
