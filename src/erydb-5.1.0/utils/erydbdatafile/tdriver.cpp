...
struct foptest_opts
{
	...
	foptest_opts() :
		...
		numthreads(1),
		numblocks(64),
		filetype(ERYDBDataFile::UNBUFFERED),
		largeFile(""),
		pluginFile(""),
		useHdfs(false),
		hdfsMaxMem(0)
...
public:
	...
	bool runTest( ERYDBDataFile::Types filetype, unsigned open_opts );
...
private:
	bool readTest( ERYDBDataFile::Types filetype );
	friend void thread_func2( TestRunner& trun );
	bool randomReadTest();
	bool openByModeStrTest();
	bool writeTest( ERYDBDataFile::Types filetype );
	bool truncateTest( ERYDBDataFile::Types filetype );
	bool renameTest( ERYDBDataFile::Types filetype );
	bool copyTest( ERYDBDataFile::Types filetype );
	bool rdwrTest( ERYDBDataFile::Types filetype );
	bool hdfsRdwrExhaustTest();
	bool concurrencyTest( ERYDBDataFile::Types filetype );
	bool tellTest( ERYDBDataFile::Types filetype );
	bool openWF( bool reopen=false );
	bool openRF();
	bool flushTest(ERYDBDataFile::Types);
	bool seekTest(ERYDBDataFile::Types);
	bool listDirTest(ERYDBDataFile::Types, const string&);
...
struct TestCleanup
{
	TestCleanup( ERYDBDataFile::Types type, const string& dir, const string& file ) :
		m_fs( ERYDBFileSystem::getFs( type ) ),
		m_dir(dir),
		mm_file(file) {}
	...
	ERYDBFileSystem& m_fs;
	string	m_dir;
	string	mm_file;
};

bool TestRunner::runTest( ERYDBDataFile::Types filetype, unsigned open_opts )
{
	m_open_opts = open_opts;
	ostringstream infostr;
	string typeString = ( filetype == ERYDBDataFile::UNBUFFERED ? "Unbuffered" :
						( filetype == ERYDBDataFile::BUFFERED ? "Buffered" : "HDFS" ) );
	infostr << "Running test for file type " << typeString;
	logMsg( INFO, infostr.str() );

	if (filetype != ERYDBDataFile::UNBUFFERED && filetype != ERYDBDataFile::BUFFERED)
	{
	    if( !ERYDBPolicy::installPlugin(m_opts.pluginFile) )
	    {
	...
	oss << "/tmp/erydbdf-dir-" << getpid() << "-" << m_id << "/erydb/data";
	...
	if( filetype != ERYDBDataFile::HDFS )
	{
		// we expect this to fail because the parent directory should not exist
		reset();
		m_file = ERYDBDataFile::open(filetype, m_fname.c_str(), "w", m_open_opts);
		if( m_file )
		{
	...
	if( returnval && (filetype == ERYDBDataFile::UNBUFFERED || filetype == ERYDBDataFile::HDFS ))
	{
	...
	{
		reset();
		m_file = ERYDBDataFile::open(filetype, m_fname.c_str(), "r", m_open_opts);
		if( !m_file )
		{
			ostringstream errstr;
	...
	if( returnval && filetype == ERYDBDataFile::BUFFERED )
	{
	...
	if( returnval && filetype == ERYDBDataFile::HDFS )
	{
...
bool TestRunner::readTest( ERYDBDataFile::Types filetype )
{
...
bool TestRunner::openByModeStrTest()
{
	...
	reset();
	m_file = ERYDBDataFile::open(ERYDBDataFile::BUFFERED, m_fname.c_str(), "r+b", m_open_opts);
	if( !m_file )
	{
		ostringstream errstr;
...
bool TestRunner::writeTest( ERYDBDataFile::Types filetype )
{
...
bool TestRunner::truncateTest( ERYDBDataFile::Types filetype )
{
	logMsg( INFO, "truncateTest" );

	reset();
	m_file = ERYDBDataFile::open(filetype, m_fname.c_str(), "a", m_open_opts);
	if( !m_file )
	{
		ostringstream errstr;
		errstr << "Unable to open " << m_fname << " for writing";
    ...
    if( (filetype != ERYDBDataFile::HDFS) && rc)
    {
		logMsg( ERROR, "truncate failed!" );
		return false;
    }
    else if ( (filetype == ERYDBDataFile::HDFS) && !rc )
    {
		logMsg( ERROR, "truncate is supposed to fail for HDFS files!" );
		return false;
    }
    else if( (filetype == ERYDBDataFile::HDFS) )
    {
...
bool TestRunner::copyTest( ERYDBDataFile::Types type )
{
	// assume this test is run after the write test so that the file m_fname exists
	reset();
	ERYDBFileSystem& fs = ERYDBFileSystem::getFs( type );
...
bool TestRunner::rdwrTest( ERYDBDataFile::Types filetype )
{
...
bool TestRunner::hdfsRdwrExhaustTest()
{
	...
	ERYDBDataFile* file = ERYDBDataFile::open(ERYDBDataFile::HDFS, newpath.c_str(), "r+", 0, 8);
	assert( file );
	...
	ERYDBFileSystem& fs = ERYDBFileSystem::getFs( ERYDBDataFile::HDFS );
	assert( fs.size( newpath.c_str() ) == 12);

	// open again - the file is bigger than the default buffer so it triggers alternate
	// logic in the constructor
	file = ERYDBDataFile::open(ERYDBDataFile::HDFS, newpath.c_str(), "r+", 0, 8);
	assert( file );
	assert( file->size() == 12);
	...
	{
		file = ERYDBDataFile::open(ERYDBDataFile::HDFS, m_opts.largeFile.c_str(), "r+", 0);
		assert( file );
	}
	return true;
}

bool TestRunner::concurrencyTest( ERYDBDataFile::Types filetype )
{
	...
	ERYDBDataFile* rdwrFile = ERYDBDataFile::open(filetype, m_fname.c_str(), "r+", m_open_opts);
	assert(rdwrFile);
...
bool TestRunner::tellTest( ERYDBDataFile::Types filetype )
{
...
bool TestRunner::flushTest( ERYDBDataFile::Types filetype )
{
	...
	ERYDBPolicy::remove(m_fname.c_str());
	...
	ostringstream errstr;
	m_file = ERYDBDataFile::open(filetype, m_fname.c_str(), "w+", m_open_opts, /*default:4*/ 1);
	if (!m_file)
	{
	...
	m_file->flush();
	if (!ERYDBPolicy::exists(m_fname.c_str()))
	{
		errstr << "flushTest: " << m_fname.c_str() << " does not exist";
		logMsg( ERROR, errstr.str() );
		return false;
	}
	if (filetype == ERYDBDataFile::HDFS && boost::filesystem::exists(scratch))
	{
	...
	ERYDBDataFile* file = ERYDBDataFile::open(filetype, m_fname.c_str(), "r", m_open_opts, 1);
	if (!file)
	{
	...
	// check file size
	if (ERYDBPolicy::size(m_fname.c_str()) != 8 + m9)
	{
		errstr << "flushTest: size of " << m_fname.c_str() << " is "
				<< ERYDBPolicy::size(m_fname.c_str()) << ", expecting " << (8 + m9);
		logMsg( ERROR, errstr.str() );
		return false;
	}
	if (filetype == ERYDBDataFile::HDFS &&
	...
	// 2nd read
	file = ERYDBDataFile::open(filetype, m_fname.c_str(), "r", m_open_opts, 1);
	if (!file)
	{
...
bool TestRunner::seekTest( ERYDBDataFile::Types filetype )
{
	...
	// read from file in "r" mode
	ERYDBDataFile* file = ERYDBDataFile::open(filetype, m_fname.c_str(), "r", m_open_opts);
	if (!file)
	{
	...
	char r1[9] = {0};
	ssize_t n1 = file->pread(r1, 0, 8);
	if (ERYDBPolicy::size(m_fname.c_str()) != 8)
	{
		errstr << "seekTest: size of " << m_fname.c_str() << " is "
				<< ERYDBPolicy::size(m_fname.c_str()) << ", expecting 8";
		logMsg( ERROR, errstr.str() );
		return false;
	}
	...
	// check file size
	if (ERYDBPolicy::size(m_fname.c_str()) != 16)
	{
		errstr << "seekTest: size of " << m_fname.c_str() << " is "
				<< ERYDBPolicy::size(m_fname.c_str()) << ", expecting 16";
		logMsg( ERROR, errstr.str() );
		return false;
	}

	// 2nd read
	file = ERYDBDataFile::open(filetype, m_fname.c_str(), "r", m_open_opts);
	if (!file)
	{
...
bool TestRunner::listDirTest( ERYDBDataFile::Types filetype, const string& dir )
{
	...
	ERYDBDataFile* file2 = ERYDBDataFile::open(filetype, fname2.c_str(), "w", m_open_opts);
	if (file2)
		delete file2;

	ERYDBDataFile* file3 = ERYDBDataFile::open(filetype, fname3.c_str(), "w", m_open_opts);
	if (file3)
		delete file3;
...
bool TestRunner::doBlock(unsigned int blocknum, unsigned char tag, unsigned int count)
{
	...
	if( m_opts.closeonwrite )
	{
		...
		m_file = ERYDBDataFile::open( ERYDBDataFile::HDFS, m_fname.c_str(), "a", m_open_opts );
		if( !m_file )
			return false;
	}
	...
	if( m_opts.reopen )
	{
		delete m_file;
		// will have to cache type somewhere later
		m_file = ERYDBDataFile::open( ERYDBDataFile::HDFS, m_fname.c_str(), "r", m_open_opts );
		if( !m_file )
			return false;
	}
...
void thread_func( TestRunner& trun )
{
	...
	ret = ret && trun.runTest( ERYDBDataFile::UNBUFFERED, 0 );
	// todo-add some capability to use USE_VBUF or not
	ret = ret && trun.runTest( ERYDBDataFile::BUFFERED, 0 );
	if( trun.runOpts().useHdfs )
		ret = ret && trun.runTest( ERYDBDataFile::HDFS, 0 );
...
int main(int argc, char** argv)
{
    ...
    ERYDBPolicy::init( true, true, "/tmp/rdwr_scratch", opts.hdfsMaxMem);
