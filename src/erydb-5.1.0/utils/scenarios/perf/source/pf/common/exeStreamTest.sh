...
do
     ...
     then
         /usr/local/erydb/bin/eryadmin restartsystem y
         sleep 90
     fi
     mkdir $k
     cd $k
     cp /usr/local/erydb/etc/erydb.xml .
     /usr/local/erydb/bin/eryadmin geterydbSoftware >erydbSoftware.txt
     ls -al /mnt/pm*/usr/local/erydb/data* > dbRoots.txt
     /usr/local/erydb/bin/eryadmin getProcessStatus >stackConfigBefore.txt
...
#
     /usr/local/erydb/bin/eryadmin getProcessStatus >stackConfigAfter.txt
