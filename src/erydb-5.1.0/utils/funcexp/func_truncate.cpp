...
erydbSystemCatalog::ColType Func_truncate::operationType(FunctionParm& fp, erydbSystemCatalog::ColType& resultType)
{
	if (resultType.colDataType == execplan::erydbSystemCatalog::DECIMAL ||
		resultType.colDataType == execplan::erydbSystemCatalog::UDECIMAL)
	{
...
{
	ERYDB_Decimal x = getDecimalVal(row, parm, isNull, op_ct);
...
double Func_truncate::getDoubleVal(Row& row,
							...
							erydbSystemCatalog::ColType& op_ct)
{
	if (execplan::erydbSystemCatalog::DOUBLE == op_ct.colDataType ||
		execplan::erydbSystemCatalog::FLOAT == op_ct.colDataType)
	{
		int64_t d = 0;
		double  p = 1;
...
ERYDB_Decimal Func_truncate::getDecimalVal(Row& row,
							FunctionParm& parm,
							bool& isNull,
							erydbSystemCatalog::ColType& op_ct)
{
	...
	switch (op_ct.colDataType)
	{
		...
		case execplan::erydbSystemCatalog::USMALLINT:
			if (x.scale != 0)
			{
				...
				{
					x.value /= ERYDB_pow[x.scale];
				}
				else if (x.scale < 0 && x.scale > -19)
				{
					x.value *= ERYDB_pow[-x.scale];  // may overflow
				}
				else if (x.scale > 0)
				{
					x.value = 0;
