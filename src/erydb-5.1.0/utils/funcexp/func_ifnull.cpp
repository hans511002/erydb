...
erydbSystemCatalog::ColType Func_ifnull::operationType(FunctionParm& fp, erydbSystemCatalog::ColType& resultType)
{
...
ERYDB_Decimal Func_ifnull::getDecimalVal(Row& row,
							FunctionParm& parm,
							bool& isNull,
							erydbSystemCatalog::ColType&)
{
	if (isNull)
		return ERYDB_Decimal();

	ERYDB_Decimal r = parm[0]->data()->getDecimalVal(row, isNull);
	if (isNull)
	{
		isNull = false;
