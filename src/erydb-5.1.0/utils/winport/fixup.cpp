...
int fixuperydbXML()
{
	...
	try
	{
		...
		if (val.empty())
		{
			val = ERYDBreadRegistry("") + "/dbrm/tablelocks";
			cf->setConfig(section, parm, val);
			cout << "Adding " << section << "." << parm << " = " << val << endl;
		}
	...
	{
		cout << "fixuperydbXML caught exception: " << e.what() << endl;
	}
	catch (...)
	{
		cout << "fixuperydbXML caught exception (...): " << endl;
	}
...
int fixupConfig(const string& installDir, const string& mysqlPort)
{
	...
	while (ifs.good())
	{
		...
		sedit(strLine, "#erydb_compression_type=0", "erydb_compression_type=2");
		sedit(strLine, "erydb_compression_type=1", "erydb_compression_type=2");
#endif
		ofs << strLine << endl;
		getline(ifs, strLine);
	}
	...
	// Add install timestamp to erydbVersion.txt
	char szTime[24];
	...
	okayToRename = false;
	cFilePath = installDir;
	cFilePath /= "etc/erydbVersion.txt";
	tmpPath = cFilePath;
	tmpPath.replace_extension(".tmp");
	fs::remove(tmpPath);
	ifs.open(cFilePath.string().c_str());
