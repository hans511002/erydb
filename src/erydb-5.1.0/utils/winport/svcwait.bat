...
rem wait for EryDB service to go to state %1 (RUNNING)
...
for /f "usebackq tokens=3" %%a in (`sc.exe query EryDB`) do call :checkinst %%a
...
:checkstate
for /f "usebackq tokens=1-4" %%a in (`sc.exe query EryDB`) do call :cshelper %%a %%d
goto :eof
...
:timedout
echo Timed-out waiting for EryDB to go to state %state%!
exit /b 1
goto :eof
