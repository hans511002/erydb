...
while read moduletype hostname hostdir; do
	...
	cp /tmp/pratinput.txt $CTOOLS/data/$hostdir
	  ...
	  echo "  and copying them to this server"
          tarfile=$hostdir.tar
          /usr/local/erydb/bin/remote_command.sh $servername $srvpwd "$CTOOLS/tarfiles.sh $hostdir $tarfile" 
	  cd $CTOOLS/data
          /usr/local/erydb/bin/remote_scp_get.sh $servername $srvpwd $CTOOLS/data/$tarfile 
       fi
       done
    fi
done
