	...
	m_pFileBuffer(NULL)
{
	...
	ERYDBFileSystem& fs = ERYDBFactory::getFs( HDFS );
    off64_t bufSize;
    if (colWidth == 0)
        bufSize = DEFSIZE;
    else if (colWidth > 8)
...
void HdfsRdwrMemBuffer::allocBuffer(size_t size)
{
    ...
    if( ERYDBLogger::isEnabled() )
    {
        ...
        ERYDBLogger::logNoArg(oss.str(), this, "buffer", size-m_size);
    }
...
void HdfsRdwrMemBuffer::releaseBuffer()
{
    ...
    if( ERYDBLogger::isEnabled() )
    {
        ...
        ERYDBLogger::logNoArg(oss.str(), this, "buffer", -m_size);
    }
...
ssize_t HdfsRdwrMemBuffer::pread(void *ptr, off64_t offset, size_t count)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logRW("pread", m_fname, this, offset, count, bytestoread);
...
void HdfsRdwrMemBuffer::checkRealloc(size_t count)
{
	...
	if( m_cur + static_cast<off64_t>( count ) > m_size )
	{
		...
		if ((ERYDBPolicy::hdfsRdwrBufferMaxSize() == 0 ||
			(HdfsRdwrMemBuffer::getTotalBuff() + newsize) < ERYDBPolicy::hdfsRdwrBufferMaxSize())
...
ssize_t HdfsRdwrMemBuffer::write(const void *ptr, size_t count)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logRW("write", m_fname, this, offset, count, count);
...
int HdfsRdwrMemBuffer::seek(off64_t offset, int whence)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logSeek(m_fname, this, offset, whence, ret);
...
int HdfsRdwrMemBuffer::truncate(off64_t length)
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logTruncate(m_fname, this, length, ret);
...
off64_t HdfsRdwrMemBuffer::size()
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logSize(m_fname, this, m_last);
...
int HdfsRdwrMemBuffer::flushImpl()
{
	...
	ERYDBDataFile* writeFile = ERYDBDataFile::open(HDFS, m_fname.c_str(), "w", 0);
	if ( !writeFile )
	{
		ostringstream oss;
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logNoArg(m_fname, this, "flush", 0);
...
int HdfsRdwrMemBuffer::close()
{
	...
	if( ERYDBLogger::isEnabled() )
		ERYDBLogger::logNoArg(m_fname, this, "close", 0);
	return 0;
}
