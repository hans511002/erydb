...
{
	ct.colWidth = 1;
	ct.constraintType = erydbSystemCatalog::NO_CONSTRAINT;
...
void dc8()
{
	...
	int status = 0;
	uint32_t anyval;
	anyval = converter.convertColumnDate( data.c_str(), ERYDBDATE_ENUM, status, data.length());
...
void dc_datetest_perf()
{
	...
	for (int i = 0; i < iters; i++)
	{
		converter.convertColumnDate( data.c_str(), ERYDBDATE_ENUM, status, data.length());
	}
    gettimeofday(&tv2, 0);
...
void dc_datetimetest_perf()
{
	...
	for (int i = 0; i < iters; i++)
	{
		converter.convertColumnDatetime( data.c_str(), ERYDBDATETIME_ENUM, status, data.length());
	}
    gettimeofday(&tv2, 0);
...
{
	bool erydbval = isDateValid( day, month, year );
	...
	bool check = boost_only ? (erydbval == boostval) :
					(erydbval == valid && ((boostval == erydbval) || boostval));
	if (!check)
	{
		cout << "(" << year << "," << month << "," << day << "), expected:"
				<< valid << ", erydb=" << erydbval << ", boost=" << boostval << endl;
	}
...
void dc9()
{
	ct.colWidth = 4;
	ct.constraintType = erydbSystemCatalog::NO_CONSTRAINT;
	ct.colDataType = erydbSystemCatalog::FLOAT;
	ct.scale = 0;
	ct.precision = 4;
	bool pushWarning;
