...
*
*/
class ERYDBExcept : public std::runtime_error
{
public:
	ERYDBExcept(uint16_t code) :
		std::runtime_error(ERYDBErrorInfo::instance()->errorMsg(code)), fErrCode(code) {}
	ERYDBExcept(uint16_t code, const Message::Args& args) :
		std::runtime_error(ERYDBErrorInfo::instance()->errorMsg(code, args)), fErrCode(code){}
	ERYDBExcept(const std::string& msg, uint16_t code) :
...
class DictionaryBufferOverflow : public ERYDBExcept
{
public:
	DictionaryBufferOverflow() : ERYDBExcept(ERR_DICTBUFFER_OVERFLOW)
	{ }
};
...
*
*/
class QueryDataExcept : public ERYDBExcept
{
public:
	/** Takes a character string describing the error.  */
	QueryDataExcept(const std::string& msg, uint16_t code) :
		ERYDBExcept(msg, code)  { }
...
/** @brief specific error exception class for getSysData in erydbsystemcatalog.
*   @bug 2574
*
*/
...
class MoreThan1RowExcept : public ERYDBExcept
{
public:
	MoreThan1RowExcept() :
		ERYDBExcept(ERR_MORE_THAN_1_ROW)
	{ }
};

class CorrelateFailExcept : public ERYDBExcept
{
public:
	CorrelateFailExcept() :
		ERYDBExcept(ERR_CORRELATE_FAIL)
	{ }
};
...
\
		...
		throw logging::ERYDBExcept(logging::ERR_ASSERTION_FAILURE); \
	} \
} while (0)
