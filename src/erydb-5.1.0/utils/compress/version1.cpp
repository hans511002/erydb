...
void initCtlShm()
{
	...
	rc = fork();
	erydbassert_s(rc >= 0, "couldn't fork DecomSvr");
//	if (rc < 0)
...
//	else if (rc == 0)
	if (rc == 0)
	{
		...
		erydbassert_s(0, "couldn't exec DecomSvr");
	...
	erydbassert_s(CreateProcess(0, (LPSTR)srvrpath.c_str(), 0, 0, false, DETACHED_PROCESS, 0, 0, &sInfo, &pInfo) != 0,
	...
	char* p = getenv("ERYDB_DECOMSVR_PORT");
	if (p && *p)
	{
		DSPort = atoi(p);
		if (DSPort <= 0)
...
void readn(int fd, void* buf, const size_t wanted)
{
	...
	while (wanted > sofar)
	{
		...
		if (rrc < 0)
		{
			...
			//this throws logging::ERYDBExcept()
			erydbassert_s(0, oss.str());
		}
		if (rrc == 0)
		{
...
bool decompress(const char* in, const uint32_t inLen, unsigned char* out, size_t* ol)
{
	...
	cpipeh = CreateNamedPipe(cpipe.c_str(), PIPE_ACCESS_OUTBOUND, PIPE_TYPE_BYTE|PIPE_READMODE_BYTE|PIPE_WAIT,
		1, 0, 8192, 0, 0);
	erydbassert_s(cpipeh != INVALID_HANDLE_VALUE, "while creating cdata fifo");
	//if (cpipeh == INVALID_HANDLE_VALUE)
	...
	upipeh = CreateNamedPipe(upipe.c_str(), PIPE_ACCESS_INBOUND, PIPE_TYPE_BYTE|PIPE_READMODE_BYTE|PIPE_WAIT,
		1, 8192, 0, 0, 0);
	erydbassert_s(upipeh != INVALID_HANDLE_VALUE, "while creating udata fifo");
	//if (upipeh == INVALID_HANDLE_VALUE)
	...
	unlink(cpipe.c_str());
	erydbassert_s(mknod(cpipe.c_str(), S_IFIFO|0666, 0) == 0, "while creating cdata fifo");
	//if (mknod(cpipe.c_str(), S_IFIFO|0666, 0) != 0)
	//	throw runtime_error("while creating cdata fifo");
	unlink(upipe.c_str());
	erydbassert_s(mknod(upipe.c_str(), S_IFIFO|0666, 0) == 0, "while creating udata fifo");
...
#ifdef _MSC_VER
	erydbassert_s(fd != INVALID_SOCKET, 
...
again:
	...
	if (rc < 0)
	{
...
#endif
		{
			erydbassert_s(++tries < MaxTries, string("socket connect error: ") + strerror(errno));
			//if (++tries >= MaxTries)
		...
		erydbassert_s(0, string("socket connect error: ") + strerror(errno));
		//throw runtime_error(string("socket connect error: ") + strerror(errno));
	}
	...
	erydbassert_s(fd >= 0, "while opening data fifo for write");
	//if (fd < 0)
	...
	wrc = writen(fd, &u64, 8);
	int err = errno;
	erydbassert_s(wrc == 8, string("while writing compressed len to the DS: ") + strerror(err));
//	if (wrc != 8)
//	{
//		ostringstream oss;
...
//	}
	...
	erydbassert_s(wrc == static_cast<ssize_t>(u64), "while writing compressed data to the DS");
...
//		throw runtime_error("while writing compressed data to the DS");
	...
	erydbassert_s(!(dwrc == 0 && GetLastError() != ERROR_PIPE_CONNECTED), "connecting to upipe");
	//if (dwrc == 0 && GetLastError() != ERROR_PIPE_CONNECTED)
	//	throw runtime_error("connecting to upipe");

	DWORD nread;
	dwrc = ReadFile(upipeh, &u64, 8, &nread, 0);
	erydbassert_s(dwrc != 0 && nread == 8, "while reading from upipe");
	//if (!(dwrc != 0 && nread == 8))
	//	throw runtime_error("while reading from upipe");

	dwrc = ReadFile(upipeh, out, u64, &nread, 0);
	erydbassert_s(dwrc != 0 && nread == u64, "while reading from upipe");
	//if (!(dwrc != 0 && nread == u64))
	//	throw runtime_error("while reading from upipe");
	...
	erydbassert_s(fd >= 0, "while opening data fifo for read");
//	if (fd < 0)
