...
static size_t sysObjectERYDByteLength;
...
system_parse_config_sysObjectID(const char *token, char *cptr)
{
	...
	netsnmp_config_error("sysobjectid token not a parsable OID:\n\t%s",
			     cptr);
        sysObjectERYDByteLength = version_sysoid_len  * sizeof(oid);
        memcpy(sysObjectID, version_sysoid, sysObjectERYDByteLength);
    } else

		sysObjectERYDByteLength = sysObjectIDLength * sizeof(oid);
}
...
init_system_mib(void)
{
    ...
    }
    {
		...
		&sysObjectID_winfo, sysObjectID, 0, ASN_OBJECT_ID,
                ...
                MAX_OID_LEN, &sysObjectERYDByteLength));
    }
    {
