...
netsnmp_parse_override(const char *token, char *line)
{
    ...
    size_t          oerydbuf_len = MAX_OID_LEN;
    int             type;
    ...
    if (!snmp_parse_oid(namebuf, oerydbuf, &oerydbuf_len)) {
    ...
    the_reg->rootoid = snmp_duplicate_objid(oerydbuf, oerydbuf_len);
    the_reg->rootoid_len = oerydbuf_len;
