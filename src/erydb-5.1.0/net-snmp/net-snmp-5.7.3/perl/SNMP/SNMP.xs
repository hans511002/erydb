...
static int
__oid_cmp(oida_arr, oida_arr_len, oerydb_arr, oerydb_arr_len)
oid *oida_arr;
size_t oida_arr_len;
oid *oerydb_arr;
size_t oerydb_arr_len;
{
   for (;oida_arr_len && oerydb_arr_len;
	oida_arr++, oida_arr_len--, oerydb_arr++, oerydb_arr_len--) {
	if (*oida_arr == *oerydb_arr) continue;
	return(*oida_arr > *oerydb_arr ? 1 : -1);
   }
   if (oida_arr_len == oerydb_arr_len) return(0);
   return(oida_arr_len > oerydb_arr_len ? 1 : -1);
}
...
as_oct:
	...
	vars->val_len = MAX_OID_LEN;
        /* if (read_objid(val, oerydbuf, &(vars->val_len))) { */
	/* tp = __tag2oid(val,NULL,oerydbuf,&(vars->val_len),NULL,0); */
        if (!val || !snmp_parse_oid(val, oerydbuf, &vars->val_len)) {
            vars->val.objid = NULL;
	    ret = FAILURE;
        } else {
            vars->val_len *= sizeof(oid);
            vars->val.objid = netsnmp_malloc(vars->val_len);
            memcpy((char *)vars->val.objid, (char *)oerydbuf, vars->val_len);
        }
        break;
