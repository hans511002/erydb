...
int64_t erydbbigdec2(const string& str)
{
	int64_t val=0;
...
int32_t erydbdate(const string& str)
{
	date_image di;
	di.i = 0;
...
int parseinto(lineitem_image& img, const string& buf)
{
	...
	img.l_quantity = erydbbigdec2(tok_iter->c_str()); ++tok_iter;
	img.l_extendedprice = erydbbigdec2(tok_iter->c_str()); ++tok_iter;
	img.l_discount = erydbbigdec2(tok_iter->c_str()); ++tok_iter;
	img.l_tax = erydbbigdec2(tok_iter->c_str()); ++tok_iter;
	img.l_returnflag = tok_iter->at(0); ++tok_iter;
	img.l_linestatus = tok_iter->at(0); ++tok_iter;
	img.l_shipdate = erydbdate(tok_iter->c_str()); ++tok_iter;
	img.l_commitdate = erydbdate(tok_iter->c_str()); ++tok_iter;
	img.l_receiptdate = erydbdate(tok_iter->c_str()); ++tok_iter;
