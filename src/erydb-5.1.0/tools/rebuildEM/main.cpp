...
OID_t pname2OID(const string& fpath)
{
	...
	erydbassert(pcomps.size() >= 4);

	string pcomp;
	pcomp = pcomps.top();
	pcomps.pop();
	erydbassert(pcomp.size() < 16);
	strcpy(args.pFName, pcomp.c_str());
	pcomp = pcomps.top();
	pcomps.pop();
	erydbassert(pcomp.size() < 10);
...
bool iserydbDBFile(const string& fpath)
{
...
int walkDB(const char* fp, const struct stat* sb, int typeflag, struct FTW* ftwbuf)
{
	...
	if (!iserydbDBFile(fpath))
	{
		if (vflg > 2)
		{
			cout << "Skipping non-erydb DB file " << fpath << endl;
		}
		return FTW_CONTINUE;
	}
