...
sub findRepos
{
	...
	open(FH, 'curl -s http://srvengcm1.erydb.com/cgi-bin/listRepos.pl' . $all . ' |');
	while (<FH>)
	{
		$_ =~ s/[\r\n]*$//;
		push(@repolist, $_);
...
{
	$cmd = 'svn list http://srvengcm1.erydb.com/svn/genii/dbcon/';
	if (defined($opt_b))
	{
		$cmd = $cmd . 'branches';
	}
...
foreach $repo (@repos)
{
	...
	else
	{
		...
		if (defined($opt_b))
		{
			$svncmd = 'svn checkout http://srvengcm1.erydb.com/svn/genii/' . $repo . '/branches/' .
				$prjroot . ' ' . $repo;
		}
		elsif (defined($opt_t))
		{
			$svncmd = 'svn checkout http://srvengcm1.erydb.com/svn/genii/' . $repo . '/tags/' .
				$prjroot . ' ' . $repo;
		}
		else
		{
			$svncmd = 'svn checkout http://srvengcm1.erydb.com/svn/genii/' . $repo . '/trunk/' . ' ' .
				$repo;
		}
		system $svncmd;
	}
