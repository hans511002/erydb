...
bool TestRunner::runTest( ERYDBDataFile::Types filetype, unsigned open_opts )
{
	...
	infostr << "Running test for file type "
			<< ( filetype == ERYDBDataFile::UNBUFFERED ? "Unbuffered" :
	...
	oss << "/tmp/erydbdf-dir-" << getpid() << "-" << m_id;
	string dir = oss.str();
	m_fname = dir + "/foobar";
	...
	if( m_opts.numDbRoots > 0 )
	{
		...
		for( int i = 0; i < m_opts.numDbRoots; ++i )
		{
			...
			dbroot << "/usr/local/erydb/data" << i+1;
...
int main(int argc, char** argv)
{
	...
	try
	{
    	...
    	ERYDBPolicy::init( true, true, hdfsRdwrScratch, 2147483648 );
	
		if( !ERYDBPolicy::installPlugin(opts.pluginFile) )
		{
	...
	{
		logFailure("failure to initialize ERYDBPolicy and/or load HDFS plugin");
		return -1;
	}
	...
	while ( !done )
	{
		...
		try
		{
			if( trun.runTest( ERYDBDataFile::HDFS, 0 ) )
			{
				done = true;
