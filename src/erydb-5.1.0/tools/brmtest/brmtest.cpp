...
void LBIDList(erydbSystemCatalog::OID OID, ostringstream& cout_str)
{
...
void doit(const erydbSystemCatalog::TableColName& tcn, ostringstream& cout_str)
{
	...
	if (OID > 0)
    {
    	...
    	erydbSystemCatalog::OID ioid = Cat->lookupOID(tcn);
    	erydbSystemCatalog::ColType CT = Cat->colType(ioid);
    
    	erydbSystemCatalog::DictOID DOID = CT.ddn;
	...
	erydbSystemCatalog::IndexOID IOID = Cat->lookupIndexNbr(tcn);
	if (IOID.objnum > 0)
	{
...
int main(int argc, char** argv)
{
	...
	Cat = erydbSystemCatalog::makeerydbSystemCatalog();
