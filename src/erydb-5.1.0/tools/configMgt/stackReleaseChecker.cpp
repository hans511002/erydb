	...
	for ( int i = 1 ; i < systemCount+1 ; i++)
	{
		...
		if ( tempSystem == systemName || systemName == "all") {
			...
			if (!VERBOSE)
				...
				cmd = "./remote_command.sh " + installParentModuleHostName + " " + systemUser + " " + password + " '" + installDir + "/bin/erydbConsole geterydbsoftware' 'tools' Error 10 " + debug_flag;
			...
			cmd = "./remote_command.sh " + installParentModuleHostName + " " + systemUser + " " + password + " '" + installDir + "/bin/erydbConsole getsystems' dummy Error 10 " + debug_flag;
