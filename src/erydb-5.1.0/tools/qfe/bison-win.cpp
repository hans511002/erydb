...
boost::shared_ptr<execplan::erydbSystemCatalog> ParserCSC;
namespace execplan
{
...
#line 180 "qfeparser.ypp"
    {
			...
			execplan::SimpleColumn* sc = new execplan::SimpleColumn("erydb_unknown", "erydb_unknown", *((yyvsp[(1) - (1)].cp)));
			sc->tableAlias("erydb_unknown");
			sc->alias(execplan::make_tcn("erydb_unknown", "erydb_unknown", *((yyvsp[(1) - (1)].cp))).toString());
...
#line 191 "qfeparser.ypp"
    {
			//This is possibly a table name, but we shove it into a SimpleColumn. We'll
			// fix this in the table_spec production
			execplan::SimpleColumn* sc = new execplan::SimpleColumn("erydb_unknown", *((yyvsp[(1) - (3)].cp)), *((yyvsp[(3) - (3)].cp)));
			sc->tableAlias(*((yyvsp[(1) - (3)].cp)));
			sc->alias(execplan::make_tcn("erydb_unknown", *((yyvsp[(1) - (3)].cp)), *((yyvsp[(3) - (3)].cp))).toString());
			//cerr << "inside parser: " << *sc << endl;
			delete (yyvsp[(1) - (3)].cp);
			delete (yyvsp[(3) - (3)].cp);
			(yyval.rcp) = sc;
