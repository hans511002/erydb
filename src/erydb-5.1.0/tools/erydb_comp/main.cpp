...
void usage()
{
	cout << "usage: erydb_comp [-e size][-h] file(s)" << endl
...
int doit(const string& infile)
{
	...
	erydbassert(sizeof(compHdr) <= 4096);
	...
	while (nread > 0)
	{
		...
		if (cbuf_len > (unsigned)nread)
		{
			...
			erydbassert((cbuf_len - nread) <= exp_buf_size * 1024);
		}
		if (cbuf_len & 0xfff)
	...
	erydbassert(nwritten == TOTAL_HDR_LEN);
...
out:
	...
	fstat(ofd, &ostatbuf);
	erydbassert(ostatbuf.st_size == (signed)hdrptr->ptrs[idx]);
	delete [] hdrbuf;
