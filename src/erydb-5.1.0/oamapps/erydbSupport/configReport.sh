...
if [ -n "$chkconfig" ]; then
	...
	echo "################# chkconfig --list | grep mysql-erydb #################"
	echo " "
	$SUDO chkconfig --list | grep mysql-erydb 2>/dev/null
fi
...
if [ -n "$systemctl" ]; then
	...
	echo "################# systemctl list-unit-files --type=service | grep mysql-erydb #################"
	echo " "
	$SUDO systemctl list-unit-files --type=service | grep mysql-erydb 2>/dev/null
fi
...
if [ -n "$updaterc" ]; then
	...
	echo "################# service --status-all | grep mysql-erydb #################"
	echo " "
	$SUDO service --status-all | grep mysql-erydb 2>/dev/null
fi
