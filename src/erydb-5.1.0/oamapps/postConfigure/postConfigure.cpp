...
*		erydb/etc/snmpd.conf
*		erydb/etc/snmptrapd.conf
*		erydb/etc/erydb.xml
...
//  	struct sysinfo myinfo; 
	...
	for( int i = 1; i < argc; i++ )
	{
		if( string("-h") == argv[i] ) {
			...
			i++;
			if (i >= argc ) {
				cout << "   ERROR: erydb Password not provided" << endl;
				exit (1);
			}
			mysqlpw = argv[i];
			if ( mysqlpw.find("-") != string::npos ) {
				cout << "   ERROR: Valid erydb Password not provided" << endl;
				exit (1);
			}			
...
//		redirectStandardOutputToFile(postConfigureLog, false );
	...
	string cmd = installDir + "/bin/erydb status > /tmp/status.log";
	system(cmd.c_str());
	if (oam.checkLogStatus("/tmp/status.log", "EryDB is running") ) {
	...
	if ( amazonInstall )
	{
		...
		cout <<     "This option is recommended and would be use if you are setting up a erydb system." << endl << endl;
...
//							cout << "copyFstab error" << endl;
	...
	if ( IserverTypeInstall == oam::INSTALL_COMBINE_DM_UM_PM && pmNumber == 1) {
		...
		cout << endl << "===== Running the erydb erydb setup scripts =====" << endl << endl;
	...
	if ( IserverTypeInstall != oam::INSTALL_COMBINE_DM_UM_PM || 
		...
		if ( install == "y" ) {
			...
			{
				cout << "ERROR: Problem getting EEPackageType from the erydb System Configuration file" << endl;
				exit(1);
			}
			...
			if ( EEPackageType == "rpm" )
				cout << "Performing an erydb System install using RPM packages located in the " + HOME + " directory." << endl;
			else
			{
				if ( EEPackageType == "binary" )
					cout << "Performing an erydb System install using a Binary package located in the " + HOME + " directory." << endl;
				else
					cout << "Performing an erydb System install using using DEB packages located in the " + HOME + " directory." << endl;
			}
			...
			if ( EEPackageType != "binary") {
				...
				if ( EEPackageType == "deb" )
					separator = "_";
				erydbPackage1 = "erydb-platform" + separator + version;
				erydbPackage2 = "erydb-libs" + separator + version;
				erydbPackage3 = "erydb-enterprise" + separator + version;
				mysqlPackage = "erydb-storage-engine" + separator + version;
				mysqldPackage = "erydb-mysql" + separator + version;
				...
				{
				//mariadb
					erydbPackage1 = "erydb-*" + separator + version;

					erydbPackage1 = HOME + "/" + erydbPackage1 + "*." + EEPackageType;
					erydbPackage2 = HOME + "/" + erydbPackage2 + "*." + EEPackageType;
					erydbPackage3 = HOME + "/" + erydbPackage3 + "*." + EEPackageType;
			...
			else
			{
				...
				if (!file)	// CE
					erydbPackage1 = "erydb-" + version;
				else		// EE
					erydbPackage1 = "erydb-ent-" + version;
				erydbPackage2 = "dummy";
				erydbPackage3 = "dummy";
				mysqlPackage = erydbPackage1;
				mysqldPackage = erydbPackage1;

				if( !pkgCheck() )
					exit(1);
				erydbPackage1 = HOME + "/" + erydbPackage1 + "*.bin.tar.gz";
				erydbPackage2 = "dummy";
				erydbPackage3 = "dummy";
			}
			...
			//If ent pkg is not there, mark it as such
			{
				...
				if (glob(erydbPackage3.c_str(), 0, 0, &gt) != 0)
					erydbPackage3 = "dummy.rpm";
				globfree(&gt);
			}
						...
						else
						{
							...
							if ( pwprompt == " " ) {
								...
								if (oam.checkLogStatus("/tmp/erydbmysql.log", "ERROR .my.cnf") ) {
									...
									rtnCode = system(cmd.c_str());
									if (WEXITSTATUS(rtnCode) != 0) {
										cout << endl << "erydb  login failure, password is assigned. Need erydb password configuration file " + HOME + "/.my.cnf on " << remoteModuleName << endl;
										exit(1);
									}
...
//						checkRemoteMysqlPort(remoteModuleIP, remoteModuleName, USER, password, mysqlPort, sysConfig);
						...
						cmd = installDir + "/bin/binary_installer.sh " + remoteModuleName + " " +
							remoteModuleIP + " " + password + " " + erydbPackage1 + " " + remoteModuleType +
...
//cout << cmd << endl;
						...
						{	// do a binary package install
							...
							cmd = installDir + "/bin/binary_installer.sh " + remoteModuleName + " " + remoteModuleIP +
								" " + password + " " + erydbPackage1 + " " + remoteModuleType + " initial " +
		...
		if ( start == "y" ) {
			...
			else
			{
				...
				for (; list1 != childmodulelist.end() ; list1++)
				{
					...
					cmd = installDir + "/bin/remote_scp_put.sh " + remoteModuleIP + " " + installDir + "/etc/erydb.xml " +  installDir + "/etc/. > /dev/null 2>&1";
					int rtnCode = system(cmd.c_str());

					cmd = installDir + "/bin/remote_command.sh " + remoteModuleIP + " " + password +
						" '" + installDir + "/bin/erydb restart' 0";
					rtnCode = system(cmd.c_str());
					if (WEXITSTATUS(rtnCode) != 0)
...
bool pkgCheck()
{
	while(true) 
	{
		string cmd = "ls " + HOME + " | grep " + erydbPackage1 + " > /tmp/erydbpkgs";
		system(cmd.c_str());
	
		string pkg = erydbPackage1;
...
void snmpAppCheck()
{
	...
	cout << endl << "===== erydb SNMP-Trap Process Check  =====" << endl << endl;
	cout << "erydb is packaged with an SNMP-Trap process." << endl;
	cout << "If the system where erydb is being installed already has an SNMP-Trap process" << endl;
	cout << "running, then you have the option of disabling erydb's SNMP-Trap process." << endl;
	cout << "Not having the erydb SNMP-Trap process will affect the" << endl;
	cout << "generation of erydb alarms and associated SNMP traps." << endl;
	cout << "Please reference the erydb Installation Guide for" << endl;
	...
	while(true)
	{
		if ( enableSNMP == "y" ) {
			string disable = "n";
			pcommand = callReadline("erydb SNMP-Trap process is enabled, would you like to disable it [y,n] (n) > ");
			if (pcommand)
			{
		...
		{
			string enable = "n";
			pcommand = callReadline("erydb SNMP-Trap process is disabled, would you like to enable it (y,n) [n] > ");
			if (pcommand)
			{
				if (strlen(pcommand) > 0) enable = pcommand;
				callFree(pcommand);
	...
	if (enableSNMP == "y") {
		...
		try
		{
			...
			cout << endl << "erydb SNMP Process successfully enabled" << endl;
		}
		catch (exception& e)
		{
		...
		cout << "This would be used to receive SNMP Traps from erydb." << endl;
	...
	{	//disabled, update config file
		try
		{
			...
			cout << endl << "erydb SNMP-Trap Process successfully disabled" << endl;
		}
		catch (exception& e)
		{
			cout << endl << "**** setProcessConfig Failed =  " << e.what() << endl;
...
bool updateBash()
{
	...
	if (!rootUser)
	{
		string cmd = "echo export ERYDB_INSTALL_DIR=" + installDir + " >> " + fileName;
		system(cmd.c_str());
	
		cmd = "echo export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$ERYDB_INSTALL_DIR/lib:$ERYDB_INSTALL_DIR/mysql/lib/mysql >> " + fileName;
		system(cmd.c_str());
	}
