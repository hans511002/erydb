	...
	for (;;)
	{
		...
		if (oam.checkLogStatus("/tmp/erydbmysql.log", "ERROR 1045") ) {
			...
			try {
				...
				cout << "NOTE: Using erydb password from " + HOME + "/.my.cnf" << endl;
			}
			catch (...)
			{
				cout << endl << "MariDB Erydb login failure, password is assigned. Need erydb password configuration file " + HOME + "/.my.cnf on local module" << endl;
				exit(1);
			}
				...
				try {
					...
					if (opState == oam::ACTIVE ||
						...
						{	
							...
							if ( returnStatus != API_SUCCESS) {
								cout << "ERROR: Error return in running the erydb Master DB Distribute, check /tmp/master-dist.logs on " << masterModule << endl;
								return returnStatus;
							}
							...
							if ( returnStatus != API_SUCCESS) {
								cout << "ERROR: Error return in running the erydb Master replication, check /tmp/master-rep*.logs on " << masterModule << endl;
								return returnStatus;
							}
