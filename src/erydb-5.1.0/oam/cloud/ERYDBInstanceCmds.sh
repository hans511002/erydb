...
subnet=`$prefix/erydb/bin/getConfig Installation AmazonSubNetID`
...
getZone() {
	#get from erydb.xml if it's there, if not, get from instance then store
	zone=`$prefix/erydb/bin/getConfig Installation AmazonZone`
	...
	if [ "$zone" = "unassigned" ] || [ "$zone" = "" ]; then
		...
		$prefix/erydb/bin/setConfig Installation AmazonZone $zone
	fi
