...
# $Id$
#
# generic EryDB Master Replication script.
#
...
cat >/tmp/erydb_master-rep.sql <<EOD
CREATE USER '$repUser'@'$hostipaddr' IDENTIFIED BY '$password';
GRANT REPLICATION SLAVE ON *.* TO '$repUser'@'$hostipaddr';
EOD

cat /tmp/erydb_master-rep.sql >>/tmp/master-rep-status-$hostipaddr.log
$installdir/mysql/bin/mysql \
	--defaults-file=$installdir/mysql/my.cnf \
	--user=root $pwprompt \
	erydbsys </tmp/erydb_master-rep.sql >>/tmp/master-rep-status-$hostipaddr.log
...
cat >/tmp/erydb_master-rep.sql <<EOD
use mysql
grant all on *.* to '$repUser'@'$hostipaddr' identified by 'erydb1';
grant REPLICATION SLAVE on *.* to '$repUser'@'$hostipaddr' identified by 'erydb1';
EOD
...
cat /tmp/erydb_master-rep.sql >/tmp/show-master-status.log
$installdir/mysql/bin/mysql \
	--defaults-file=$installdir/mysql/my.cnf \
	--user=root $pwprompt \
	erydbsys </tmp/erydb_master-rep.sql >>/tmp/show-master-status.log
