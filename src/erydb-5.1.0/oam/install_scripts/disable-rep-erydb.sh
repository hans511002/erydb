...
# $Id$
#
# generic EryDB Disable Replication script.
#
...
cat >/tmp/erydb_disable-rep.sql <<EOD
stop slave;
EOD

cat /tmp/erydb_disable-rep.sql >>/tmp/disable-rep-status.log
$installdir/mysql/bin/mysql \
	--defaults-file=$installdir/mysql/my.cnf \
	--user=root $pwprompt \
	erydbsys </tmp/erydb_disable-rep.sql >>/tmp/disable-rep-status.log
