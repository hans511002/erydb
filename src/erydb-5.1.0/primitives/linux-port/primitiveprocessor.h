...
struct ParsedColumnFilter {
	...
	boost::shared_array<erydb_regex_t> prestored_regex;
	uint8_t  likeOps;
...
//	void p_ColAggregate(const NewColAggRequestHeader *in, NewColAggResultHeader *out);
	...
	static int convertToRegexp(erydb_regex_t *regex, const p_DataValue *str);
	inline static bool isEscapedChar(char c);
	boost::shared_array<erydb_regex_t> makeLikeFilter(const DictFilterElement *inputMsg, uint32_t count);
	void setLikeFilter(boost::shared_array<erydb_regex_t> filter) { parsedLikeFilter = filter; }
...
private:
	...
	bool isLike(const p_DataValue *dict, const erydb_regex_t *arg) throw();
...
//    void do_unsignedsum8(NewColAggResultHeader *out, int64_t val);
	...
	boost::shared_array<erydb_regex_t> parsedLikeFilter;
