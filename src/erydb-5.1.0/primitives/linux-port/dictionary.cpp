	...
	boost::shared_ptr<DictEqualityFilter> eqFilter)
{
	...
	boost::scoped_array<erydb_regex_t> regex;
	...
	if ((h->NVALS > 0 && h->COP1 & COMPARE_LIKE) || 
	  (h->NVALS == 2 && h->COP2 & COMPARE_LIKE)) {
		regex.reset(new erydb_regex_t[h->NVALS]);
		...
		switch (h->NVALS) {
			case 1: {
				if (h->COP1 & COMPARE_LIKE) {
					...
					if (utf8) {
						tmp = utf8::erydb_strcoll(sig_utf8.c_str(), arg_utf8.c_str());
	...
	for (nextSig(in->NVALS, in->tokens, &sigptr); sigptr.len != -1;
	  nextSig(in->NVALS, in->tokens, &sigptr), out->Count++) {
		...
		if (utf8) {
			...
			sig_utf8 = tmpString;
			cmp = utf8::erydb_strcoll(cMin_utf8.c_str(), sig_utf8.c_str());
		} else {
		...
		if (utf8) {
			...
			cmp = utf8::erydb_strcoll(cMax_utf8.c_str(), sig_utf8.c_str());
		} else {
...
int PrimitiveProcessor::convertToRegexp(erydb_regex_t *regex, const p_DataValue *str)
{
...
bool PrimitiveProcessor::isLike(const p_DataValue *dict, const erydb_regex_t *regex) throw()
{
#ifdef POSIX_REGEX
...
PrimitiveProcessor::makeLikeFilter (const DictFilterElement *filterString, uint32_t count)
{
	...
	for (filterIndex = 0, filterOffset = 0; filterIndex < count; filterIndex++) {
		...
		if (filter->COP & COMPARE_LIKE) {
			if (!ret)
				ret.reset(new erydb_regex_t[count]);
		...
		if (in->OutputType & OT_AGGREGATE) {
			...
			if (max.len != 0) {
				if (utf8 ) {
					...
					tmp = utf8::erydb_strcoll(sig_utf8.c_str(), max_utf8.c_str());
				} else {
			...
			if (min.len != 0) {
				if (utf8) {
					...
					tmp = utf8::erydb_strcoll(sig_utf8.c_str(), min_utf8.c_str());
				} else {
		...
		for (filterIndex = 0; filterIndex < in->NOPS; filterIndex++) {
			...
			if (filter->COP & COMPARE_LIKE) {
				...
				if (utf8) {
					...
					tmp = utf8::erydb_strcoll(sig_utf8.c_str(), filt_utf8.c_str());
