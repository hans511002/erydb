...
Command* FilterCommand::makeFilterCommand(ByteStream& bs, vector<SCommand>& cmds)
{
	...
	if (columns == CC)
	{
		...
		// due to endian issue
		if (cmd0->getColType().colDataType == execplan::erydbSystemCatalog::CHAR ||
			cmd0->getColType().colDataType == execplan::erydbSystemCatalog::VARCHAR)
		{
...
void FilterCommand::setColTypes(const execplan::erydbSystemCatalog::ColType& left,
								const execplan::erydbSystemCatalog::ColType& right)
{
	leftColType = left;
	rightColType = right;
}
