...
//#endif
  ...
  if(parser.Good())
  {
		...
		for ( unsigned i=0; i < createTable->fTableDef->fColumns.size(); i++ )
		{
			...
			if (createTable->fTableDef->fColumns[i]->fDefaultValue)
			{
				...
				if (!createTable->fTableDef->fColumns[i]->fDefaultValue->fNull)
				{
					...
					if (((colType.colDataType == execplan::erydbSystemCatalog::DATE) && (createTable->fTableDef->fColumns[i]->fDefaultValue->fValue =="0000-00-00")) ||
									((colType.colDataType == execplan::erydbSystemCatalog::DATETIME) && (createTable->fTableDef->fColumns[i]->fDefaultValue->fValue =="0000-00-00 00:00:00")))
					{
						rc = 1;
						thd->get_stmt_da()->set_overwrite_status(true);
						thd->raise_error_printf(ER_INTERNAL_ERROR, "The default value is out of range for the specified data type.");
	...
	if ( comment.find("AUTOINCREMENT") != string::npos )
	{
		int rc = 1;
		thd->get_stmt_da()->set_overwrite_status(true);
		thd->raise_error_printf(ER_CHECK_NOT_IMPLEMENTED, (ERYDBErrorInfo::instance()->errorMsg(ERR_CREATE_AUTOINCREMENT_NOT_SUPPORT)).c_str());
		ci->alterTableState = cal_connection_info::NOT_ALTER;
		ci->isAlter = false;
		return rc;
	}
			...
			if ( ddlpackage::AtaAddColumn *addColumnPtr = dynamic_cast<AtaAddColumn*> (actionList[i]) )
			{
				...
				if ( (addColumnPtr->fColumnDef->fConstraints.size() > 0 ) || addColumnPtr->fColumnDef->fDefaultValue )
				{
					...
					if (addColumnPtr->fColumnDef->fDefaultValue)
					{
						...
						if (((colType.colDataType == execplan::erydbSystemCatalog::DATE) && (addColumnPtr->fColumnDef->fDefaultValue->fValue =="0000-00-00")) ||
									((colType.colDataType == execplan::erydbSystemCatalog::DATETIME) && (addColumnPtr->fColumnDef->fDefaultValue->fValue =="0000-00-00 00:00:00")))
						{
							rc = 1;
							thd->get_stmt_da()->set_overwrite_status(true);
							thd->raise_error_printf(ER_INTERNAL_ERROR, "The default value is out of range for the specified data type.");
			...
			else if ( ddlpackage::AtaAddColumns *addColumnsPtr = dynamic_cast<AtaAddColumns*>(actionList[i]))
			{
				...
				if ( (addColumnsPtr->fColumns[0]->fConstraints.size() > 0 ) || addColumnsPtr->fColumns[0]->fDefaultValue )
				{
					...
					if (addColumnsPtr->fColumns[0]->fDefaultValue)
					{
						...
						if (((colType.colDataType == execplan::erydbSystemCatalog::DATE) && (addColumnsPtr->fColumns[0]->fDefaultValue->fValue =="0000-00-00")) ||
									((colType.colDataType == execplan::erydbSystemCatalog::DATETIME) && (addColumnsPtr->fColumns[0]->fDefaultValue->fValue =="0000-00-00 00:00:00")))
						{
							rc = 1;
							thd->get_stmt_da()->set_overwrite_status(true);
							thd->raise_error_printf(ER_INTERNAL_ERROR, "The default value is out of range for the specified data type.");
