...
//		if (dmlRowCount != 0) //Bug 5117. Handling self join.
	...
	{
		//string errmsg ("Out of range value detected. Please check erydb Syntax Guide for supported data range." );
...
int ha_erydb_impl_rnd_init(TABLE* table)
{
	...
	else
	{
		...
		if (thd->erydb_vtable.vtable_state != THD::ERYDB_SELECT_VTABLE)
		{
			...
			csep->data(thd->erydb_vtable.original_query.c_ptr());
			try {
...
int ha_erydb_impl_delete_table(const char *name)
{
	...
	if (strcmp(first_table->db, "erydbsys") == 0 && string(name).find("@0024vtable") == string::npos)
	{
	...
	int rc = ha_erydb_impl_delete_table_(first_table->db, name, *ci);
	return rc;
}
...
COND* ha_erydb_impl_cond_push(COND *cond, TABLE* table)
{
	...
	if (((thd->lex)->sql_command == SQLCOM_UPDATE) ||
		...
		((thd->lex)->sql_command == SQLCOM_DELETE_MULTI))
		return cond;
	IDEBUG( cout << "ha_erydb_impl_cond_push: " << table->alias.c_ptr() << endl );
