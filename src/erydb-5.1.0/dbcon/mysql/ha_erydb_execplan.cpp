...
ConstantColumn* buildDecimalColumn(Item *item, gp_walk_info &gwi)
{
	...
	return new ConstantColumn(str->c_ptr(), erydb_decimal);
}
...
int cp_get_table_plan(THD* thd, SCSEP& csep, cal_table_info& ti)
{
	...
	if (gwi->columnMap.empty())
	{
		...
		erydbSystemCatalog::TableAliasName tan = make_aliastable(table->s->db.str, table->s->table_name.str, table->alias.ptr());
