	...
	{	
		if (what[0].matched)
		{
			...
			if ( i <= params.length() )
			{
				...
				{
					if (what[0].matched)
						throw runtime_error(ERYDBErrorInfo::instance()->errorMsg(ERR_INVALID_NUMBER_AUTOINCREMENT));
				}
				...
				if ( i <= startVal.length() )
				{
					...
					{
						throw runtime_error(ERYDBErrorInfo::instance()->errorMsg(ERR_INVALID_START_VALUE));
					}
				}
			}
			else
