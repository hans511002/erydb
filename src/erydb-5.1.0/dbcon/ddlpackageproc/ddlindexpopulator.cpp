...
namespace ddlpackageprocessor
{
 ...
 erydbSystemCatalog::ColType DDLIndexPopulator::makeIdxStruct(const ColumnResult* cr, size_t cols, boost::shared_ptr<erydbSystemCatalog> csc )
  {
    IdxStruct idx;
    idx.treeOid = fIdxOID.treeOID;
    idx.listOid = fIdxOID.listOID;
    idx.multiColFlag = cols > 1;
    erydbSystemCatalog::ColType coltype = csc->colType(cr->ColumnOID());
    idx.idxDataType = static_cast<erydbSystemCatalog::ColDataType>(coltype.colDataType);
...
// Workaround to get original column token and not "retokenize" the string value
  ...
  boost::any DDLIndexPopulator::tokenizeData( const execplan::erydbSystemCatalog::ColType& colType, const std::string& data )
  {
...
// @bug 359 use bulk load build
  ...
  bool DDLIndexPopulator::isDictionaryType(const erydbSystemCatalog::ColType& colType)
  {
    return ( (erydbSystemCatalog::CHAR == colType.colDataType && 8 < colType.colWidth ) 
	     || (erydbSystemCatalog::VARCHAR == colType.colDataType  &&  7 < colType.colWidth ) 
	     || (erydbSystemCatalog::DECIMAL == colType.colDataType  &&  18 < colType.precision ));
