...
PseudoColumn::PseudoColumn(const string& schemaName,
                           ...
                           const uint32_t pseudoType,
                           const uint32_t sessionID):
    SimpleColumn(schemaName, tableName, columnName, isEryDB, sessionID),
    fPseudoType(pseudoType)
{
	adjustResultType();
}
...
const string PseudoColumn::toString() const
{
	...
	output << "  s/t/c/v/o/ct/TA/CA/RA/#/card/join/source/engine: " << schemaName() << '/'
	       ...
	       << joinInfo() << '/'
	       << colSource() << '/'
	       << (isEryDB()? "EryDB" : "ForeignEngine") << endl;
...
{
	if (boost::iequals(name, "erydbpm"))
		return PSEUDO_PM;
	if (boost::iequals(name, "erydbdbroot"))
		return PSEUDO_DBROOT;
	if (boost::iequals(name, "erydbextentrelativerid"))
		return PSEUDO_EXTENTRELATIVERID;
	if (boost::iequals(name, "erydbsegment"))
		return PSEUDO_SEGMENT;
	if (boost::iequals(name, "erydbsegmentdir"))
		return PSEUDO_SEGMENTDIR;
	if (boost::iequals(name, "erydbextentmin"))
		return PSEUDO_EXTENTMIN;
	if (boost::iequals(name, "erydbextentmax"))
		return PSEUDO_EXTENTMAX;
	if (boost::iequals(name, "erydbblockid"))
		return PSEUDO_BLOCKID;
	if (boost::iequals(name, "erydbextentid"))
		return PSEUDO_EXTENTID;
	if (boost::iequals(name, "erydbpartition"))
		return PSEUDO_PARTITION;
	if (boost::iequals(name, "erydblocalpm"))
...
void PseudoColumn::adjustResultType()
{
	switch (fPseudoType)
	{
		...
		{
			fResultType.colDataType = erydbSystemCatalog::BIGINT;
			fResultType.colWidth = 8;
			break;
		}
		case PSEUDO_PARTITION:
		{
			fResultType.colDataType = erydbSystemCatalog::VARCHAR;
