...
public:
	...
	virtual inline ERYDB_Decimal getDecimalVal(rowgroup::Row& row, bool& isNull);
...
SimpleColumn_Decimal<len>::SimpleColumn_Decimal(const std::string& schema, 
		           ...
		           SimpleColumn(schema, table, col, isEryDB, sessionID) 
{
	setNullVal();
}
...
template<int len>
inline ERYDB_Decimal SimpleColumn_Decimal<len>::getDecimalVal(rowgroup::Row& row, bool& isNull)
{ 
	if (row.equals<len>(fNullVal, fInputIndex))
		isNull = true;
