...
erydbExecutionPlan* ObjectReader::createExecutionPlan(messageqcpp::ByteStream& b)
{
	CLASSID id = ZERO;
	erydbExecutionPlan *ret;
	
        b.peek(reinterpret_cast<messageqcpp::ByteStream::byte&>(id));
	switch (id) {
		case ERYDBSELECTEXECUTIONPLAN:
			ret = new erydbSelectExecutionPlan();
			break;
		case NULL_CLASS:
...
void ObjectReader::checkType(messageqcpp::ByteStream& b, const CLASSID id)
{
	...
	if (readId != id)
		switch (id) {
			...
			case ERYDBSELECTEXECUTIONPLAN:
				throw UnserializeException("Not a erydbSelectExecutionPlan");
			case NULL_CLASS:
