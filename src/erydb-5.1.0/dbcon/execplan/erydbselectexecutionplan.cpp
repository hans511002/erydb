...
/***********************************************************************
*   $Id: erydbselectexecutionplan.cpp 9576 2013-05-29 21:02:11Z zzhu $
*
*
***********************************************************************/
#include <iostream>
...
/** Static */
erydbSelectExecutionPlan::ColumnMap erydbSelectExecutionPlan::fColMap;

/**
 * Constructors/Destructors
 */
erydbSelectExecutionPlan::erydbSelectExecutionPlan(const int location):
...
erydbSelectExecutionPlan::erydbSelectExecutionPlan(
...
erydbSelectExecutionPlan::erydbSelectExecutionPlan (string data) :
							 fLocalQuery (GLOBAL_QUERY),
							 fData(data),
							 fTxnID(-1),
							 fTraceFlags(TRACE_NONE),
...
void erydbSelectExecutionPlan::filterTokenList( FilterTokenList& filterTokenList)
{
...
void erydbSelectExecutionPlan::havingTokenList( const FilterTokenList& havingTokenList)
{
...
string erydbSelectExecutionPlan::toString() const
{
	...
	switch (location())
	{
		case erydbSelectExecutionPlan::MAIN:
		output << "MAIN" << endl;
		break;
	case erydbSelectExecutionPlan::FROM:
		output << "FROM" << endl;
		break;
	case erydbSelectExecutionPlan::WHERE:
		output << "WHERE" << endl;
		break;
	case erydbSelectExecutionPlan::HAVING:
		output << "HAVING" << endl;
		break;
	}

	// Returned Column
	erydbSelectExecutionPlan::ReturnedColumnList retCols = returnedCols();
	...
	for (unsigned int i = 0; i < retCols.size(); i++)
	{
		...
		if (retCols[i]->colSource() & SELECT_SUB)
		{
			...
			erydbSelectExecutionPlan *plan = dynamic_cast<erydbSelectExecutionPlan*>(fSelectSubList[seq++].get());
			if (plan)
				output << "{" << *plan << "}" << endl;
		}
	}
	...
	// From Clause
	erydbSelectExecutionPlan::TableList tables = tableList();
	...
	for (unsigned int i = 0; i < tables.size(); i++)
	{
		...
		if (tables[i].schema.length() == 0 && tables[i].table.length() == 0)
		{
			...
			erydbSelectExecutionPlan *plan = dynamic_cast<erydbSelectExecutionPlan*>(fDerivedTableList[seq++].get());
			if (plan)
				output << "{" << *plan << "}" << endl;
		}
		else
	...
	// Group by columns
	const erydbSelectExecutionPlan::GroupByColumnList& gbc = groupByCols();
	if (gbc.size() > 0)
	{
	...
	// Order by columns
	const erydbSelectExecutionPlan::OrderByColumnList& obc = orderByCols();
	if (obc.size() > 0)
	{
	...
	{
		erydbSelectExecutionPlan *plan =
			 dynamic_cast<erydbSelectExecutionPlan*>(unionVec()[i].get());
		if (plan)
			output << "{" << *plan << "}\n" << endl;
	}
...
string erydbSelectExecutionPlan::queryTypeToString(const uint32_t queryType)
{
	switch (queryType)
	{
		case SELECT:
...
void erydbSelectExecutionPlan::serialize(messageqcpp::ByteStream& b) const
{
	...
	b << static_cast<ObjectReader::id_t>(ObjectReader::ERYDBSELECTEXECUTIONPLAN);
...
void erydbSelectExecutionPlan::unserialize(messageqcpp::ByteStream& b)
{
	ReturnedColumn *rc;
	...
	ObjectReader::checkType(b, ObjectReader::ERYDBSELECTEXECUTIONPLAN);
...
bool erydbSelectExecutionPlan::operator==(const erydbSelectExecutionPlan& t) const
{
...
bool erydbSelectExecutionPlan::operator==(const erydbExecutionPlan* t) const
{
	const erydbSelectExecutionPlan *ac;

	ac = dynamic_cast<const erydbSelectExecutionPlan*>(t);
	if (ac == NULL)
		return false;
	return *this == *ac;
}
...
void erydbSelectExecutionPlan::columnMap (const ColumnMap& columnMap)
{
...
void erydbSelectExecutionPlan::rmParms (const RMParmVec& parms)
{
