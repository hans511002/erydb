...
public:
  ...
  /** return a copy of this pointer
	...
	virtual ERYDB_Decimal getDecimalVal(rowgroup::Row& row, bool& isNull, ParseTree* lop, ParseTree* rop)
	{
		evaluate(row, isNull, lop, rop);
		...
		if (fResultType.colDataType == erydbSystemCatalog::DOUBLE && fResultType.precision == -1)
		{
			ERYDB_Decimal rv;
			rv.scale = fResultType.scale;
			rv.precision = 15;
			rv.value = (int64_t)(TreeNode::getDoubleVal() * ERYDB_pow[rv.scale]);
	...
	void adjustResultType(const erydbSystemCatalog::ColType& m);

private:
	template <typename result_t>
	inline result_t execute(result_t op1, result_t op2, bool& isNull);
	inline void execute(ERYDB_Decimal& result, ERYDB_Decimal op1, ERYDB_Decimal op2, bool& isNull);
};
...
inline void ArithmeticOperator::execute(ERYDB_Decimal& result, ERYDB_Decimal op1, ERYDB_Decimal op2, bool& isNull)
{
	switch (fOp)
	{
		case OP_ADD:
			...
			if (result.scale >= op1.scale)
				op1.value *= ERYDB_pow[result.scale-op1.scale];
			else
				op1.value = (int64_t)(op1.value > 0 ?
							(double)op1.value/ERYDB_pow[op1.scale-result.scale] + 0.5 :
							(double)op1.value/ERYDB_pow[op1.scale-result.scale] - 0.5);
			if (result.scale >= op2.scale)
				op2.value *= ERYDB_pow[result.scale-op2.scale];
			else
				op2.value = (int64_t)(op2.value > 0 ?
							(double)op2.value/ERYDB_pow[op2.scale-result.scale] + 0.5 :
							(double)op2.value/ERYDB_pow[op2.scale-result.scale] - 0.5);
			result.value = op1.value + op2.value;
			break;
		case OP_SUB:
		...
		case OP_MUL:
			if (result.scale >= op1.scale + op2.scale)
				result.value = op1.value * op2.value * ERYDB_pow[result.scale-(op1.scale+op2.scale)];
			else
				result.value = (int64_t)(( (op1.value > 0 && op2.value >0) || (op1.value < 0 && op2.value < 0) ?
								(double)op1.value * op2.value / ERYDB_pow[op1.scale+op2.scale-result.scale] + 0.5 :
								(double)op1.value * op2.value / ERYDB_pow[op1.scale+op2.scale-result.scale] - 0.5));
			break;
		case OP_DIV:
			if (op2.value == 0)
			...
			if (result.scale >= op1.scale - op2.scale)
				result.value = (int64_t)(( (op1.value > 0 && op2.value >0) || (op1.value < 0 && op2.value < 0) ?
							   (long double)op1.value / op2.value * ERYDB_pow[result.scale-(op1.scale-op2.scale)] + 0.5 :
							   (long double)op1.value / op2.value * ERYDB_pow[result.scale-(op1.scale-op2.scale)] - 0.5));
			else
				result.value = (int64_t)(( (op1.value > 0 && op2.value >0) || (op1.value < 0 && op2.value < 0) ?
								(long double)op1.value / op2.value / ERYDB_pow[op1.scale-op2.scale-result.scale] + 0.5 :
								(long double)op1.value / op2.value / ERYDB_pow[op1.scale-op2.scale-result.scale] - 0.5));
			break;
		default:
		{
