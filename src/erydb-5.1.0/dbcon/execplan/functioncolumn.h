...
public:
	...
	{
		ERYDB_Decimal decimal = fFunctor->getDecimalVal(row, fFunctionParms, isNull, fOperationType);
		if (fResultType.scale == decimal.scale)
			return decimal;

		if (fResultType.scale > decimal.scale)
			decimal.value *= ERYDB_pow[fResultType.scale-decimal.scale];
		else
			decimal.value = (int64_t)(decimal.value > 0 ?
			                (double)decimal.value/ERYDB_pow[decimal.scale-fResultType.scale] + 0.5 :
			                (double)decimal.value/ERYDB_pow[decimal.scale-fResultType.scale] - 0.5);
