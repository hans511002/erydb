...
void ExpressionStep::expressionFilter(const Filter* filter, JobInfo& jobInfo)
{
	...
	erydbassert(fExpressionFilter != NULL);
	if (fExpressionFilter == NULL)
	{
...
void ExpressionStep::populateColumnInfo(ReturnedColumn* rc, JobInfo& jobInfo)
{
	...
	if (rc->resultType().colDataType == erydbSystemCatalog::VARBINARY && !fVarBinOK)
...
{
	// As of bug3695, make sure varbinary is not used in function expression.
	if (sc->resultType().colDataType == erydbSystemCatalog::VARBINARY && !fVarBinOK)
		throw runtime_error ("VARBINARY in filter or function is not supported.");

	erydbSystemCatalog::OID tblOid = joblist::tableOid(sc, jobInfo.csc);
...
//X
		if (ct.scale == 0)       // keep passed original ct for decimal type
			sc->resultType(ct);  // update from mysql type to erydb type
	}
	...
	// @bug 2990, MySQL date/datetime type is different from ERYDB type
	if (ti.dtype == erydbSystemCatalog::DATE || ti.dtype == erydbSystemCatalog::DATETIME)
	{
	...
	erydbSystemCatalog::OID dictOid = joblist::isDictCol(ct);
	if (dictOid > 0)
	{
...
{
	// As of bug3695, make sure varbinary is not used in function expression.
	if (wc->resultType().colDataType == erydbSystemCatalog::VARBINARY && !fVarBinOK)
		throw runtime_error("VARBINARY in filter or function is not supported.");
...
{
	// As of bug3695, make sure varbinary is not used in function expression.
	if (ac->resultType().colDataType == erydbSystemCatalog::VARBINARY && !fVarBinOK)
		throw runtime_error("VARBINARY in filter or function is not supported.");
