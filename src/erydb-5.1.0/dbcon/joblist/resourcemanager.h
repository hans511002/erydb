...
//passed by ExeMgr and DistributedEngineComm to MessageQueueServer or -Client
    ...
    {
		fTraceFlags = flags;
		fHJUmMaxMemorySmallSideDistributor.setTrace(((fTraceFlags & execplan::erydbSelectExecutionPlan::TRACE_RESRCMGR) != 0));
    }
    bool rmtraceOn() const { return ((fTraceFlags & execplan::erydbSelectExecutionPlan::TRACE_RESRCMGR) != 0); }
