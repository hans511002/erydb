...
// This is used for converting a erydb date to an integer representing the day number since the year 0 for use in
...
uint64_t RowEstimator::adjustValue(const execplan::erydbSystemCatalog::ColType& ct,
...
uint32_t RowEstimator::estimateDistinctValues(const execplan::erydbSystemCatalog::ColType& ct,
					      const uint64_t& min,
					      const uint64_t& max,
					      const char cpStatus)
{
...
uint64_t RowEstimator::estimateRows(const vector<ColumnCommandJL*>& cpColVec,
				    const std::vector<bool>& scanFlags,
                                    BRM::DBRM& dbrm,
                                    const execplan::erydbSystemCatalog::OID oid)
