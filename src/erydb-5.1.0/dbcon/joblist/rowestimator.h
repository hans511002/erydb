...
private:
	...
	*
	*/
	uint64_t adjustValue(const execplan::erydbSystemCatalog::ColType& ct,
        	             const uint64_t& value);

	uint32_t daysThroughMonth(uint32_t mth);

	uint32_t estimateDistinctValues(const execplan::erydbSystemCatalog::ColType& ct,
                                        const uint64_t& min,
                                        const uint64_t& max,
                                        const char cpStatus);
	...
	*/
	...
	// Configurables read from erydb.xml - future.
