...
LBIDList::LBIDList(const erydbSystemCatalog::OID oid,
				const int debug)
{
	init(oid, debug);
}
...
void LBIDList::init(const erydbSystemCatalog::OID oid,
				const int debug)
{
    LBIDRange LBIDR;
    fDebug=debug;
...
//
  void LBIDList::UpdateMinMax(int64_t min, int64_t max, int64_t lbid, erydbSystemCatalog::ColDataType type,
		  bool validData)
  {
    MinMaxPartition *mmp=NULL;
#ifdef DEBUG
...
bool LBIDList::CasualPartitionDataType(const erydbSystemCatalog::ColDataType type, const uint8_t size) const
{
	switch(type) {
