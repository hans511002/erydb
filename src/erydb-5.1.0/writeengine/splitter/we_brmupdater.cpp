...
bool WEBrmUpdater::prepareCasualPartitionInfo()
{
	...
	while (aIt != fRef.fBrmRptVec.end())
	{
		...
		if ((!aEntry.empty()) && (aEntry.at(0) == 'C'))
		{
			...
			pTok = strtok(NULL, " ");
			if (pTok)
				cpInfoMerge.type = (execplan::erydbSystemCatalog::ColDataType)atoi(pTok);
			else
			{
				//cout << "CP Entry : " << aEntry << endl;
											   ...
											   int& OorValues)
{
	...
	if ((!Entry.empty()) && (Entry.at(0) == 'D'))
	{
		...
		if (pTok)
		{
			ColType = (erydbSystemCatalog::ColDataType)atoi(pTok);
		}
		else
		{
			//cout << "HWM Entry : " << aEntry << endl;
		...
		if (pTok)
		{
			...
			erydbSystemCatalog::TableColName colname = systemCatalogPtr->colName(columnOid);
