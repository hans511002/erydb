    ...
    uint32_t segNum)
{
    ...
    if ( ERYDBPolicy::exists(backupFileName.c_str()) )
    {
        // Rename current db file to make room for restored file
        rc = ERYDBPolicy::rename( dbFileName, dbFileNameTmp.c_str() );
        if (rc != 0)
        {
            std::ostringstream oss;
            oss << "Error restoring " << colType <<
        ...
        rc = ERYDBPolicy::rename( backupFileName.c_str(), dbFileName );
        if (rc != 0)
        {
            std::ostringstream oss;
            oss << "Error restoring " << colType <<
        ...
        ERYDBPolicy::remove( dbFileNameTmp.c_str() );
    }
