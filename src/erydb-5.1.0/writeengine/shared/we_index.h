...
/*                        ridArray = new RID[size];
   ...
   struct IdxLoadParam {
      ...
      execplan::erydbSystemCatalog::ColDataType indexColDataType;          /** @brief Target index column type */
      int               indexWidth;                /** @brief Target index width */

      int               maxLoadRow;                /** @brief Max rows for one load */

      void  setIdxLoadParam( const OID treeOid, const OID listOid, const execplan::erydbSystemCatalog::ColDataType colDataType, const int width, const int maxRow ) 
