...
def find_paths():
  ...
  try:
    config_file = os.environ['ERYDB_CONFIG_FILE']
  except KeyError:
    try:
        logger.info("Environment variable ERYDB_CONFIG_FILE not set, looking for system erydb.xml")
        config_file = '/usr/local/erydb/etc/erydb.xml'
