...
again:
	...
	else if (ERYDBPolicy::exists(txnidFilename.c_str())) {
		scoped_ptr<ERYDBDataFile> txnidfp(ERYDBDataFile::open(
										ERYDBPolicy::getType(txnidFilename.c_str(),
...
const TxnID SessionManagerServer::newTxnID(const SID session, bool block, bool isDDL) 
{
	...
	semValue--;
	erydbassert(semValue <= (uint32_t)maxTxns);
...
void SessionManagerServer::saveSMTxnIDAndState()
{
	...
	scoped_ptr<ERYDBDataFile> txnidfp(ERYDBDataFile::open(
									ERYDBPolicy::getType(txnidFilename.c_str(), ERYDBPolicy::WRITEENG),
