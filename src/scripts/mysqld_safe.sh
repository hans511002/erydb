...
do
  ...
  max_wsrep_restarts=1
  start_time=`date +%M%S`

  # Perform wsrep position recovery if wsrep_on=1, skip otherwise.
  if test $wsrep_on -eq 1
  then
	cfgDir=`cd "$binDir/../../etc/">/dev/null; pwd`
  	wsrep_erydb_type=$(cat $binDir/../my.cnf |grep wsrep_erydb_type |sed -e 's/.*=//')
        if [ "$wsrep_erydb_type" = "" ] ; then
        	wsrep_erydb_type=0
        fi
	if [ "$wsrep_erydb_type" = "1" -o "$wsrep_erydb_type" = "true"  ] ; then
		cfgFile="$cfgDir/erydb.xml"
...
log_notice "Normal shutdown end_time=$end_time"
...
log_notice "1121 pid_file=$pid_file have_sleep=$have_sleep"
...
log_notice "1179 wsrep_restart=$wsrep_restart max_wsrep_restarts=$max_wsrep_restarts"
  if [ -n "$wsrep_restart" ]
  then
    if [ $wsrep_restart -lt $max_wsrep_restarts ]
    then
