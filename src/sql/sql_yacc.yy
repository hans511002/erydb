...
#include "item_create_window_function.h"   // EryDB Window functions
#include "item_window_function.h"          // EryDB Window functions
#include "sp_head.h"
...
// EryDB
static char erydb_on_clause_err_str[] = "ON clause";
static char erydb_where_clause_err_str[] = "WHERE clause";
static char erydb_having_clause_err_str[] = "HAVING clause";
static char erydb_group_by_clause_err_str[] = "GROUP BY clause";
...
%token  FLUSH_SYM
%token  FOLLOWING_SYM                 /* EryDB Window Function */
%token  FORCE_SYM
...
%token  NTH_VALUE_SYM                 /* @EryDB Window Function */
%token  NULL_SYM                      /* SQL-2003-R */
%token  NULLS_SYM                     /* @EryDB Window Function */
%token  NUM
...
%token  OVER_SYM                      /* EryDB Window Function */
%token  OWNER_SYM
%token  PACK_KEYS_SYM
%token  PAGE_SYM
...
%token  PASSWORD_SYM
%token  PERCENTILE_CONT_SYM           /* EryDB Window Function */
%token  PERCENTILE_DISC_SYM           /* EryDB Window Function */
...
%token  PRECEDING_SYM                 /* EryDB Window Function */
...
%token  RESOURCES
%token  RESPECT_SYM                   /* EryDB Window Function */
%token  RESTORE_SYM
%token  RESTRICT
%token  RESUME_SYM
...
%token  ULONGLONG_NUM
%token  UNBOUNDED_SYM                 /* EryDB Window Function */
...
%token  WITHIN                        /* EryDB Window Function */
...
select_option:
        ...
        | ERYDB_ORDERED_SYM
          {
            Lex->thd->erydb_vtable.override_largeside_estimate=1;
          }
        | SQL_CACHE_SYM
          {
            /* 
...
simple_expr:
        ...
        | function_call_keyword
        | function_call_nonkeyword
        | function_call_window     /* EryDB Window function */
        | function_call_generic
...
function_call_window:
          IDENT_sys '(' opt_udf_expr_list ')' window_clause
          {
            ...
            else
            {
              LEX_STRING args[1];
              args[0] = $1;
              ERYDB_set_error(thd, logging::ERR_WF_FUNCTION_NOT_EXISTS, args, 1);
              MYSQL_YYABORT;
            }
          }
        ...
        | LAST_VALUE '(' expr_list ')' respect window_clause
          ...
          // @erydb the following window functions have the same name
        ...
        | SUM_SYM '(' DISTINCT in_sum_expr ')' window_clause
          {
              ...
              if ($6->ordering)
              {
                ERYDB_set_error(thd, logging::ERR_WF_ORDER_BY_DISTINCT, NULL, 0);
                MYSQL_YYABORT;
              }
...
window_clause:
          OVER_SYM '(' opt_window_partition_by_clause opt_window_order_by_clause ')'
          {
            ...
            {
              ERYDB_set_error(thd, logging::ERR_WF_NOT_ALLOWED, erydb_on_clause_err_str);
              MYSQL_YYABORT;
            }
            if (Select->parsing_place == IN_WHERE)
            {
              ERYDB_set_error(thd, logging::ERR_WF_NOT_ALLOWED, erydb_where_clause_err_str);
              MYSQL_YYABORT;
            }
            if (Select->parsing_place == IN_HAVING)
            {
              ERYDB_set_error(thd, logging::ERR_WF_NOT_ALLOWED, erydb_having_clause_err_str);
              MYSQL_YYABORT;
            }
            if (Select->parsing_place == IN_GROUP_BY)
            {
              ERYDB_set_error(thd, logging::ERR_WF_NOT_ALLOWED, erydb_group_by_clause_err_str);
              MYSQL_YYABORT;
            }
            $$ = new Window_context();
            $$->setPartitions($3);
...
opt_window_order_by_clause:
        ...
        | frame
          {
            ERYDB_set_error(thd, logging::ERR_WF_WINDOW_WITHOUT_ORDER, NULL, 0);
            MYSQL_YYABORT;
          }
        | ORDER_SYM BY window_order_list opt_frame
          {
            ...
            */
            ...
            {
              ERYDB_set_error(thd, logging::ERR_WF_INVALID_ORDER_KEY, NULL, 0);
              MYSQL_YYABORT;
            }
            $$ = new Ordering();
            $$->orders = $3;
...
frame:
          boundary_unit BETWEEN_SYM bounding AND_SYM bounding
          {
            ...
            {
              ERYDB_set_error(thd, logging::ERR_WF_INVALID_WINDOW, NULL, 0);
              MYSQL_YYABORT;
            }
            $$ = new Frame();
            $$->start = $3;
...
/*
  ...
  @EryDB Note: mid-rule action was converted to end rule action to allow analtyic
