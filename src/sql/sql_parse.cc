...
// EryDB replace \t, \n to space.
static std::string erydb_cleanQuery(char* str);
		      ...
		      char* packet, uint packet_length)
{
 ...
 dispatch_end:
	  ...
	  && !(thd->lex->sql_command == SQLCOM_DELETE_MULTI) && !(thd->lex->sql_command == SQLCOM_DELETE))
  {
  	...
  	thd->erydb_vtable.vtable_state = THD::ERYDB_INIT;
  }
...
{ 
	...
	if (thd ){
		if (thd->lex ){
		    // cout<<"thd->lex="<<thd->lex<<endl;
		    // cout<<"thd->query()="<<thd->query()<<endl;
		    // cout<<"thd->lex->sql_command="<<thd->lex->sql_command
		    ...
		    //     <<endl;
		    // cout<<"thd->lex->create_info="<<&thd->lex->create_info<<endl;
		    // cout<<"thd->erydb_vtable.isEryDBDML="<<thd->erydb_vtable.isEryDBDML<<endl;
		    // cout<<"thd->erydb_vtable.hasEryDBTable="<<thd->erydb_vtable.hasEryDBTable<<endl;
		    // cout<<"thd->erydb_vtable.vtable_state="<<thd->erydb_vtable.vtable_state
		    //     <<" THD::ERYDB_SELECT_VTABLE="<<THD::ERYDB_SELECT_VTABLE
		    //     <<" THD::ERYDB_CREATE_VTABLE="<<THD::ERYDB_CREATE_VTABLE
		    //     <<" THD::ERYDB_ALTER_VTABLE="<<THD::ERYDB_ALTER_VTABLE
		    //     <<" THD::ERYDB_DROP_VTABLE="<<THD::ERYDB_DROP_VTABLE
		    //     <<" THD::ERYDB_REDO_QUERY="<<THD::ERYDB_REDO_QUERY
		    //     <<" THD::ERYDB_REDO_PHASE1="<<THD::ERYDB_REDO_PHASE1 
		    //     <<endl; 
            if ( (thd->lex->sql_command == SQLCOM_INSERT ||
                ...
                || thd->lex->sql_command == SQLCOM_CREATE_TABLE || thd->lex->sql_command == SQLCOM_ALTER_TABLE
    			|| thd->lex->sql_command == SQLCOM_DROP_TABLE || thd->lex->sql_command == SQLCOM_RENAME_TABLE))
    		{
			    if((thd->erydb_vtable.vtable_state == THD::ERYDB_SELECT_VTABLE ||
        			...
        			thd->erydb_vtable.isEryDBDML))
    			 {
    			    thd->erydb_vtable.isErydb =true;
    			 }
    			 else 
                    {
                        if( thd->lex->create_info.db_type){
            		        // cout<<"thd->lex->create_info.db_type->slot="<<thd->lex->create_info.db_type->slot<<endl;
            		        if(thd->lex->create_info.db_type->slot >= 0 && thd->lex->create_info.db_type->slot < MAX_HA){
                    			st_plugin_int * ph = hton2plugin[thd->lex->create_info.db_type->slot];
                    		   // cout<<"ph="<<ph<<endl;
                    		    if (ph){
                    		    //cout<<"ph->name.str="<<ph->name.str<<endl;
                    				thd->erydb_vtable.isErydb = strcmp(ph->name.str, "EryDB") == 0;
                    			} 
                            }
            		    }
                    }
		...
		if (!thd->erydb_vtable.isErydb && thd->lex && thd->lex->query_tables && thd->lex->query_tables->table){
			thd->erydb_vtable.isErydb = thd->lex->query_tables->table->isEryDB();
		} 
	}
	if (wsrep_erydb_type && thd->isWsrepQuery && thd->erydb_vtable.isErydb)
	{
		if ((!thd->erydb_vtable.isEryDBDML && 
		     thd->erydb_vtable.hasEryDBTable && 
		     thd->erydb_vtable.vtable_state == THD::ERYDB_SELECT_VTABLE) )
		{
		  push_warning(thd, Sql_condition::WARN_LEVEL_WARN, 9999, 
		                      ...
		                      "with other engines based on rows in EryDB tables "
		                      "are not replicated.");
		  return(0);
		}
		// makes replication ignore erydb_vtable and erydb_querystats
		if ((thd->erydb_vtable.vtable_state == THD::ERYDB_SELECT_VTABLE ||
			thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE ||
			thd->erydb_vtable.vtable_state == THD::ERYDB_ALTER_VTABLE ||
			thd->erydb_vtable.vtable_state == THD::ERYDB_DROP_VTABLE ||
...
// EryDB: Execute a sql statement for a vtable in a clean environment.
int erydb_parse_vtable(THD* thd, String& vquery, THD::erydb_state vtable_state)
{
	DBUG_ENTER("erydb_parse_vtable");
	LEX *old_lex;
...
int erydb_vtable_process(THD* thd, Statement* statement)
{
	...
	{
		thd->erydb_vtable.vtable_state = THD::ERYDB_DISABLE_VTABLE;
		thd->erydb_vtable.autoswitch = false;
	}
	else if (thd->variables.erydb_vtable_mode == 2)
	{
		thd->erydb_vtable.vtable_state = THD::ERYDB_INIT;
		thd->erydb_vtable.autoswitch = true;
	}
	else
	{
		thd->erydb_vtable.vtable_state = THD::ERYDB_INIT;
	...
	// @bug 3014. Erydb does not support lock tables. So if the tables are locked,
	// they must be myisam tables. Change vtable to disable_vtable to make it through.
	if (thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE || thd->locked_tables_mode != LTM_NONE)
	{
		thd->erydb_vtable.vtable_state = THD::ERYDB_DISABLE_VTABLE;
		DBUG_RETURN(-1);
	}
		    ...
		    std::string(thd->query()).find("@@version_comment") == std::string::npos)
		{
		    ...
		    thd->get_command() == COM_STMT_EXECUTE)
			{
				...
				*/
				...
				{
					thd->erydb_vtable.isInsertSelect = true;
				}
...
//							sp_cache_flush_obsolete(&thd->sp_func_cache);
				...
				if (!ERYDB_execute)
				{
					thd->erydb_vtable.call_sp = true;
					if ( isSqlExecute )
					{
						thd->erydb_vtable.call_sp = false;
					}
					delete_explain_query(thd->lex);
					...
					{
						if (thd->erydb_vtable.autoswitch)
						{
				...
				else // normal ERYDB_execute
				{
					...
					// erydb bug 3409. pre-check the outfile access
					...
					thd->erydb_vtable.alter_vtable_query.append(vtable_name.c_str(), vtable_name.length());
					thd->erydb_vtable.alter_vtable_query.append(STRING_WITH_LEN(" engine=erydb comment='SCHEMA SYNC ONLY'"));
					...
					thd->erydb_vtable.drop_vtable_query.append(vtable_name.c_str(), vtable_name.length());
					thd->erydb_vtable.drop_vtable_query.append(STRING_WITH_LEN(" restrict"));

					// thd->erydb_vtable now has all the queries needed.

					// Make sure there's no old vtable around to muddle up things.
					erydb_parse_vtable(thd, thd->erydb_vtable.drop_vtable_query, THD::ERYDB_DROP_VTABLE);

					thd->lex->result = 0;
					thd->erydb_vtable.vtable_state = THD::ERYDB_INIT;
					thd->erydb_vtable.duplicate_field_name = false;
					thd->erydb_vtable.isUnion = false;
					thd->erydb_vtable.mysql_optimizer_off = false;
					thd->erydb_vtable.impossibleWhereOnUnion = false;
					thd->erydb_vtable.isEryDBDML = false;
					thd->erydb_vtable.hasEryDBTable = false;
					...
					else if (thd->erydb_vtable.vtable_state == THD::ERYDB_REDO_PHASE1)
					{
						// redo phase 1;
						while (thd->erydb_vtable.vtable_state == THD::ERYDB_REDO_PHASE1)
						{
							erydb_parse_vtable(thd, thd->erydb_vtable.drop_vtable_query, THD::ERYDB_DROP_VTABLE);

							// If the execution plan failed EryDB, turn off the optimizer constant re-write
							// and let the query go through optimizer again. Change it to CREATE_PHASE
							// because we want to re-generate and send EryDB plan.
							if (thd->erydb_vtable.mysql_optimizer_off)
							{
								thd->erydb_vtable.vtable_state = THD::ERYDB_CREATE_VTABLE;
								thd->erydb_vtable.create_vtable_query.free();
								thd->erydb_vtable.create_vtable_query.append(create.c_str(), create.length());
					...
					else if (thd->erydb_vtable.vtable_state == THD::ERYDB_REDO_QUERY)
					{
...
//						mysql_parse(thd, thd->query(), thd->query_length(), &parser_state);
					...
					// error out erydb non-supported error
					if (thd->erydb_vtable.vtable_state == THD::ERYDB_ERROR)
					{
						if (thd->erydb_vtable.autoswitch || isExplain)
						{
							// auto switch -- vtablemode = 2. rerun the original query
							// We don't need the vtable anymore
							erydb_parse_vtable(thd, thd->erydb_vtable.drop_vtable_query, THD::ERYDB_DROP_VTABLE);
					...
					else if ( thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE )
					{
						erydb_parse_vtable(thd, thd->erydb_vtable.alter_vtable_query, THD::ERYDB_ALTER_VTABLE);
