...
/* EryDB */
static Sys_var_ulong	sys_erydb_vtable_mode(
	   "erydb_vtable_mode",
       "The default mode in which to start EryDB.",
       SESSION_VAR(erydb_vtable_mode),
       CMD_LINE(REQUIRED_ARG),
       VALID_RANGE(0, 2), DEFAULT(1), BLOCK_SIZE(1));

static Sys_var_ulong	sys_erydb_decimal_scale( 
       "erydb_decimal_scale",
       "The default decimal precision for calculated column sub-operations "
	   "in EryDB.",
       SESSION_VAR(erydb_decimal_scale),
	   CMD_LINE(REQUIRED_ARG),
	   VALID_RANGE(0, 18), DEFAULT(8), BLOCK_SIZE(1));

static Sys_var_mybool	sys_erydb_use_decimal_scale(
       "erydb_use_decimal_scale",
       "Enable/disable the EryDB decimal scale to be used internally",
       SESSION_VAR(erydb_use_decimal_scale),
	   CMD_LINE(OPT_ARG),
	   DEFAULT(FALSE));

static Sys_var_mybool   sys_erydb_ordered_only(
       "erydb_ordered_only",
	   "Always use the first table in the from clause as the large side "
	   "table for joins",
       SESSION_VAR(erydb_ordered_only),
	   CMD_LINE(OPT_ARG),
	   DEFAULT(FALSE));

static Sys_var_ulong	sys_erydb_string_scan_threshold(
       "erydb_string_scan_threshold",
	   "Max number of blocks in a dictionary file to be scanned for filtering",
	   SESSION_VAR(erydb_string_scan_threshold),
	   CMD_LINE(REQUIRED_ARG),
	   VALID_RANGE(1, ULONG_MAX), DEFAULT(10), BLOCK_SIZE(1));

static Sys_var_ulong	sys_erydb_compression_type(
       "erydb_compression_type",
	   "The column compression algorithm to use for EryDB tables.",
	   SESSION_VAR(erydb_compression_type),
	   CMD_LINE(REQUIRED_ARG),
	   VALID_RANGE(0, 65535), DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_ulong	sys_stringtable_threshold(
       "erydb_stringtable_threshold",
	   "The minimum width of a string column to be stored in a string table; min=9.",
	   SESSION_VAR(erydb_stringtable_threshold),
	   CMD_LINE(REQUIRED_ARG),
	   VALID_RANGE(9, ULONG_MAX), DEFAULT(20), BLOCK_SIZE(1));

static Sys_var_ulong	sys_diskjoin_smallsidelimit(
       "erydb_diskjoin_smallsidelimit",
	   "The maximum amount of disk space in MB to use per query for storing "
	   "'small side' tables for a disk-based join. (0 = unlimited)",
	   SESSION_VAR(erydb_diskjoin_smallsidelimit),
	   CMD_LINE(REQUIRED_ARG),
	   VALID_RANGE(0, ULONG_MAX), DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_ulong	sys_diskjoin_largesidelimit(
       "erydb_diskjoin_largesidelimit",
	   ...
	   SESSION_VAR(erydb_diskjoin_largesidelimit),
	   CMD_LINE(REQUIRED_ARG),
	   VALID_RANGE(0, ULONG_MAX), DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_ulong	sys_diskjoin_bucketsize(
       "erydb_diskjoin_bucketsize",
	   "The maximum size in MB of each 'small side' table in memory.",
	   SESSION_VAR(erydb_diskjoin_bucketsize),
	   CMD_LINE(REQUIRED_ARG),
...
static Sys_var_ulong	sys_um_mem_limit(
	   ...
	   "(0 = unlimited)",
	   SESSION_VAR(erydb_um_mem_limit),
	   CMD_LINE(REQUIRED_ARG),
	   VALID_RANGE(0, ULONG_MAX), DEFAULT(0), BLOCK_SIZE(1));


static Sys_var_mybool   sys_erydb_varbin_always_hex(
       "erydb_varbin_always_hex",
	   "Always display/process varbinary columns as if they have been hexified.",
	   SESSION_VAR(erydb_varbin_always_hex),
	   CMD_LINE(OPT_ARG),
	   DEFAULT(FALSE));

static Sys_var_mybool   sys_erydb_double_for_decimal_math(
       "erydb_double_for_decimal_math",
	   "Enable/disable the EryDB to replace DECIMAL with DOUBLE in arithmetic operation.",
	   SESSION_VAR(erydb_double_for_decimal_math),
	   CMD_LINE(OPT_ARG),
	   DEFAULT(FALSE));

static Sys_var_ulong    sys_erydb_local_query(
       "erydb_local_query",
	   "Enable/disable the Erydb local PM query only feature.",
	   SESSION_VAR(erydb_local_query),
	   CMD_LINE(REQUIRED_ARG),
	   VALID_RANGE(0,2), DEFAULT(0), BLOCK_SIZE(1));

static Sys_var_mybool	sys_erydb_use_import_for_batchinsert(
       "erydb_use_import_for_batchinsert",
	   ...
	   SESSION_VAR(erydb_use_import_for_batchinsert),
	   CMD_LINE(OPT_ARG),
	   DEFAULT(TRUE));

static Sys_var_ulong    sys_erydb_import_for_batchinsert_delimiter(
       "erydb_import_for_batchinsert_delimiter",
	   "LDI and insert with select will use the input character as delimiter. (default to 7)",
	   SESSION_VAR(erydb_import_for_batchinsert_delimiter),
	   CMD_LINE(OPT_ARG),
	   VALID_RANGE(0, 127), DEFAULT(7), BLOCK_SIZE(1));

static Sys_var_ulong    sys_erydb_import_for_batchinsert_enclosed_by(
       "erydb_import_for_batchinsert_enclosed_by",
	   "LDI and insert with select will use the input character as enclosed by. (default to 17)",
	   SESSION_VAR(erydb_import_for_batchinsert_enclosed_by),
	   CMD_LINE(OPT_ARG),
...
/* EryDB */
static Sys_var_mybool Sys_wsrep_erydb_type(
	"wsrep_erydb_type", "if wsrep replication enable,erydb replication type",
	GLOBAL_VAR(wsrep_erydb_type),
	CMD_LINE(OPT_ARG), DEFAULT(FALSE));
/* EryDB */
