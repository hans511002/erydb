...
{
  DBUG_ASSERT(thd);
  m_thd=thd;                              // EryDB needs access to thd.
  marker= 0;
...
void Item_ident::print(String *str, enum_query_type query_type)
{
  ...
  // @infiniDB
  if (query_type == QT_ERYDB || query_type == QT_ERYDB_NO_QUOTE || query_type == QT_ERYDB_DERIVED)
  {
  	if (query_type != QT_ERYDB_NO_QUOTE && query_type != QT_ERYDB_DERIVED)
  		str->append('`');

  	// print referencing view name for ERYDB post process
  	if (cached_table)
  	{
  		if (cached_table->db && strlen(cached_table->db) > 0)
	  	{
			...
			// ERYDB: table referenced by view is represented by "viewAlias_tableAlias"
			if (cached_table->referencing_view && query_type != QT_ERYDB_DERIVED)
			{
...
{
  DBUG_ASSERT(fixed == 0);
  // @EryDB @bug5501 context is not always set in EryDB context
...
Field *Item::make_string_field(TABLE *table)
{
  ...
  if (table->in_use)
  {
    if (table->in_use->erydb_vtable.vtable_state != THD::ERYDB_DISABLE_VTABLE)
    {
      skipBlob = true;
    }
    else
    {
      ...
      for (; global_list; global_list = global_list->next_global)
      {
        if (global_list->table && global_list->table->isEryDB())
        {
          ...
          ((Item_sum*)this)->thd()->erydb_vtable.vtable_state != THD::ERYDB_DISABLE_VTABLE)
  {
    skipBlob = true;
  }
