  ...
  */
       ...
       IF_WSREP(thd->wsrep_exec_mode != REPL_RECV, 0)) || is_open())
  {
    ...
    */
    ...
    //// EryDB does not allow INSERT SELECT and CREATE TABLE AS SELECT to replicate
    //// on the slave if there is EryDB table involved on the select part.
    //// This is to prevent the scenario that a immediate "DROP TABLE" statement
    //// on master has already dropped the EryDB table on slave.
    ...
    //    (thd->lex->sql_command == SQLCOM_CREATE_TABLE && thd->erydb_vtable.hasEryDBTable))
    //{
    ...
    if ((thd->erydb_vtable.vtable_state == THD::ERYDB_SELECT_VTABLE ||
        ...
        (local_db && strcmp(local_db, "erydb") == 0))
    {
      DBUG_RETURN(0);
    }
