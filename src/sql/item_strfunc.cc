...
void Item_func_format::fix_length_and_dec()
{
  ...
  //@EryDB. @bug3488, update max_length for formatting double data.
...
void Item_func_repeat::fix_length_and_dec()
{
  ...
  if (args[1]->const_item())
  {
    ...
    // @EryDB. For negative count, MySQL returns empty string, ERYDB returns NULL.
    // set length to 0 so order by can go through
    if (count < 0 && thd() && (thd()->erydb_vtable.vtable_state != THD::ERYDB_DISABLE_VTABLE ||
        thd()->variables.erydb_vtable_mode == 0))
    {
      max_length = 0;
      maybe_null = 1;
      return;
  ...
  {
    // @EryDB. For dynamic (column) count, ERYDB sets length 256 for order by to go through
    if (thd() && (thd()->erydb_vtable.vtable_state != THD::ERYDB_DISABLE_VTABLE ||
        thd()->variables.erydb_vtable_mode == 0) )
      max_length = 256;
    else 
      max_length= MAX_BLOB_WIDTH;
