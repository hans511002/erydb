...
/*
EryDB: Checks to see if we should replicate 
vtables and EryDB tables do not replicate
*/
static bool erydb_okay_to_repl(THD* thd, char const* query, uint32 q_len)
{
	char* ptr = 0;

	if (strcmp(thd->db, "erydb_vtable") == 0)
		return FALSE;

	//we only care about a few patterns, and they're always the same because we write them

	//drop table erydb_vtable.$vtable_nnn ...
	ptr = strstr((char*)query, "drop table erydb_vtable.$vtable_");
	if (ptr)
	{
		return FALSE;
	}

	//alter table erydb_vtable.$vtable_nnn ...
	ptr = strstr((char*)query, "alter table erydb_vtable.$vtable_");
	if (ptr)
	{
		return FALSE;
	}
                                    ...
                                    const char *query_arg, uint32 q_len_arg)
{
  ...
  */
  // Add check for EryDB OK to replicate.
  bool erydb_okay = true;
  erydb_okay = erydb_okay_to_repl(thd, query_arg, q_len_arg);
  if (erydb_okay && (is_trans_keyword() || rpl_filter->db_ok(thd->db)) )
  {
