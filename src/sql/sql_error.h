...
// @EryDB util API for error handling
#include "handler.h"                            /* ha_resolve_by_name */
#include "sql_plugin.h"                         /* plugin_ref */
const LEX_STRING EryDB= { C_STRING_WITH_LEN("EryDB") };

// generic API
inline void ERYDB_set_error(THD* thd, uint64_t errCode, LEX_STRING* args, uint argCount)
{
    plugin_ref plugin =     ha_resolve_by_name(thd, &EryDB, false);
    handlerton* hton = plugin_data(plugin, handlerton*);
    hton->set_error(thd, errCode, args, argCount);
}

// one arg API
inline void ERYDB_set_error(THD* thd, uint64_t errCode, char* arg)
{
    plugin_ref plugin =     ha_resolve_by_name(thd, &EryDB, false);
    handlerton* hton = plugin_data(plugin, handlerton*);
    LEX_STRING args[1];
