...
JOIN::optimize_inner()
{
  ...
  */
  ...
  // EryDB: turn off the constant and join subsitute optimization for vtable mode now.
  // It has been found so far that this optimization brings EryDB more harm than good.
  if ((thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE) && ((thd->lex)->sql_command != SQLCOM_UPDATE ) &&
  ...
  {
  	// EryDB: This variable was not initialized and will be set in optimize_cond.
  ...
  */
  ...
  {
	  // @EryDB mysql treats subquery (with other engine?) as const table so will
	  // remove the columns from the subquery on the order by list. EryDB needs to
  ...
  if (const_table_map != found_const_table_map &&
      ...
      !thd->erydb_vtable.isUpdateWithDerive) // @InfinDB Do not let zero_result_cause set for ERYDB update
  {
  ...
  thd->erydb_vtable.isUpdateWithDerive = false; // @InfinDB reset
  ...
  */
  ...
  /* Optimize distinct away if possible */
  {
    ...
    // @EryDB mysql treat subquery (with other engine?) as const table so will
    // remove the columns from the subquery on the order by list. EryDB needs to
    // keep them for the post process. So skip remove_const optimization.
    if (thd->erydb_vtable.vtable_state != THD::ERYDB_CREATE_VTABLE)
  ...
  */
  ...
  if (group)
  {
    ...
    */
    // @EryDB skip MySQL optimization of group by list.
    if (thd->erydb_vtable.vtable_state != THD::ERYDB_CREATE_VTABLE)
	{
      group_list= remove_const(this, group_list, conds,
							   rollup.state == ROLLUP::STATE_NONE,
  ...
  // EryDB change. We don't need tmp table for vtable create phase. Plus
  ...
  if (const_tables != table_count)
  {
    ...
    */
    ...
    if (!(select_options & SELECT_BIG_RESULT) &&
        ...
        thd->erydb_vtable.vtable_state != THD::ERYDB_CREATE_VTABLE) // @EryDB
    {
...
// ------------------------------ erydb EryDB ------------------------------
bool JOIN::exec_erydb()
{
  DBUG_ENTER("JOIN::exec_erydb");
    // There's a lot of EryDB specific stuff to be done in exec_innier(),
  // so rather than clutter up that function, I broke it all out here.
  if (thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE/* && tables_list*/)
  {
    /* @EryDB We've found MySQL gives "Impossible Where" for such query:
     * select * from region where 1 in (select n_regionkey from nation);
     * which confused EryDB processing. Do not redo query if the query has subselect.
     */
    ...
    if (thd->erydb_vtable.isUnion)
      DBUG_RETURN(TRUE);

    //@todo special api to send plan
    TABLE_LIST* tl = tables_list;
    bool hasErydb = false;
    //bool hasNonErydb = false;
    TABLE_LIST* ERYDBtable = NULL;

    // @bug 2976. Check global tables for ERYDB table. If no ERYDB tables involved, redo this query with normal path.
    ...
    // @EryDB. Cross engine support
    else if (/*hasNonErydb && hasErydb*/false)
    {
      const char* emsg = "ERYDB-7001: Non EryDB table(s) on the FROM clause.";
    ...
    if (tables_list && tables_list->table && tables_list->table->isEryDB())
      // ZZ: In deriived_tables_processing phase, the plan out of mysql's optimizer may not be
      // complete. ERYDB will skip the phase in case of plan error instead of error out, so
      // MySQL will continue it's optimization. Now the first table in tables_list may not
      // be an ERYDB table (could be a derived table, for example). In such case, we use an ERYDB table
      // to trigger rnd_init; otherwise, still use the first table in the list.
      // @todo always use ERYDBtable without checking the first table. Still have some plan
      // mistery to solve.
      ERYDBtable = tables_list;

	if (ERYDBtable && ERYDBtable->table && ERYDBtable->table->file)
	{
		if (ERYDBtable->table->file->ha_rnd_init(1))
			thd->erydb_vtable.vtable_state = THD::ERYDB_ERROR;
		...
		if (thd->is_error() && ERYDBtable->table->file->inited == handler::RND)
		{
			ERYDBtable->table->file->ha_rnd_end();
		}
	}
    ...
    if (zero_result_cause)
    {
      ...
      {
        // @EryDB. Bug4422. Need to check all pointer not null
  ...
  else if (!(thd->erydb_vtable.vtable_state == THD::ERYDB_DISABLE_VTABLE) &&
           thd->lex &&
           thd->lex->derived_tables &&
           thd->erydb_vtable.isUnion)
  {
    //thd->erydb_vtable.isUnion = false; // imply derived table
    DBUG_RETURN(TRUE);
  }
  ...
  else if (thd->erydb_vtable.vtable_state == THD::ERYDB_REDO_PHASE1)
  {
    if (!union_part)
    {
      ...
      thd->erydb_vtable.isUnion = true; // make it skip rnd_init for redo phase.
    }
    DBUG_RETURN(TRUE);
  }
  DBUG_RETURN(FALSE);
	     ...
	     SELECT_LEX *select_lex)
{
  ...
  // MCOL-424 Disable indexes on all tables for a EryDB query
  ...
  // When the cross-engine query happens it won't be a EryDB query so
  ...
  if ((thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE) && hasErydb)
  {
    for (global_list = thd->lex->query_tables; global_list; global_list = global_list->next_global)
    {
  ...
  if (select)
  {
      ...
      */
	  ...
	  make_cond_for_table(thd, cond,
        ...
        DBUG_EXECUTE("where",print_where(join->exec_const_cond,"constants",
					 QT_ORDINARY););
        // @EryDB: skip this check for inifinerydb queries
        if (!(thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE))
        {
	...
	*/     
             ...
             inner_tab++)
        {
          ...
          if (!on_expr) continue;      // @EryDB. Is this still needed?
          /*
...
Field *Item::create_tmp_field(bool group, TABLE *table, uint convert_int_length)
{
  ...
  case REAL_RESULT:
    // @EryDB: create field according to arg field type for
                        ...
                        bool make_copy_field)
{
  ...
  /* Fall through */
  case Item::FUNC_ITEM:
  case Item::WINDOW_FUNC_ITEM:  // @EryDB
...
void st_select_lex::print(THD *thd, String *str, enum_query_type query_type)
{
  ...
  */
  ...
  // EryDB skip WHERE clause and after. the print functions are
  // Called just for post process vtable creation.
  if (query_type == QT_ERYDB_NO_QUOTE || query_type == QT_ERYDB_DERIVED)
	return;
