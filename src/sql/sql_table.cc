                           ...
                           uint *key_count, int create_table_mode)
{
  ...
  for (field_no=0; (sql_field=it++) ; field_no++)
  {
    ...
    */
    ...
    // @EryDB: Trim trailing spaces for vtable
    ...
    // @EryDB: Skip the column name checking for vtable creation.
    ...
    for (dup_no=0; (dup_field=it2++) != sql_field; dup_no++)
    {
      ...
      {
      	// @EryDB Ignore duplicate field name error for now. Adjust the field name in get_plan and redo phase 1.
