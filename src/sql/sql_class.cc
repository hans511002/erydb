...
int THD::decide_logging_format(TABLE_LIST *tables)
{
  ...
  */
  bool tbf = tables && tables->table && tables->table->isEryDB();
  if (mysql_bin_log.is_open() && (variables.option_bits & OPTION_BIN_LOG)  &&
      !(wsrep_binlog_format() == BINLOG_FORMAT_STMT && !binlog_filter->db_ok(db))  && !tbf)
  {
    /*
    ...
    */
    ...
    else if ((flags_write_all_set & HA_BINLOG_ROW_CAPABLE) == 0)
    {
      ...
      else if (wsrep_binlog_format() == BINLOG_FORMAT_ROW && !tbf &&
    ...
    if (variables.binlog_format == BINLOG_FORMAT_ROW && !tbf
