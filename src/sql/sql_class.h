...
typedef struct system_variables
{
  ...
  */
  ...
  ulong mrr_buff_size;
/* EryDB */
  ulong erydb_vtable_mode;
  ulong erydb_decimal_scale;
  my_bool erydb_use_decimal_scale;
  my_bool erydb_ordered_only;
  ulong erydb_string_scan_threshold;
  ulong erydb_compression_type;
  ulong erydb_stringtable_threshold;
  ulong erydb_diskjoin_smallsidelimit;
  ulong erydb_diskjoin_largesidelimit;
  ulong erydb_diskjoin_bucketsize;
  ulong erydb_um_mem_limit;
  my_bool erydb_varbin_always_hex;
  my_bool erydb_double_for_decimal_math;
  ulong erydb_local_query;
  my_bool erydb_use_import_for_batchinsert;
  ulong erydb_import_for_batchinsert_delimiter;
  ulong erydb_import_for_batchinsert_enclosed_by;
/* EryDB */
...
public:
  ...
  */
  ...
  enum erydb_state
  {
    ERYDB_INIT_CONNECT = 0,		// intend to use to drop leftover vtable when logon. not being used now.
    ERYDB_INIT,
    ERYDB_CREATE_VTABLE,
    ERYDB_ALTER_VTABLE,
    ERYDB_SELECT_VTABLE,              
    ERYDB_DROP_VTABLE,  
    ERYDB_DISABLE_VTABLE,
    ERYDB_REDO_PHASE1,	    // post process requires to re-create vtable
    ERYDB_ORDER_BY,			// for EryDB handler to ignore the 2nd scan for order by
    ERYDB_REDO_QUERY,		// redo query with the normal mysql path
    ERYDB_ERROR = 32,
  };
  ...
  struct ERYDB_VTABLE
  {
  	...
  	erydb_state vtable_state;  // flag for EryDB MySQL virtual table structure
  	bool autoswitch;
  	bool has_order_by;
	...
	bool isEryDBDML; // default false
  	bool hasEryDBTable; // default false
	bool isNewQuery;
	bool isErydb;
	ERYDB_VTABLE() : autoswitch(false),
		                has_order_by(false),
		                ...
		                hasEryDBTable(false),
		                isNewQuery(true),
						vtable_state(ERYDB_INIT_CONNECT),
						isErydb(false){}
  };			   
   
  ERYDB_VTABLE erydb_vtable;					// EryDB custom structure
...
public:
  uint num_of_tables;  // EryDB - make public
  ...
  virtual void abort_result_set();
  TABLE_LIST* get_tables() {return delete_tables;} // EryDB to get the tables to be deleted from
};
...
// EryDB: Move class Select_fetch_protocol_binary and Prepared_statement's definition from sql_parse.cc here
...
public:
  ...
  // EryDB: make set_parameters public
