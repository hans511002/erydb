...
{
	if (!(current_thd->erydb_vtable.vtable_state == THD::ERYDB_CREATE_VTABLE))
		ERYDB_set_error(current_thd, logging::ERR_WF_ERYDB_ONLY, NULL, 0);
	return 0;
}
...
enum_field_types Item_func_window_hybrid::field_type() const
{
	if (args && arg_count < 1)
	{
		ERYDB_set_error(current_thd, logging::ERR_WF_WRONG_ARGS, funcname);
...
bool Item_func_window_rank::fix_fields(THD* thd, Item** ref)
{
	...
	{
		ERYDB_set_error(thd, logging::ERR_WF_ORDER_MISSING, funcname);
		return TRUE;
	}
	...
	{
		ERYDB_set_error(thd, logging::ERR_WF_WINDOW_CLAUSE, funcname);
		return TRUE;
	}
...
bool Item_func_window_nth_value::fix_fields(THD* thd, Item** ref)
{
	...
	if (!args || arg_count != 4)
	{
		ERYDB_set_error(thd, logging::ERR_WF_WRONG_ARGS, name);
		return TRUE;
	}
	...
	{
		ERYDB_set_error(thd, logging::ERR_WF_WRONG_ARGS, funcname);
		return TRUE;
	}
...
bool Item_func_window_median::fix_fields(THD* thd, Item** ref)
{
	...
	if (window_context && window_context->ordering)
	{
		ERYDB_set_error(thd, logging::ERR_WF_ORDER_BY, funcname);
		return TRUE;
	}
...
bool Item_func_window_percentile::fix_fields(THD* thd, Item** ref)
{
	...
	{
		ERYDB_set_error(thd, logging::ERR_WF_INVALID_ORDER_KEY_WITHIN, funcname);
		return TRUE;
	}
